...
Чтобы добавить пользовательский значок уведомления по умолчанию, добавьте значок в каталог **Resources/Draw** , измените **файл AndroidManifest. XML**и вставьте в `<application>` раздел `<meta-data>` следующий элемент:
...
Чтобы отличить это уведомление от фоновых уведомлений, этот код помечает уведомления значками, которые отличаются от значка приложения. Добавьте файл [IC\_\_stat МФ\_Notification. png](remote-notifications-with-fcm-images/ic-stat-ic-notification.png) в **Resources (ресурсы** ) и включите его в проект **фкмклиент** .
