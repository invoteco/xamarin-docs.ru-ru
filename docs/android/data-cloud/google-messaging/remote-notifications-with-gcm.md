...
- *package_name*`.permission.C2D_MESSAGE` &ndash; регистрирует приложение в Android и запрашивает разрешение на монопольное получение всех сообщений C2D (от облака к устройству). Префикс *PACKAGE_NAME* совпадает с идентификатором приложения. 
