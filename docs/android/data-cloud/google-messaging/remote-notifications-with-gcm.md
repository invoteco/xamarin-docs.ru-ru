...
Прежде чем продолжить работу с этим пошаговым руководством, необходимо получить необходимые учетные данные для использования серверов GCM в Google. Этот процесс описан в [Google Cloud Messaging](~/android/data-cloud/google-messaging/google-cloud-messaging.md).
В частности, вам потребуется *ключ API* и *идентификатор отправителя* для вставки в пример кода, представленного в этом пошаговом руководстве.
...
Это приложение будет использовать новую функцию GCM, известную как *раздел сообщений*. В разделе Обмен сообщениями сервер приложений отправляет сообщение в раздел, а не в список отдельных устройств. Устройства, подписанные на этот раздел, могут получать сообщения раздела в виде push-уведомлений.
...
[![установки Сервисы Google Play](remote-notifications-with-gcm-images/1-google-play-services-sml.png)](remote-notifications-with-gcm-images/1-google-play-services.png#lightbox)
...
- `com.google.android.c2dm.permission.RECEIVE` &ndash; предоставляет приложению разрешение на регистрацию и получение сообщений от Google Cloud Messaging. (Что означает `c2dm`? Это означает _Обмен сообщениями между облаком и устройствами_, который теперь является устаревшим предшественником gcm.
    GCM по-прежнему использует `c2dm` во многих строках разрешений.)

- `android.permission.WAKE_LOCK` &ndash; (необязательно) не позволяет ПРОЦЕССОРу устройства переходить в спящий режим во время прослушивания сообщения.

- `android.permission.INTERNET` &ndash; предоставляет доступ к Интернету, поэтому клиентское приложение может взаимодействовать с GCM.

- *package_name*`.permission.C2D_MESSAGE` &ndash; регистрирует приложение в Android и запрашивает разрешение на монопольное получение всех сообщений C2D (из облака на устройство). Префикс *PACKAGE_NAME* совпадает с идентификатором приложения.
...
При запуске клиентского приложения необходимо убедиться, что Сервисы Google Play доступен, прежде чем пытаться связаться с GCM. Измените **MainActivity.CS** и замените объявление переменной экземпляра ``count`` следующим объявлением переменной экземпляра:
...
Затем замените метод `OnCreate` следующим кодом:
...
[доступна Сервисы Google Play ![](remote-notifications-with-gcm-images/3-first-screen-sml.png)](remote-notifications-with-gcm-images/3-first-screen.png#lightbox)
...
После реализации этого `IntentService`мы протестируем его, чтобы узнать, возвращает ли мы маркер регистрации из GCM.
...
В приведенном выше примере кода измените *YOUR_SENDER_ID* на идентификатор отправителя для проекта клиентского приложения. Чтобы получить идентификатор отправителя для проекта, сделайте следующее:
...
1. Войдите в [консоль Google Cloud](https://console.cloud.google.com/) и выберите свое имя проекта в раскрывающемся меню. В области **сведений о проекте** , отображаемом для проекта, щелкните **Переход к параметрам проекта**:
    ...
    [![выборе проекта Ксамарингкм](remote-notifications-with-gcm-images/7-choose-project-sml.png)](remote-notifications-with-gcm-images/7-choose-project.png#lightbox)

2. На странице **Параметры** выберите **номер проекта** &ndash; это идентификатор отправителя для вашего проекта:
...
Мы хотим запустить наш `RegistrationIntentService`, когда приложение запустится. Измените **MainActivity.CS** и измените метод `OnCreate`, чтобы `RegistrationIntentService` был запущен после проверки наличия сервисы Google Play:
...
Сначала мы зададим `RegistrationIntentService` со следующим атрибутом, чтобы указать, что в системе не создается экземпляр службы:

```csharp
[Service (Exported = false)]
```

Конструктор `RegistrationIntentService` именует рабочий поток *регистратионинтентсервице* , чтобы упростить отладку.

```csharp
public RegistrationIntentService() : base ("RegistrationIntentService") { }
```

Основные функциональные возможности `RegistrationIntentService` размещаются в методе `OnHandleIntent`. Давайте рассмотрим этот код, чтобы увидеть, как он регистрирует приложение с помощью GCM.

#### <a name="request-a-registration-token"></a>Запрос маркера регистрации

`OnHandleIntent` сначала вызывает метод [InstanceId. токена](https://developers.google.com/android/reference/com/google/android/gms/iid/InstanceID.html#getToken&#40;java.lang.String,%20java.lang.String&#41;) Google для запроса маркера регистрации из gcm. Мы переносите этот код в `lock`, чтобы защититься от возможности одновременного выполнения нескольких целей регистрации &ndash; `lock` гарантирует, что эти способы обрабатывались последовательно. Если не удалось получить маркер регистрации, выдается исключение и регистрируется ошибка. Если регистрация будет выполнена, `token` задается маркер регистрации, полученный от GCM:
...
Сервер приложений должен отправлять сообщения с уведомлениями `/topics/global`, если они будут получены. Обратите внимание, что имя раздела в разделе `/topics` может быть любым. при условии, что сервер приложений и клиентское приложение согласны с этими именами. (Здесь мы выбрали имя `global`, чтобы указать, что мы хотим получить сообщения во всех разделах, поддерживаемых сервером приложений.)

#### <a name="implement-an-instance-id-listener-service"></a>Реализация службы прослушивателя ИДЕНТИФИКАТОРов экземпляров

Маркеры регистрации являются уникальными и безопасными. Однако клиентскому приложению (или GCM) может потребоваться обновить маркер регистрации в случае повторной установки приложения или проблемы безопасности. По этой причине необходимо реализовать `InstanceIdListenerService`, реагирующий на запросы обновления маркера от GCM.
...
Закомментировать `InstanceIdListenerService` со следующим атрибутом, чтобы указать, что служба не должна создавать экземпляр службы и может получать запросы на обновление маркера регистрации GCM (также называемого *идентификатором экземпляра*):

```csharp
[Service(Exported = false), IntentFilter(new[] { "com.google.android.gms.iid.InstanceID" })]
```

Метод `OnTokenRefresh` в нашей службе запускает `RegistrationIntentService`, чтобы он мог перехватить новый маркер регистрации.

#### <a name="test-registration-with-gcm"></a>Проверка регистрации с помощью GCM

Давайте полностью перестройте и запустите приложение. Если вы успешно получили маркер регистрации от GCM, маркер регистрации должен отображаться в окне вывода. Пример:
...
Давайте сначала добавим маленький значок, который будет отображаться в области уведомлений при запуске уведомления. [Этот значок](remote-notifications-with-gcm-images/ic-stat-ic-notification.png) можно скопировать в свой проект или создать собственный пользовательский значок. Файл значка будет назван **ic_stat_button_click. png** и скопирован в папку **Resources/Draw** . Не забудьте использовать **добавление > существующий элемент...** , чтобы включить этот файл значка в проект.
...
Во-первых, мы замечаем `GcmListenerService` атрибутом, указывающим, что эта служба не создается системой, и мы включаем фильтр намерения, указывающий, что он получает сообщения GCM:

```csharp
[Service (Exported = false), IntentFilter (new [] { "com.google.android.c2dm.intent.RECEIVE" })]
```

Когда `GcmListenerService` получает сообщение от GCM, вызывается метод `OnMessageReceived`. Этот метод извлекает содержимое сообщения из переданного `Bundle`, записывает содержимое сообщения (поэтому его можно просмотреть в окне вывода) и вызывает `SendNotification` для запуска локального уведомления с полученным содержимым сообщения:
...
Метод `SendNotification` использует `Notification.Builder` для создания уведомления, а затем использует `NotificationManager` для запуска уведомления. Фактически это позволяет преобразовать удаленное сообщение уведомления в локальное уведомление, которое будет представлено пользователю.
...
Прежде чем мы можем получить сообщения от GCM, необходимо объявить прослушиватель GCM в манифесте Android. Давайте изменим **AndroidManifest. XML** и заместим раздел `<application>` следующим кодом XML:
...
В приведенном выше XML-коде измените *YOUR_PACKAGE_NAME* на имя пакета для проекта клиентского приложения. В нашем примере пошагового руководства имя пакета имеет значение `com.xamarin.gcmexample`.
...
Кроме того, можно снабдить `GcmListenerService` атрибутами, а не указывать их в XML. Здесь мы указываем их в **AndroidManifest. XML** , чтобы упростить дальнейшие действия с примерами кода.

### <a name="create-a-message-sender-to-test-the-app"></a>Создание отправителя сообщений для тестирования приложения

Добавим в решение C# проект консольного приложения для настольных систем и назовем его **MessageSender**. Мы будем использовать это консольное приложение для имитации сервера приложений &ndash; он будет отсылать сообщения с уведомлениями в **ClientApp** через gcm.
...
[![установки пакета Json.NET](remote-notifications-with-gcm-images/4-add-json.net-sml.png)](remote-notifications-with-gcm-images/4-add-json.net.png#lightbox)

#### <a name="add-a-reference-to-systemnethttp"></a>Добавление ссылки на System .NET. http

Также необходимо добавить ссылку на `System.Net.Http`, чтобы можно было создать экземпляр `HttpClient` для отправки тестового сообщения в GCM. В проекте **MessageSender** щелкните правой кнопкой мыши **ссылки > Добавить ссылку** и прокрутите вниз, пока не появится **System .NET. http**. Установите флажок рядом с **System .NET. http** и нажмите кнопку **ОК**.
...
Теперь мы готовы протестировать клиентское приложение. Если вы используете эмулятор или ваше устройство взаимодействует с GCM по Wi-Fi, необходимо открыть следующие порты TCP в брандмауэре для получения сообщений GCM: 5228, 5229 и 5230.

Запустите клиентское приложение и просмотрите окно вывода. После того как `RegistrationIntentService` успешно получит маркер регистрации от GCM, в окне вывода должен отобразиться маркер с выходом журнала, похожим на следующий:
