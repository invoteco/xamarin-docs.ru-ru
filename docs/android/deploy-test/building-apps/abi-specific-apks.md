...
- 11413456: версия ABI — `armeabi`; целевой уровень API — 14; экраны от малого до большого размеров; номер версии — 456.
- 21423456: версия ABI — `armeabi-v7a`; целевой уровень API — 14; экраны обычного и большого размеров; номер версии — 456.
- 61423456: версия ABI — `x86`; целевой уровень API — 14; экраны обычного и большого размеров; номер версии — 456.
...
- 11413456: версия ABI — `armeabi`; целевой уровень API — 14; экраны от малого до большого размеров; имя версии — 456.
- 21423457: версия ABI — `armeabi-v7a`; целевой уровень API — 14; экраны обычного и большого размеров; номер версии —  457.
- 61923500: версия ABI — `x86`; целевой уровень API — 19; экраны обычного и большого размеров; имя версии — 500.
...
- `/p:AndroidManifest` &ndash; указывает путь к файлу **AndroidManifest.XML**, который следует использовать при сборке.
...
- `/p:Configuration=Release` &ndash; выполнение сборки выпуска для APK. Отладочные сборки не всегда загружаются в Google Play.

- `<CS_PROJ FILE>` &ndash; путь к файлу `.csproj` для проекта Xamarin.Android.
