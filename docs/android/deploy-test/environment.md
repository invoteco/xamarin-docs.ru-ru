...
Если *ключ* начинается с прописной буквы, то этот *ключ* рассматривается как системное свойство Android, а указанное *значение* используется как *значение по умолчанию*: Поиск системных свойств, которые управляют выполнением Xamarin.Android, сначала осуществляется в хранилище системных свойств Android. Если значение свойства отсутствует, используется значение, указанное в файле среды. Такой механизм позволяет в диагностических целях переопределить значения, указанные в файле среды, с помощью `adb shell setprop`.
...
- `assembly`: вывод `.apk` и сообщений анализа сборки.
- `gc`: вывод сообщений о сборке мусора.
- `gref`: вывод сообщений по глобальным ссылкам JNI.
- `lref`: вывод сообщений по локальным ссылкам JNI.  
    *Примечание*. Это выводит *огромный* объем `adb logcat`.  
...
*Обратите внимание!* Этот вариант можно использовать только с `adb shell setprop
debug.mono.max_grefc`, так как значение не будет доступно в нужное время при использовании файла **environment.txt**.
