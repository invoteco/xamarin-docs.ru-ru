...
**Примечание**. Код `armeabi` в Xamarin.Android не является потокобезопасным и не должен использоваться на многопроцессорных устройствах `armeabi-v7a` (как описано ниже). Код `aremabi` можно безопасно использовать на одноядерных устройствах `armeabi-v7a`.
...
**Примечание.** Хотя платформа Google TV работает на архитектуре x86, ее не поддерживает Android NDK.
...
**Примечание.** Xamarin.Android сейчас не поддерживает устройства MIPS, но поддержка планируется в будущих выпусках.
