...
**Примечание**. Код `armeabi` в Xamarin.Android не является потокобезопасным. Его нельзя использовать на многопроцессорных устройствах `armeabi-v7a` (как описано ниже). Код `armeabi` можно безопасно использовать на одноядерных устройствах `armeabi-v7a`.
