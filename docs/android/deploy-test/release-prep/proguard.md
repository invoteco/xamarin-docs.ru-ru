...
1. **Действие сжатия** — ProGuard рекурсивно определяет, какие классы и члены классов используются. Все остальные классы и члены классов не учитываются. 

2. **Действие оптимизации** — ProGuard оптимизирует код. 
...
3. **Действие скрытия** — при разработке в собственной среде Android средство ProGuard переименовывает классы и члены классов, которые не являются точками входа. Сохраненные точки входа будут по-прежнему доступны по своим исходным именам. Тем не менее этот шаг не поддерживается в Xamarin.Android, так как приложение компилируется до промежуточного языка (IL).

4. **Действие предварительной проверки** — ProGuard выполняет AOT-проверки байт-кода Java и помечает файлы класса для виртуальной машины Java. Это единственное действие, где не используются точки входа. 
...
2. Выберите **ProGuard** из раскрывающегося списка **Средство сжатия кода** в окне **Свойства > Параметры Android**: 

    [![Выбрано средство сжатия кода ProGuard](proguard-images/03-enable-proguard-shrinker-sml.png)](proguard-images/03-enable-proguard-shrinker.png#lightbox)
