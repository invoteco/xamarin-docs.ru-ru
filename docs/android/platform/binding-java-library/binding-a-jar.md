...
В этом пошаговом руководством мы рассмотрим основы создания библиотеки привязок для одного. JAR-файл. Мы продемонстрируем пример, где все &ndash;ся правильно, если не требуется настройка или Отладка привязок. 
...
После создания библиотеки привязок для **Picasso-2. x. x. jar**мы можем вызывать эти методы из C#. Пример:
...
[для уровня API![Target задано значение API 19](binding-a-jar-images/06-set-target-framework-sml.png)](binding-a-jar-images/06-set-target-framework.png#lightbox)

Наконец, создайте библиотеку привязок. Хотя могут отображаться некоторые предупреждающие сообщения, проект библиотеки привязок должен быть успешно построен и выдавать выходные данные. DLL в следующем расположении: **жарбиндинг/bin/debug/жарбиндинг. dll**
...
В следующих шагах мы создадим минимальное приложение, которое использует библиотеку привязок для загрузки и вывода изображения в `ImageView`; «тяжелая работа» выполняется кодом, который находится в. JAR-файл. 
...
[![выбрать проект Жарбиндинг](binding-a-jar-images/09-choose-jar-binding-sml.png)](binding-a-jar-images/09-choose-jar-binding.png#lightbox)
...
Измените макет **биндингтест** (**Main. axml**), чтобы он был одним `ImageView`:
...
Добавьте следующую инструкцию `using` в **MainActivity.cs** &ndash; это делает возможным простой доступ к методам класса `Picasso` на основе Java, который находится в библиотеке привязок:

```csharp
using Com.Squareup.Picasso;
```

Измените метод `OnCreate`, чтобы в нем использовался класс `Picasso`, чтобы загрузить изображение из URL-адреса и отобразить его в `ImageView`: 
