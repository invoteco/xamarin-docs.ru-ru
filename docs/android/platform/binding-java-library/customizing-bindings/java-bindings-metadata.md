...
**Библиотека привязки Java** для Xamarin. Android пытается автоматизировать большую часть работы, необходимой для привязки существующей библиотеки Android, с помощью средства, которое иногда называют _генератором привязок_. При привязке библиотеки Java Xamarin. Android проверяет классы Java и создает список всех пакетов, типов и элементов, которые должны быть привязаны. Этот список интерфейсов API хранится в XML-файле, который можно найти  **\{в каталоге проекта} \обж\релеасе\апи.ксмл** для сборки **выпуска** и в  **\{каталоге проекта} \обж\дебуг\апи.ксмл** для **отладочной** сборки.
