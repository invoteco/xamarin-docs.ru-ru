...
[Веб-представления](https://developer.xamarin.com/api/type/Android.Webkit.WebView/) есть преобразование KitKat. Самым значительным изменением является Усиленная защита для загрузки содержимого в `WebView`. Хотя большинство приложений, предназначенных для более старых версий API должен работать соответствующим образом, тестирование приложений, использующих `WebView` настоятельно рекомендуется использовать класс. Дополнительные сведения о затронутые API веб-представления см. Android [переход на веб-представления в Android 4.4](https://developer.android.com/guide/webapps/migrating.html) документации.
...
В следующем образце кода определяет связующее звено, который выполняет анимацию в течение 5 секунд и использует [перенапряжение интерполятора](https://developer.android.com/reference/android/views/animation/OvershootInterpolator.html):
...
Дополнительные сведения о загрузке, создание и редактирование содержимого с помощью платформы доступа хранилища см [документации для платформы доступа хранилища](https://developer.android.com/guide/topics/providers/document-provider.html).
...
Команда «Печать» она обычно будет отображаться в действии [меню параметров](https://developer.android.com/guide/topics/ui/menus.html#options-menu).
...
KitKat включает в себя новый экран, возможности записи, таким образом, разработчики могут записывать приложения в действии. Запись экрана доступна через [Android Debug Bridge (ADB)](https://developer.android.com/tools/help/adb.html) клиент, который можно загрузить как часть пакета SDK для Android.
...
-  *Используйте в полноэкранном режиме* -KitKat вводится новый [Иммерсивном режиме](https://developer.android.com/reference/android/view/View.html#setSystemUiVisibility(int)) для просмотра содержимого, игры и выполнения других приложений, которые выиграют от полноэкранного исследования.
...
-  *Зеркальное отображение Прорисовываемые ресурсы* -Прорисовываемые ресурсы имеют новый [`autoMirrored`](https://developer.android.com/reference/android/R.attr.html#autoMirrored)
...
-  *Чтение динамически изменение текста* -обозначают части пользовательского интерфейса, которые динамически обновить новым текстом «live» регионы с новым [`accessibilityLiveRegion`](https://developer.android.com/reference/android/R.attr.html#accessibilityLiveRegion)
...
Дополнительные сведения об изменениях API выше, обратитесь к Google [API-интерфейсы Android 4.4](https://developer.android.com/about/versions/android-4.4.html) Обзор.
