...
- [Взаимодействие с пользователем](#user_experience) &ndash; простую анимацию с помощью инфраструктуры переходов, полупрозрачного состояния и панелей навигации, а также полноэкранного режима, позволяющие пользователю повысить удобство работы.

- [Пользовательское содержимое](#user_content) &ndash; Управление файлами пользователей упрощено с помощью платформы доступа к хранилищу; Печать изображений, веб-сайтов и другого содержимого упрощается благодаря улучшенным интерфейсам API печати.

- [Оборудование](#hardware) &ndash; превратить любое приложение в NFC-карту с эмуляцией карты на основе поставщика NFC; Запускайте датчики с низким энергопотреблением с `SensorManager`.

- [Средства для разработчиков](#developer_tools) &ndash; приложения ролика в действии с Android Debug Bridge клиентом, доступным как часть пакет SDK для Android.
...
[![выборе Android 4,4 в диспетчере пакет SDK для Android](kitkat-images/api19.png)](kitkat-images/api19.png#lightbox)
...
Чтобы перевести приложение в спящий режим в течение заданного интервала времени, вызовите `SetWindow` на [`AlarmManager`](xref:Android.App.AlarmManager), передав минимальное и максимальное время в миллисекундах, которое может пройти до того, как приложение будет пробуждении, и выполнить операцию при пробуждении.
...
В приведенном выше примере используется платформа переходов для создания автоматического перехода между изменяющимися значениями свойств по умолчанию. Поскольку анимация обрабатывается одной строкой кода, вы можете легко обеспечить совместимость с более старыми версиями Android, заключив `BeginDelayedTransition` вызов в проверку версии системы. Дополнительные сведения см. в разделе [миграция приложения в KitKat](#Migrating_Your_App_to_KitKat) .
...
Статический элемент пользовательского интерфейса, размещает динамическое содержимое, называется *контейнером* или *основой сцены*. В приведенном ниже примере используется Android Designer для создания `RelativeLayout` с именем `container`.

[![использования Android Designer для создания контейнера RelativeLayout](kitkat-images/container.png)](kitkat-images/container.png#lightbox)

В примере макета также определена кнопка, которая называется `sceneButton` под `container`. Эта кнопка вызывает переход.
...
В приведенном выше примере используется `merge`, чтобы сделать код представления короче и упростить иерархию представлений. Дополнительные сведения о макетах `merge` см. [здесь](https://android-developers.blogspot.com/2009/03/android-layout-tricks-3-optimize-by.html).

Сцена создается путем вызова [`Scene.GetSceneForLayout`](xref:Android.Transitions.Scene.GetSceneForLayout*), передачи объекта контейнера, идентификатора ресурса файла макета сцены и текущего `Context`, как показано в следующем примере кода:
...
> [!NOTE]
> В библиотеке переходов Android есть [известная ошибка](https://code.google.com/p/android/issues/detail?id=62450) , из-за которой сцены, созданные с помощью `GetSceneForLayout`, прерываются, когда пользователь переходит по действию во второй раз. Решение Java описывается [здесь](http://www.doubleencore.com/2013/11/new-transitions-framework/).
...
- `fitsSystemWindows` — установка верхней или нижней полосы по умолчанию трансклуент сдвигает содержимое в прозрачных элементах пользовательского интерфейса. Присвоение этому свойству значения `true` — это простой способ предотвратить перекрытие содержимого с полупрозрачными элементами пользовательского интерфейса системы.
...
[![Пример снимка экрана приложения со полупрозрачными состояниями и панелями навигации](kitkat-images/theme.png)](kitkat-images/theme.png#lightbox)
...
В KitKat взаимодействия с `ContentProviders` абстрактны с классом `DocumentsProvider`. Это означает, что SAF не следит за физическими данными, пока они доступны через API `DocumentsProvider`. Локальные поставщики, облачные службы и внешние запоминающие устройства используют один и тот же интерфейс и обрабатываются таким же образом, предоставляя пользователю и разработчику одно место для взаимодействия с содержимым пользователя.
...
Мы можем сообщить KitKat, что нам нужно выбрать содержимое с помощью пользовательского интерфейса SAF, с целью `ActionOpenDocument`, что означает, что мы хотим подключиться ко всем поставщикам содержимого, доступным для устройства. Можно добавить фильтрацию по этому намерению, указав `CategoryOpenable`. Это означает, что будет возвращено только содержимое, которое можно открыть (т. е. доступное для использования содержимое). KitKat также позволяет фильтровать содержимое с `MimeType`. Например, код, приведенный ниже, фильтрует результаты по изображению, указывая `MimeType`изображения:
...
После того как пользователь выбрал изображение, `OnActivityResult` возвращает `Android.Net.Uri` выбранного файла. В приведенном ниже примере кода отображается выбранный пользователем образ:
...
Когда пользователь нажимает кнопку " **сохранить**", `OnActivityResult` передается `Android.Net.Uri` созданного файла, доступ к которому можно получить с помощью `data.Data`. Универсальный код ресурса (URI) можно использовать для потоковой передачи данных в новый файл:
...
[снимок экрана![пример экрана параметров печати](kitkat-images/printing.png)](kitkat-images/printing.png#lightbox)
...
KitKat автоматически создает [`PrintDocumentAdapter`](xref:Android.Print.PrintDocumentAdapter) для веб-представления с `WebView.CreatePrintDocumentAdapter`. Печать веб-содержимого — это координированные усилия между [`WebViewClient`](xref:Android.Webkit.WebViewClient) , ожидающими загрузки содержимого HTML, и позволяет действию сообщить о том, что параметр печати доступен в меню Параметры, и действие, которое ожидает от пользователя выбора параметра Print и c. alls `Print`на `PrintManager`. В этом разделе описывается базовая настройка, необходимая для печати содержимого на экране на экранах HTML.
...
[![Пример снимка экрана пункта меню "Печать", отображаемого в правом верхнем углу экрана](kitkat-images/menu.png)](kitkat-images/menu.png#lightbox)
...
Взаимодействие с меню Options в действии выполняется с помощью методов `OnCreateOptionsMenu` и `OnOptionsItemSelected`.
...
Приведенный выше код также определяет переменную с именем `dataLoaded`, чтобы контролировать состояние содержимого HTML. `WebViewClient` задаст этой переменной значение true после загрузки всего содержимого, чтобы действие знало добавить пункт меню "Печать" в меню "Параметры".

##### <a name="webviewclient"></a>WebViewClient

Задача `WebViewClient` заключается в том, чтобы убедиться, что данные в `WebView` полностью загружены, прежде чем в меню появится параметр Печать, который выполняется с помощью метода `OnPageFinished`. `OnPageFinished` прослушивает веб-содержимое, чтобы завершить загрузку, и предписывает действию повторно создать меню параметров с `InvalidateOptionsMenu`:
...
`OnPageFinished` также задает для `dataLoaded` значение `true`, поэтому `OnCreateOptionsMenu` может повторно создать меню с помощью параметра Печать на месте.
...
(`null` в примере выше). Можно указать `PrintAttributes`, чтобы разместить содержимое на печатной странице, хотя атрибуты по умолчанию должны работать в большинстве случаев.
...
[![снимок экрана: меню "Сохранить как PDF"](kitkat-images/print2.png)](kitkat-images/print2.png#lightbox)
...
Эмуляция карт на основе узла (ХЦЕ) позволяет приложениям работать как с NFC-картами или устройствами чтения NFC, не полагаясь на собственный защищенный элемент перевозчика. Перед настройкой ХЦЕ убедитесь, что ХЦЕ доступен на устройстве с `PackageManager.HasSystemFeature`:

```csharp
bool hceSupport = PackageManager.HasSystemFeature(PackageManager.FeatureNfcHostCardEmulation);
```

ХЦЕ требует, чтобы функция ХЦЕ и разрешение `Nfc` были зарегистрированы в `AndroidManifest.xml`приложения:

```xml
<uses-feature android:name="android.hardware.nfc.hce" />
```

[![установке разрешения NFC в параметрах приложения](kitkat-images/nfc.png)](kitkat-images/nfc.png#lightbox)

Для работы ХЦЕ должен быть способен работать в фоновом режиме, и он должен запускаться, когда пользователь выполняет NFC-транзакцию, даже если приложение, использующее ХЦЕ, не запущено. Это можно сделать, написав код ХЦЕ как `Service`. Служба ХЦЕ реализует интерфейс `HostApduService`, который реализует следующие методы:
...
- *OnDeactivate* — `HostAdpuService` деактивируется, когда служба хце больше не взаимодействует с NFC-сканером.

Служба ХЦЕ также должна быть зарегистрирована в манифесте приложения и дополнена соответствующими разрешениями, фильтром намерения и метаданными. Следующий код представляет собой пример `HostApduService`, зарегистрированного в манифесте Android с помощью атрибута `Service` (Дополнительные сведения об атрибутах см. в руководстве по [работе с манифестом Xamarin Working с Android](~/android/platform/android-manifest.md) ).
...
Приведенная выше служба предоставляет NFC-сканеру способ взаимодействия с приложением, но средство чтения NFC, тем не менее, не имеет возможности узнать, эмулирует ли эта служба NFC-карту, которую необходимо проверить. Чтобы помочь NFC-сканеру идентифицировать службу, можно назначить службе уникальный *идентификатор приложения (помощь)* . Мы указываем ВСПОМОГАТЕЛЬное средство, а также другие метаданные о службе ХЦЕ в файле ресурсов XML, зарегистрированном с помощью атрибута `MetaData` (см. пример кода выше). В этом файле ресурсов указывается один или несколько фильтров поддержки — строк уникальных идентификаторов в шестнадцатеричном формате, которые соответствуют вспомогательным средствам одного или нескольких устройств NFC-сканера:
...
Можно создать `SensorManager`, вызвав `GetSystemService(SensorService)` и приведя результат в качестве `SensorManager`. Чтобы использовать счетчик шагов, вызовите `GetDefaultSensor` на `SensorManager`. Вы можете зарегистрировать датчик и прослушать изменения в количестве шагов с помощью [`ISensorEventListener`](xref:Android.Hardware.ISensorEventListener)
...
`OnSensorChanged` вызывается, если количество шагов обновляется, пока приложение находится на переднем плане. Если приложение входит в фоновый режим или устройство находится в спящем режиме, `OnSensorChanged` не будет вызываться; Однако действия будут учитываться до тех пор, пока не будет вызван `UnregisterListener`.

Помните, что *значение счетчика шагов является совокупным для всех приложений, регистрирующих датчик*. Это означает, что даже если удалить и переустановить приложение и инициализировать переменную `count` в 0 при запуске приложения, значение, сообщаемое датчиком, останется общим числом шагов, выполненных при регистрации датчика, будь то или другого приложения. Вы можете запретить приложению добавлять в счетчик шагов, вызвав `UnregisterListener` на `SensorManager`, как показано в следующем коде:
...
> [!NOTE]
> Хотя API для обнаружения и инвентаризации шагов поставляется с KitKat, не все телефоны имеют датчик. Проверьте, доступен ли датчик, запустив `PackageManager.HasSystemFeature(PackageManager.FeatureSensorStepCounter);`или убедитесь, что возвращаемое значение `GetDefaultSensor` не `null`.
...
- *Зеркальные отображаемые ресурсы* . у создаваемых ресурсов есть новый [`autoMirrored`](https://developer.android.com/reference/android/R.attr.html#autoMirrored)
...
- *Приостановка анимации* — Приостановка и возобновление анимации, созданной с помощью [`Animator`](xref:Android.Animation.Animator)
  .

- *Чтение динамически изменяемых частей текста* в пользовательском интерфейсе, которые динамически обновляются новым текстом как "динамические регионы" с новым [`accessibilityLiveRegion`](https://developer.android.com/reference/android/R.attr.html#accessibilityLiveRegion)
  атрибут, поэтому новый текст будет автоматически прочитан в режиме специальных возможностей.

- *Улучшение качества звука* — делайте дорожки с помощью [`LoudnessEnhancer`](xref:Android.Media.Audiofx.LoudnessEnhancer)
  , найдите пиковую и RMS аудиопотока с помощью [`Visualizer`](xref:Android.Media.Audiofx.Visualizer.MeasurementModePeakRms)
  и получить сведения из [метки времени звука](xref:Android.Media.AudioTimestamp) , чтобы помочь при синхронизации аудио-видео.

- *ContentResolver синхронизации с настраиваемым интервалом* — KitKat добавляет некоторую вариативность к времени выполнения запроса на синхронизацию. Синхронизируйте `ContentResolver` в настраиваемое время или интервал, вызвав `ContentResolver.RequestSync` и передав `SyncRequest`.

- *Различие контроллеров* . в KitKat контроллерам назначаются уникальные целочисленные идентификаторы, доступ к которым можно получить с помощью свойства `ControllerNumber` устройства. Это упрощает разделение игроков в игре.

- *Удаленное управление* . с несколькими изменениями на стороне оборудования и программного обеспечения KitKat позволяет превратить устройство с инфракрасным приемопередатчиком в пульт дистанционного управления с помощью `ConsumerIrService`и взаимодействовать с периферийными устройствами с новым [`RemoteController`](xref:Android.Media.RemoteController)
  Интерфейсы.
