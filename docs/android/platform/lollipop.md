...
- **Анимации** &ndash; анимации *касаний* , анимации *переходов на действия* , анимации изменения *состояния просмотра* и *результата отображения*.

- Представления **теней и повышения уровня** &ndash; теперь имеют свойство `elevation`;   представления с более высоким значением `elevation` приводят к более крупным теням на фоне.

- **Цветовая палитра** &ndash; *графические оттенки* позволяют повторно использовать ресурсы изображений, изменяя их цвет, и *выразительное извлечение цветов* помогает динамически создать тему приложения на основе цветов в изображении.
...
- **Улучшенные уведомления** &ndash; уведомления в Android 5,0 были значительно обновлены с новым оформлением, поддержкой уведомлений по отрезкой экрана и новым форматом представления уведомлений в виде *заголовков* .

- **Новые графические элементы пользовательского интерфейса** &ndash; нового мини-приложения `RecyclerView` облегчают приложениям передачу больших наборов данных и сложной информации, а новое мини-приложение `CardView` предоставляет упрощенный формат представления для отображения текста и изображений.

- **Новые API-интерфейсы** &ndash; Android 5,0 добавляют новые API для поддержки нескольких сетей, улучшенное подключение Bluetooth, упрощенное управление хранением и более гибкое управление мультимедийными проигрывателями и устройствами камеры. Новая функция планирования заданий доступна для асинхронного выполнения задач в запланированное время. Эта функция помогает повысить время работы батареи, например, при планировании задач, выполняемых при подключении устройства к сети и зарядки.
...
3. Запустите диспетчер пакет SDK для Android (в Visual Studio для Mac используйте **средства &gt; открыть пакет SDK для Android Manager&hellip;** ) и установите Android SDK Tools 23.0.5 или более поздней версии.
   ...
   Кроме того, установите последние пакеты SDK для Android 5,0 (API 21 или более поздней версии):
    ...
    [![установке пакетов SDK для Android 5,0 в диспетчере пакет SDK для Android](lollipop-images/android-l-sdk-pkgs-sml.png)](lollipop-images/android-l-sdk-pkgs.png#lightbox)
...
4. Создайте новый проект Xamarin. Android. Если вы не знакомы с разработкой Android с помощью Xamarin, ознакомьтесь со статьей [Hello, Android,](~/android/get-started/hello-android/index.md) чтобы узнать о создании проектов Android. При создании проекта Android обязательно настройте параметры версии для Android 5,0.
   В Visual Studio для Mac перейдите в раздел **Параметры проекта &gt; сборка &gt; общие** и задайте в **качестве целевой платформы** **Android 5,0 (без описания операций)** или более поздней версии:
...
Примечание. при обновлении существующего проекта Android, предназначенного для Android L Preview, необходимо обновить **целевую платформу** и **версию Android** на значения, описанные выше.
...
- JIT- **компиляция** &ndash; AOT может улучшить производительность приложения, выполняя компиляцию кода приложения перед первым запуском приложения. При установке приложения ART создает исполняемый файл скомпилированного приложения для целевого устройства.

- **Улучшенная сборка мусора (GC)** &ndash; улучшения GC в картинках также могут повысить производительность приложения. Сборка мусора теперь использует одну приостановку GC вместо двух, а параллельные операции GC выполняются более своевременно.
...
Существующие приложения должны работать без изменений в &ndash; ART, за исключением приложений, которые используют методы, уникальные для предыдущей среды выполнения Dalvik могут вызывать, которая может не работать на иллюстрации. Дополнительные сведения об этих изменениях см. в разделе [Проверка поведения приложения в среде выполнения Android (Иллюстрация)](https://developer.android.com/guide/practices/verifying-apps-art.html).

### <a name="notification-changes"></a>Изменения уведомлений

В Android 5,0 значительно изменились уведомления:

- **Звуки и вибрация обрабатываются** по-разному &ndash; звуки уведомлений и вибрации теперь обрабатываются `Notification.Builder` а не `Ringtone`, `MediaPlayer`и `Vibrator`.

- **Новые цветовые схемы** &ndash; в соответствии с темой материала, уведомления отображаются с темным текстом на белой или очень светлой фоне. Кроме того, альфа-каналы в значках уведомлений могут быть изменены Android для координации с системными схемами цветов.

- **Уведомления экрана** от&ndash; могут отображаться на экране отправки экрана устройства.

- В **Заголовке** &ndash; уведомления с высоким приоритетом теперь отображаются в маленьком плавающем окне (головное уведомление), когда устройство разблокировано и экран включен.
...
Если в уведомлениях представлены элементы управления передачей, отображение состояния воспроизведения мультимедиа, использование `RemoteControlClient`или вызов `ActivityManager.GetRecentTasks`, см. раздел [важные изменения в поведении](https://developer.android.com/preview/api-overview.html#Behaviors) для получения дополнительных сведений об обновлении уведомлений для Android 5,0.
...
[![снимки экрана домашней страницы темы материалов, экрана приложений и параметров экрана](lollipop-images/android-5-gallery-labeled-sml.png)](lollipop-images/android-5-gallery-labeled.png#lightbox)
...
Тема материалов включает три встроенных варианта, которые можно использовать в приложении: `Theme.Material` темная тема (по умолчанию), тема `Theme.Material.Light` и тема `Theme.Material.Light.DarkActionBar`:
...
Анимация с помощью сенсорных отзывов позволяет пользователям визуально отдать отзыв о том, что представление было затронуто. Например, кнопки теперь могут отображать эффект Ripple, если они затронуты &ndash; это анимация по умолчанию для сенсорных отзывов в Android 5,0. Анимация Ripple реализуется новым классом `RippleDrawable`. Эффекты Ripple можно настроить на окончание в границах представления или выходят за границы представления. Например, следующая последовательность снимков экрана иллюстрирует эффект Ripple в кнопке во время анимации касания:
...
- **Введите &ndash; перехода** для, когда действие входит в сцену.

- **Выход из &ndash; перехода** , когда действие выходит из сцены.

- &ndash; **перехода общего элемента** для, когда представление, которое является общим для двух действий, изменяется при переходе к следующему действию.

Например, следующая последовательность снимков экрана иллюстрирует переход общего элемента:

[![снимка кадра по кадрам для анимации перехода общего элемента](lollipop-images/activity-transition-sml.png)](lollipop-images/activity-transition.png#lightbox)
...
- **Развернуть анимацию** &ndash; увеличивает масштаб представления в центре сцены.

- **Анимация слайда** &ndash; перемещает представление из одного края сцены.

- Эффект **затухания** &ndash; плавное представление сцены.
...
- **Анимация слайда** &ndash; перемещает представление на один из граней сцены.
...
- Создайте драваблес, чтобы анимировать изменения состояния, связанные с конкретным представлением. Новый класс `AnimatedStateListDrawable` позволяет создавать драваблес, которые отображают анимацию между изменениями состояния представления.

- Определите функции анимации, выполняемые при изменении состояния представления. Новый класс `StateListAnimator` позволяет определить аниматор, который выполняется при изменении состояния представления.
...
[![снимков экрана кадров для отображения анимации](lollipop-images/reveal-center-sml.png)](lollipop-images/reveal-center.png#lightbox)

Следующая последовательность иллюстрирует анимацию эффекта отображения, которая выполняется в левом нижнем углу экрана:

[![снимков кадра по кадрам для анимации обрезки](lollipop-images/reveal-left-sml.png)](lollipop-images/reveal-left.png#lightbox)
...
- **Быстрое\_\_линейные\_в** &ndash; быстро ускоряется и ускоряется до окончания анимации.

- **Быстрое\_\_медленное\_в** &ndash; ускоряется и медленно замедляется в сторону конца анимации.

- **Линейное\_out\_медленный\_в** &ndash; начинается с пиковой скорости и медленно замедляется до конца анимации.

Вы можете использовать новый класс `PathInterpolator`, чтобы указать, как происходит интерполяция движения. `PathInterpolator` — это интерполяция, которая проходит по путям анимации в соответствии с заданными контрольными точками и кривыми движения. Дополнительные сведения об указании параметров криволинейного движения в Android 5,0 см. в разделе [Использование криволинейного движения](https://developer.android.com/training/material/animations.html#CurvedMotion).

## <a name="view-shadows--elevation"></a>Просмотр теней & повышения прав

В Android 5,0 можно указать *повышение уровня* представления, задав новое свойство `Z`. Чем больше `Z` значение, тем самым представление приводит к приведению более крупной тени на фоне, что делает представление плавающим выше, чем верх фона. Начальное повышение уровня представления можно задать, настроив его атрибут `elevation` в макете.

В следующем примере показаны тени, приведенные в пустом элементе управления `TextView`, если его атрибут повышения имеет значение 2DP, 4dp и 6DP соответственно:
...
Макеты Android 5,0 распознают новый атрибут `tint`, который можно использовать для установки цвета драваблес без необходимости создания нескольких версий этих ресурсов для вывода различных цветов. Чтобы использовать эту функцию, необходимо определить точечный рисунок как альфа-маску и использовать атрибут `tint` для определения цвета ресурса. Это позволяет создавать активы и разкрасить их в макете в соответствии со своей темой.

В следующем примере один ресурс изображения &ndash; белый логотип с прозрачным фоновым &ndash; используется для создания вариантов оттенка:

![Белая эмблема Xamarin с прозрачным фоном](lollipop-images/xamarin-logo-white.png)

Эта эмблема отображается над синей круглой фоном, как показано в следующих примерах. Изображение слева выглядит как эмблема без параметра `tint`. В центральном изображении для атрибута `tint` логотипа задается темно-серый цвет. На изображении справа `tint` задается светло-серый цвет:
...
Новый класс `Palette` Android 5,0 позволяет извлекать цвета из изображения, чтобы их можно было динамически применять к пользовательской цветовой палитре. Класс `Palette` извлекает шесть цветов из изображения и помечает эти цвета в соответствии с их относительными уровнями насыщенности и яркости цвета:
...
[![снимков зеленого, розового и синего цветов темы](lollipop-images/prominent-color-extraction-sml.png)](lollipop-images/prominent-color-extraction.png#lightbox)
...
Снимок экрана слева является примером `RecyclerView`, используемого в приложении электронной почты, а снимок экрана справа — пример `CardView`, используемого в приложении для резервирования командировок.

### <a name="recyclerview"></a>RecyclerView

`RecyclerView` похож на `ListView,` но лучше подходит для больших наборов представлений или списков с динамически изменяемыми элементами. Как и `ListView,` вы указываете адаптер для доступа к базовому набору данных. Однако в отличие от `ListView,` *Диспетчер макетов* используется для размещения элементов в `RecyclerView`. Диспетчер макетов также отвечает за перезапуск представления. Он управляет повторной попыткой использования представлений элементов, которые больше не видны пользователю.
...
[![снимков экрана приложения RecyclerView с прокруткой по изображениям](lollipop-images/recyclerview-scroll-sml.png)](lollipop-images/recyclerview-scroll.png#lightbox)

`RecyclerView` обрабатывает этот большой набор данных с простотой &ndash; прокруткой от начала списка до конца списка в этом примере приложения занимает всего несколько секунд. `RecyclerView` также поддерживает анимацию; по сути, анимации для добавления и удаления элементов включены по умолчанию. Когда элемент добавляется в `RecyclerView`, он исчезает, как показано в этой последовательности снимков экрана:

[снимок экрана ![кадров по кадрам для затухания элемента фотографии в](lollipop-images/recyclerview-animation-sml.png)](lollipop-images/recyclerview-animation.png#lightbox)
...
`CardView` — это простое представление, моделирующее плавающую карту со скругленными углами. Так как `CardView` имеет встроенные тени представления, она предоставляет простой способ добавления визуальной глубины в приложение. На следующих снимках экрана показаны три примера, ориентированные на текст `CardView`:

[![пример снимков приложений с помощью RecyclerView с элементами на основе Кардвиев](lollipop-images/recyclerview-cardview-sml.png)](lollipop-images/recyclerview-cardview.png#lightbox)
...
[Снимок экрана ![уведомления на экране блокировки](lollipop-images/lockscreen-notification-sml.png)](lollipop-images/lockscreen-notification.png#lightbox)
...
Android 5,0 предоставляет несколько новых интерфейсов API для улучшенных возможностей камеры. Новое пространство имен `Android.Hardware.Camera2` включает функции для доступа к отдельным устройствам камеры, подключенным к устройству Android. Кроме того, `Android.Hardware.Camera2` моделирует каждое устройство камеры как конвейер: оно принимает запрос на захват, записывает образ, а затем выводит результат. Такой подход позволяет приложениям ставить в очередь несколько запросов на захват на устройство камеры.

Следующие API делают эти новые возможности возможными:

- `CameraManager.GetCameraIdList` &ndash; помогает программным способом получить доступ к камере. для подключения к определенному устройству камеры используется `CameraManager.OpenCamera`.

- `CameraCaptureSession` &ndash; записи или потоковые образы с устройства камеры. Для управления новыми событиями записи изображения реализуется интерфейс `CameraCaptureSession.CaptureListener`.
...
Android 5,0 обновляет класс `AudioTrack` для улучшения воспроизведения звука:

- `ENCODING_PCM_FLOAT` &ndash; настраивает `AudioTrack` для приема звуковых данных в формате с плавающей запятой для улучшения динамического диапазона, повышения уровня запаса и более высокого качества (благодаря увеличению точности). Кроме того, формат с плавающей запятой помогает избежать обрезки звука.

- `ByteBuffer` &ndash; теперь можно передавать аудиоданные в `AudioTrack` в виде массива байтов.

- `WRITE_NON_BLOCKING` &ndash; этот параметр упрощает буферизацию и многопоточность для некоторых приложений.

Дополнительные сведения об улучшениях `AudioTrack` в Android 5,0 см. на [носителе](https://developer.android.com/about/versions/android-5.0.html#Media).

### <a name="media-playback-control"></a>Управление воспроизведением мультимедиа

В Android 5,0 появился новый класс `Android.Media.MediaController`, который заменяет `RemoteControlClient`. `Android.Media.MediaController` предоставляет упрощенные API-интерфейсы управления транспортом и обеспечивает поточно-ориентированное управление воспроизведением вне контекста пользовательского интерфейса. Следующие новые интерфейсы API обработают управление транспортом:

- `Android.Media.Session.MediaSession` &ndash; сеансу управления мультимедиа, который обрабатывает несколько контроллеров. Вы вызываете `MediaSession.GetSessionToken`, чтобы запросить маркер, используемый приложением для взаимодействия с сеансом.

- `MediaController.TransportControls` &ndash; обрабатывает транспортные команды, такие как **Play**, **останавливаться**и **Skip**.

Кроме того, можно использовать новый класс `Android.App.Notification.MediaStyle`, чтобы связать сеанс мультимедиа с богатым содержимым уведомлений (например, извлечение и отображение обложки альбома).
...
### <a name="storage"></a>Система хранения данных

Android 5,0 обновляет структуру доступа к хранилищу, чтобы упростить работу приложений с каталогами и документами.

- Чтобы выбрать поддерево каталога, можно создать и отправить `Android.Intent.Action.OPEN_DOCUMENT_TREE` намерение. Это заставляет систему отображать все экземпляры поставщиков, поддерживающие выбор поддеревьев. Затем пользователь просматривает и выбирает каталог.

- Для создания новых документов и каталогов и управления ими в любом месте поддерева используются новые методы `CreateDocument`, `RenameDocument`и `DeleteDocument` `DocumentsContract`.

- Чтобы получить пути к каталогам мультимедиа на всех общих устройствах хранения, вызовите новый метод `Android.Content.Context.GetExternalMediaDirs`.
...
В Android 5,0 появился новый API `JobScheduler`, который помогает пользователям снизить нагрузку на батарею, планируя выполнение определенных задач только в том случае, если устройство подключено к сети и заряжается. Эта функция планировщика заданий также может использоваться для планирования выполнения задачи, когда условия более подходят для этой задачи, например для загрузки большого файла, когда устройство подключено через сеть Wi-Fi, а не для сети с лимитным тарифным планом.
