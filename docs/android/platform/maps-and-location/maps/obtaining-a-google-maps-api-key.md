...
**В.\\Program Files (x86)\\Java\\JDK [VERSION]\\bin\\keytool. exe**
...
**/Систем/либрари/Жава/жававиртуалмачинес/[версия]. JDK/Contents/Home/bin/keytool**
...
7. Обратите внимание, что для APK доступа к Google Maps необходимо включить отпечатки SHA-1 и имена пакетов для каждого хранилища ключей (Отладка и выпуск), которое используется для подписания APK. Например, если вы используете один компьютер для отладки и другой компьютер для создания APK выпуска, следует включить отпечаток сертификата SHA-1 из хранилища ключей отладки первого компьютера и отпечаток сертификата SHA-1 из хранилища ключей выпуска второй компьютер. Щелкните **+ Добавить имя пакета и отпечаток** , чтобы добавить еще один отпечаток и имя пакета, как показано в следующем примере:
