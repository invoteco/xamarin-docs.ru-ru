...
[![Hero изображений планшетов и телефонов Android с Android Nougat](nougat-images/android-n-hero-sml.png)](nougat-images/android-n-hero.png#lightbox)
...
Список известных проблем Xamarin. Android 7,0 см. в заметках [о](https://github.com/xamarin/release-notes-archive/blob/master/release-notes/android/xamarin.android_7/xamarin.android_7.0/index.md)выпуске.
...
-   **Усовершенствования уведомлений** Переработанная система уведомлений в Android Nougat включает функцию *прямого ответа* , позволяющую пользователям быстро реагировать на текстовые сообщения непосредственно из пользовательского интерфейса уведомления. &ndash; Кроме того, если приложение создает уведомления для полученных сообщений, Новая функция *Объединенных уведомлений* может объединять уведомления в одну группу при получении нескольких сообщений.

-   **Заставка данных** &ndash; Эта функция является новой системной службой, которая помогает сократить объем данных сотовой сети, используемую приложениями. она дает пользователям возможность управлять использованием сотовых данных приложениями.
...
-   **Пакет Java Developer Kit** Для разработки Xamarin Android 7,0 требуется [JDK 8](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html) или более поздняя версия, если вы разрабатываете для API уровня 24 или выше (JDK 8 также поддерживает уровни API, предшествующие 24). &ndash; 64-разрядная версия JDK 8 необходима, если используются пользовательские элементы управления или средства просмотра форм.
...
2.  Установите **Android 7,0 (API 24)** или более поздней версии.
    ...
    [![Выбор пакетов Android 7,0 в диспетчере пакет SDK для Android](nougat-images/preview-packages.png)](nougat-images/preview-packages.png#lightbox)

3.  Установите новейшие средства пакет SDK для Android:

    [![Выбор последних пакет SDK для Android инструментов в диспетчере пакет SDK для Android](nougat-images/preview-tools.png)](nougat-images/preview-tools.png#lightbox)
...
-   Устройство: Хранилища 5X, хранилища 6, хранилища 6P, проигрывателя хранилища, хранилища 9 или пиксель C.
-   Целевой объект: Android 7,0 — уровень API 24
-   ABI: x86 или x86\_64

Например, это виртуальное устройство настроено для эмуляции хранилища 6:

[![Настройка AVD с помощью устройства хранилища, целевого объекта Android 7,0 и Intel Atom x86 CPU/ABI](nougat-images/android-n-avd.png)](nougat-images/android-n-avd.png#lightbox)
...
[![Примеры приложений, выполняемых в многооконном режиме в книжной и альбомной ориентации](nougat-images/multi-window-mode.png)](nougat-images/multi-window-mode.png#lightbox)

Вы можете настроить, как действия приложения Xamarin. Android поддерживают многооконный режим. Например, можно настроить атрибуты, устанавливающие минимальный размер и высоту и ширину по умолчанию приложения в многооконном режиме. Вы можете использовать новое `Activity.IsInMultiWindowMode` свойство, чтобы определить, находится ли ваше действие в многооконном режиме. Например:
...
Пример приложения [мултивиндовплайграунд](https://developer.xamarin.com/samples/monodroid/android-n/MultiWindowPlayground/) включает в C# себя код, демонстрирующий использование преимуществ нескольких пользовательских интерфейсов окон в приложении.
...
Например, следующий код создает объект `RemoteInput` для получения текстового ввода, создает цель ожидания для действия ответа и создает действие Remote input Enabled.
...
Пример приложения [службы обмена](https://developer.xamarin.com/samples/monodroid/android-n/MessagingService/) сообщениями C# включает в себя код, демонстрирующий способ расширения `RemoteInput` уведомлений с помощью объекта. Дополнительные сведения о добавлении встроенных действий по ответу в приложение для Android 7,0 или более поздней версии см. в разделе Android, [отвечающий на уведомления](https://developer.android.com/guide/topics/ui/notifiers/notifications.html#direct) .
...
[![Пример снимка экрана с объединенными уведомлениями](nougat-images/bundled-notifications-sml.png)](nougat-images/bundled-notifications.png#lightbox)
...
Чтобы активировать меню ярлыков, пользователь долго нажимает значок приложения для второго или более &ndash; меню, что позволяет быстро вибрации.
...
-   [Мултивиндовплайграунд](https://developer.xamarin.com/samples/monodroid/android-n/MultiWindowPlayground/) демонстрирует использование многооконного API, доступного в Android Nougat. Можно переключить пример приложения в многооконный режим, чтобы увидеть, как он влияет на жизненный цикл и поведение приложения.

-   [Служба обмена сообщениями](https://developer.xamarin.com/samples/monodroid/android-n/MessagingService/) — это простая служба, которая отправляет `NotificationCompatManager`уведомления с помощью. Он также расширяет уведомление с помощью `RemoteInput` объекта, позволяя устройствам Android Nougat отвечать через текст непосредственно из уведомления, не открывая приложение.

-   [Активные уведомления](https://developer.xamarin.com/samples/monodroid/android-n/ActiveNotifications/) показывают, как использовать `NotificationManager` API, чтобы узнать, сколько уведомлений в настоящее время отображается в приложении.

-   [Доступ к каталогу](https://developer.xamarin.com/samples/monodroid/android-n/ScopedDirectoryAccess/) с заданной областью Демонстрирует использование API доступа к каталогам с заданной областью для простого доступа к конкретным каталогам. Это является альтернативой определению `READ_EXTERNAL_STORAGE` или `WRITE_EXTERNAL_STORAGE` разрешениям в манифесте.

-   [Прямая загрузка](https://developer.xamarin.com/samples/monodroid/android-n/DirectBoot/) Показывает, как хранить данные в хранилище, зашифрованном с помощью устройства, которое всегда доступно, пока устройство загружается как до, так и после ввода учетных данных пользователя (ПИН-код/шаблон/пароль).
