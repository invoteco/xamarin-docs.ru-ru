...
- [Служба обмена сообщениями](https://docs.microsoft.com/samples/xamarin/monodroid-samples/android-n-messagingservice) — это простая служба, которая отправляет `NotificationCompatManager`уведомления с помощью. Он также расширяет уведомление с помощью `RemoteInput` объекта, позволяя устройствам Android Nougat отвечать через текст непосредственно из уведомления, не открывая приложение.

- [Активные уведомления](https://docs.microsoft.com/samples/xamarin/monodroid-samples/android-n-activenotifications) показывают, как использовать `NotificationManager` API, чтобы узнать, сколько уведомлений в настоящее время отображается в приложении.
