...
Существует известная ошибка с apksigner и JDK 9, в которой файл `apksigner.bat` вызывает `apksigner.jar` с `-Djava.ext.dirs`, а не `-classpath`, что в JDK 9. Рекомендуется использовать JDK 8 (1,8). Сведения об установке JDK 8 см [. в разделе разделы справки обновление версии пакета Java Development Kit (JDK).](~/android/troubleshooting/questions/update-jdk.md)
