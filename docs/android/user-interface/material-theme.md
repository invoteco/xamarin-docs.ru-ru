...
Кроме того, можно задать атрибут `Theme` приложения в **AssemblyInfo.CS** (или **Properties.CS**). Например:
...
- В макете установите атрибут `style` для этого представления в соответствии с именем пользовательского стиля, выбранным на предыдущем шаге. Например:
