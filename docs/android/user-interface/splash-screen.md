...
Этот `layer-list` центрирует изображение-заставку на фоне цвета фона, заданного ресурсом `@color/splash_background`. Пример приложения определяет этот цвет в файле **ресурсов/значений/Colors. XML** :
