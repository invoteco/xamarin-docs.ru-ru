...
@No__t_0 является одноэлементным, гарантируя, что весь доступ выполняется к тому же экземпляру. Блокировка используется для предотвращения параллельного доступа из нескольких потоков.
 ...
 **Тасклистадаптер** — для вывода списка \<T > объектов, которые необходимо реализовать адаптер для вывода пользовательских объектов в `ListView`. Адаптер определяет, какой макет используется для каждого элемента в списке. в этом случае в коде используется встроенная `SimpleListItemChecked` макета Android.
...
Класс MainPage использует `TaskListViewModel` для вывода данных с помощью функций привязки данных XAML. @No__t_0 страницы устанавливается в модель представления, которая заполняется асинхронно. Синтаксис `{Binding}` в XAML определяет, как отображаются данные.
