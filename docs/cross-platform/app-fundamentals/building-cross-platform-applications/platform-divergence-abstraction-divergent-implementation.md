...
Проекты Xamarin всегда определяют `__MOBILE__` , что это справедливо для проектов приложений iOS и Android (Обратите внимание на символы двойной подчеркивания до и после исправления этих символов).
...
Xamarin. iOS определяет `__IOS__` , какие из них можно использовать для обнаружения устройств iOS.
...
В настоящее время нет встроенного символа для Xamarin. Mac, но вы можете добавить его в параметры проекта приложения Mac **> сборки > компилятора** в поле " **Определение символов** " или изменить **CSPROJ** -файл и добавить его (например `__MAC__`,).
...
Используйте ключевое слово `WINDOWS_UWP`. Нет подчеркивания вокруг строки, подобной символам платформы Xamarin.
...
В следующем коде используется условная компиляция, чтобы `DatabaseFilePath` гарантировать правильность для каждой платформы:
