...
[![](localization-images/language-compare-sml.png "Немецкая и японская длина строки")](localization-images/language-compare.png#lightbox)
...
Избегайте использования строк `"You have {0} message/s."` , таких как использование конкретных строк для каждого состояния, чтобы обеспечить лучшее взаимодействие с пользователем:
...
Существует также более очевидный случай, даже на английском языке, где строки ссылаются на конкретного пользователя или пользователя приложения. Например, некоторые сайты отображают такие сообщения, `"Bob commented on his post"` как, например, для пол, женщина, недвоичного или неизвестного пола:
...
Одна из наиболее известных служб — [лионбридже](http://www.lionbridge.com/). Большинство профессиональных служб поддерживают все распространенные типы файлов, включая строки, XML, RESX и POT/PO.
