...
> Хотя по-прежнему можно открыть, изменить и скомпилировать PCL, для новых проектов рекомендуется использовать [библиотеки .NET Standard](~/cross-platform/app-fundamentals/net-standard.md) для доступа к большую контактную зону API.
