...
description: Общие проекты позволяют писать общий код, на который ссылается несколько различных проектов приложений. Код компилируется как часть каждого ссылающегося проекта и может включать директивы компилятора, которые помогают внедрить функции, зависящие от платформы, в общую базу кода.
ms.prod: xamarin
...
![](shared-projects-images/xs-sharedprojectoptions.png "Параметры общего проекта")
...
![](shared-projects-images/vs-empty.png "Пустой общий проект")
...
![](shared-projects-images/vs-sharedprojectproperties.png "Свойства общего проекта")
...
В примере [задачи](https://github.com/xamarin/mobile-samples/tree/master/Tasky) используется общий проект для хранения общего кода, используемого как приложениями iOS, Android, так и Windows Phone. Оба файла исходного `TaskRepository.cs` кода иимеютдирективыкомпилятора(например,`SQLite.cs` `#if __ANDROID__`) для получения различных выходных данных для каждого из приложений, ссылающихся на них.
...
![](shared-projects-images/example.png "Примеры для iOS, Android, Windows Phone")
