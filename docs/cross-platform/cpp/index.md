...
Библиотеки в пошаговом руководстве были созданы с помощью Visual Studio Code с сопутствующим сценарием оболочки. Расширенная версия этого пошагового руководства находится в [репозитории GitHub для Mobile Cat](https://github.com/xamarin/mobcat/blob/dev/samples/cpp_with_xamarin/) , в котором подробно рассматривается эта часть примера. В этом случае машинные библиотеки обрабатываются как зависимость от сторонних производителей, однако этот этап проиллюстрирован для контекста.
...
### <a name="stage-2-wrapping-the-native-libraries-with-a-visual-studio-solution"></a>Этап 2. Создание оболочек для собственных библиотек с помощью решения Visual Studio
...
Репозиторий исходного кода содержит [список дополнительных](https://github.com/xamarin/mobcat/tree/master/samples/cpp_with_xamarin#wrapping-up) материалов, в которых содержатся статьи о настройке частного канала NuGet в Azure DevOps и о том, как отправить пакет в этот веб-канал. Хотя при настройке локального каталога потребуется немного больше времени, этот тип веб-канала лучше использовать в среде коллективной разработки.
...
Функциональные возможности собственной библиотеки основаны на примере из [пошагового руководства. Создание и использование статической библиотеки (C++).](https://docs.microsoft.com/cpp/windows/walkthrough-creating-and-using-a-static-library-cpp?view=vs-2017)

В этом пошаговом руководстве пропускается первый этап — создание собственных библиотек, так как в этом сценарии библиотека предоставляется как зависимость от стороннего разработчика. Предкомпилированные машинные библиотеки включены вместе с [образцом кода](https://github.com/xamarin/mobcat/tree/master/samples/cpp_with_xamarin) или могут быть [загружены](https://github.com/xamarin/mobcat/tree/master/samples/cpp_with_xamarin/Sample/Artefacts) напрямую.
...
Дополнительный класс определяет функции-оболочки, позволяющие потребителю .NET создавать, удалять и взаимодействовать с базовым собственным `MyMathFuncs` классом.
...
Для этого этапа требуются [предварительно скомпилированные библиотеки](https://github.com/xamarin/mobcat/tree/master/samples/cpp_with_xamarin/Sample/Artefacts) , описанные в [предыдущем разделе](##creating-the-native-libraries-stage-1).
...
3. Обновите следующие поля и нажмите кнопку **создать**.
    ...
    - **Места** Использовать расположение для сохранения по умолчанию (или выбрать альтернативу)   
...
10. Обновите следующие поля и нажмите кнопку **создать**.
    ...
    - **Места** Использовать то же расположение для сохранения, что и для общего проекта   
...
4. Добавьте соответствующие библиотеки **. Таким образом** в каждую из папок **ABI** на основе следующего сопоставления:
    ...
    **x86_64:** библиотеки, Android и x86_64
...
5. Откройте **ApiDefinition.CS**, удалив шаблонный код с комментарием (не выкрывая `MathFuncs` только пространство имен), а затем выполните то же действие для **structs.CS** . 
...
4. Обновите **масфункс. nuspec** , указав базовые метаданные пакета, которые будут отображаться для потребителя **NuGet** . Например:
    ...
    > [!NOTE]
    > Дополнительные сведения о схеме, используемой для этого манифеста, см. в справочном документе по [nuspec](https://docs.microsoft.com/nuget/reference/nuspec) .

5. Добавьте элемент как дочерний `<package>` элемент элемента (как показано ниже `<metadata>`), определяя каждый файл с помощью отдельного `<file>` элемента: `<files>`
...
6. Добавьте элементы для сборок **Android:** `<file>`
...
7. Добавьте элементы для сборок **iOS:** `<file>`
...
8. Добавьте элементы для сборок **netstandard 2.0:** `<file>`
...
3. В окне **терминала**выполните команду **NuGet Pack** , указав файл **nuspec** , **версию** (например, 1.0.0) и **OutputDirectory** , используя папку, созданную на [предыдущем шаге](https://docs.microsoft.com/xamarin/cross-platform/cpp/index#creating-a-local-nuget-feed), то есть  **локальный — NuGet — веб-канал**. Например:
...
5. Обновите следующие поля и нажмите кнопку **создать**.
    ...
    - **Места** Используйте расположение для сохранения по умолчанию (или выберите альтернативный вариант).
...
7. Выберите предпочитаемое **устройство** или/**эмулятор**симулятора. 
8. Запустите решение (**команда + Return**), чтобы убедиться в том, что шаблон проекта **Xamarin. Forms** строится и работает нормально. 

    > [!NOTE]
    > **iOS** (в частности, симулятор) обычно имеет самое быстрое время сборки или развертывания.
...
3. Обновите следующие поля и нажмите кнопку **Добавить источник**:
    ...
    - **Места** Укажите папку **локального канала NuGet** , созданную на [предыдущем шаге](#preparing-a-local-packages-directory).
...
3. Объявите экземпляр `MyMathFuncs` класса в верхней `MainPage` части класса:

    ```csharp
    MyMathFuncs myMathFuncs;
    ```

4. Переопределите методы `OnDisappearing` `ContentPage` и из базового класса: `OnAppearing`
...
5. Обновите `myMathFuncs` метод, чтобы инициализировать переменную, объявленную ранее: `OnAppearing`
...
6. Обновите `Dispose` метод, чтобы вызвать метод для `myMathFuncs`: `OnDisappearing`
...
8. Наконец, вызовите `TestMathFuncs` `OnAppearing` метод в конце метода:
