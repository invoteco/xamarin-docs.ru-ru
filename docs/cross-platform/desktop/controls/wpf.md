...
WPF поддерживает концепцию *шаблонов элементов управления* , предоставляющих инструкции по визуализации для элемента управления`Button`( `ListBox`, и т. д.). Как упоминалось выше, Xamarin. Forms использует конкретные классы отрисовки, которые взаимодействуют с собственной платформой (iOS, Android и т. д.) для визуализации элемента управления.

Однако Xamarin. Forms имеет `ControlTemplate` тип — он `Page` используется для объектов. Он предоставляет определение для `Page` , которое обеспечивает единообразное содержимое, но позволяет пользователю страницы изменять цвета, шрифты и т. д. и даже добавлять элементы, чтобы сделать его уникальным для приложения.
...
На телефонных устройствах часто используется несколько экранов, поэтому в Xamarin. Forms предусмотрена поддержка нескольких форм навигации:
