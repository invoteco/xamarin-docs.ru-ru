...
>
> Чтобы приложения продолжали работать с этими серверами и службами, необходимо **обновить проекты Xamarin с помощью параметра `NSUrlSession`, показанного ниже, а затем повторно создать и повторно развернуть приложения** для пользователей.

## <a name="selecting-an-httpclient-stack"></a>Выбор стека HttpClient

Чтобы настроить `HttpClient`, используемые приложением, выполните следующие действия.
...
3. В раскрывающемся списке **Реализация HttpClient** выберите тип `HttpClient` один из следующих: **NSUrlSession** (рекомендуется), **CFNetwork**или **Managed**.

[![выбрать реализацию HttpClient из управляемых, CFNetwork или NSUrlSession](http-stack-images/http-xs-sml.png)](http-stack-images/http-xs.png#lightbox)

> [!TIP]
> Для TLS 1,2 поддержка параметра `NSUrlSession` рекомендуется.

### <a name="nsurlsession"></a>NSUrlSession

Обработчик на основе `NSURLSession`основан на собственной инфраструктуре `NSURLSession`, доступной в iOS 7 и более поздних версиях. 
...
Обработчик на основе `CFNetwork`основан на собственной инфраструктуре `CFNetwork`, доступной в iOS 6 и более поздних версиях.
...
Помимо конфигурации на уровне проекта, показанной выше, можно также создать экземпляр `HttpClient` и внедрить необходимые `HttpMessageHandler` с помощью конструктора, как показано в следующих фрагментах кода:
...
Это позволяет использовать другие `HttpMessageHandler`, которые объявляются в диалоговом окне " **Параметры проекта** ".

## <a name="ssltls-implementation"></a>Реализация SSL/TLS

SSL (уровень защищенного сокета) и его последователем, TLS (безопасность транспортного уровня) обеспечивают поддержку HTTP и других сетевых подключений с помощью `System.Net.Security.SslStream`. Реализация `System.Net.Security.SslStream` Xamarin. iOS, Xamarin. tvOS или Xamarin. Mac будет вызывать собственную реализацию SSL/TLS для Apple вместо использования управляемой реализации, предоставляемой Mono. Собственная реализация Apple поддерживает TLS 1,2.
...
Поскольку ATS включена по умолчанию в приложениях, созданных для iOS 9, tvOS 9 и OS X 10,11 (El Capitan) и более поздних версий, все подключения, использующие `NSUrlConnection`, `CFUrl` или `NSUrlSession`, будут подвержены требованиям безопасности ATS. Если подключения не соответствуют этим требованиям, они завершатся сбоем с исключением.
...
Эта ошибка вызвана введением параметра `MtouchTlsProvider` в файл проекта в Xamarin. iOS 9,8. Если обновление до Xamarin. iOS 9,8 (или более поздней версии) невозможно, то обходным путем можно вручную изменить приложение с расширением **CSPROJ** , удалить элемент `MtouchTlsprovider`, а затем сохранить измененный файл проекта.

В следующем фрагменте кода приведен пример того, как может выглядеть `MtouchTlsProvider` в файле **CSPROJ** :
