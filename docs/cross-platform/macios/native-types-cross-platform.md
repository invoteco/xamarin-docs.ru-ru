...
Способ обработки этих изменений зависит от размера и сложности приложения и форме, совместно использовать код используется, как мы увидим в следующих разделах.
...
Переносимые библиотеки классов (PCL) позволяет платформ, которые вы хотите поддерживать и использовать интерфейсы для предоставления функциональных возможностей платформы.

Так как тип проекта переносимой библиотеки Классов компилируется вниз до `.DLL` и в нем нет смысла на единый API, вы будете вынуждены продолжать использовать существующие типы данных (`int`, `uint`, `float`) в PCL исходный код и тип приведения вызовы в переносимую библиотеку классов классы и методы в клиентские приложения. Пример:
...
Размер и сложность внешнего интерфейса приложений для мобильных устройств, которые используют общий код, а также размер и сложность кода, общего доступа, необходимо принимать во внимание при выборе метода поддержки собственных типов данных в разных платформах Проект общего ресурса.
...
В этом случае решение может быть для создания версии перегрузка методов, с помощью 32-разрядные типы данных, благодаря чему они теперь использовать `CGRect` как параметр или возвращаемое значение, преобразовать это значение к `RectangleF` (зная, что преобразование из `nfloat` для `float` с потерей данных преобразования) и вызвать исходную версию подпрограммы для выполняют фактическую работу. Пример:
...
Обратите внимание, что мы изменили `CalculateArea` метод для возврата `nfloat` вместо стандартного `float`. Это было сделано, то не получили бы ошибку компиляции, попытка _неявно_ преобразовать `nfloat` результат наших вычислений (так как оба значения умножаемое типа `nfloat`) в `float` возвращаемое значение.
...
Как и в стандартного кросс платформенные приложения следует использовать существующие типы 32-разрядные данные в любом коде общего, кросс платформенных для большинства ситуаций. Новые собственные типы данных использовать только в том случае, когда требуется сделать вызов к API iOS или Mac где поддержка архитектуры с поддержкой типов.
...
В этой статье мы видели когда следует использовать собственные типы данных в приложении единый API и их последствия между различными платформами. Мы представили несколько решений, которые можно использовать в ситуациях, когда необходимо использовать новые собственные типы данных в кросс платформенные библиотеки. Кроме того мы увидели, краткое руководство по поддержке Unified API-интерфейсов в Xamarin.Forms кросс платформенных приложений.
