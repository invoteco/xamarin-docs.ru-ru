...
### <a name="taskbool-replaced-with-taskbooleannserror"></a>Задача<bool> заменила < задачи Boolean, > нсеррор >
...
Так как некоторые ошибки очень распространены и возвращаемое значение не было полезным, этот шаблон был  изменен в унифицированном `Task<Tuple<Boolean,NSError>>`виде для возврата. Это позволяет проверить как успешность, так и любые ошибки, которые могли произойти во время асинхронного вызова.
