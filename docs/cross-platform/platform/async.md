...
Метод, помеченный `async`, должен содержать по крайней мере одно выражение или оператор await. Если в методе отсутствуют `await`ы, то они будут выполняться синхронно (то же, что и при отсутствии модификатора `async`). Это также приведет к появлению предупреждения компилятора (но не ошибки).
