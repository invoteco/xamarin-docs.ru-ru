...
Нет, пакет NuGet 3,0 "System. Diagnostics. Tracing" включает только реализации, зависящие от платформы, для "DNXCore50" и "netcore50". В нем явно опущены реализации Xamarin. Android ("iOS") и Xamarin. iOS ("коtouch" и "ксамариниос"). Это означает, что установка пакета не будет _оказывать влияния_ на проекты Xamarin. Android и Xamarin. iOS. Пакет NuGet предполагает, что обе платформы предоставляют _собственную_ реализацию типов. Это предположение «правильный», в том смысле, что у _моно_ реализации пространства имен, а также как описано в разделе точек \#2 и \#3 из «Сведения о трех уровней ошибок» выше, Реализация в настоящее время неполон. Чтобы команда Mono могла устранить [ошибку 27337](https://bugzilla.xamarin.com/show_bug.cgi?id=27337) и [ошибку 34890](https://bugzilla.xamarin.com/show_bug.cgi?id=34890), нужно исправить проблему.
