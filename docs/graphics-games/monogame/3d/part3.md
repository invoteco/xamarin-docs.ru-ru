...
В этом пошаговом руководстве рассматривается перемещение объектов в трехмерном пространстве. Начнем с проекта для подготовки к просмотру моделей и массивов вершин [, которые можно найти здесь](https://developer.xamarin.com/samples/mobile/ModelsAndVertsMG/). После скачивания распакуйте и откройте проект, чтобы убедиться, что он выполняется, и мы должны увидеть следующее:
