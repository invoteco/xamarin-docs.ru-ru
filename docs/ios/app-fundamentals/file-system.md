...
Несмотря на то, что работа с полным пространством имен `System.Xml` выходит за рамки этой статьи, можно легко десериализовать XML-документ из файловой системы, используя StreamReader, например следующий фрагмент кода:
...
Дополнительные сведения см. в документации по [System. XML](xref:System.Xml) и [сериализации](xref:System.Xml.Serialization). См. [документацию по Xamarin. iOS](~/ios/deploy-test/linker.md) в компоновщике — часто требуется добавить атрибут `[Preserve]` к классам, которые планируется сериализовать.

### <a name="creating-files-and-directories"></a>Создание файлов и каталогов

В этом примере показано, как использовать класс `Environment` для доступа к папке «документы», в которой можно создавать файлы и каталоги.
...
Наконец, создайте экземпляр класса `Account`, выполните его сериализацию в данные JSON и запишите в файл:
...
Чтобы пометить файл для включения, щелкните правой кнопкой мыши файл (ы) и выберите **действие сборки &gt; содержимое** в Visual Studio для Mac. Также можно изменить **действие сборки** на странице **свойств** файла.
...
Из-за такого затруднения рекомендуется использовать метод `System.IO.Path.Combine`, который корректирует текущую платформу, а не жестко определенный разделитель пути. Это простой шаг, который делает ваш код более переносимым на другие платформы.
...
[![содержимое пакета приложений](file-system-images/45-bundle-sml.png)](file-system-images/45-bundle.png#lightbox)
...
|Библиотека/настройки/|Файлы предпочтений, относящиеся к приложению, хранятся в этом каталоге. Не создавайте эти файлы напрямую. Вместо этого используйте класс `NSUserDefaults`.<br /><br />Содержимое этого каталога архивируется в iTunes.|
...
В предыдущих примерах каталогов и файлов был доступ к каталогу `Documents`. Для записи в другой каталог необходимо создать путь с помощью синтаксиса "..", как показано ниже:
...
Пути к каталогам `Caches` и `tmp` могут быть построены следующим образом:
...
Пользователи могут получить доступ к файлам в каталоге документов приложения, изменив `Info.plist` и создав **приложение, поддерживающее** запись в формате iTunes (`UIFileSharingEnabled`) в представлении **исходного кода** , как показано ниже:
...
Доступ к этим файлам можно получить в iTunes, когда устройство подключено, и пользователь выбрал вкладку `Apps`. Например, на следующем снимке экрана показаны файлы в выбранном приложении, совместно используемые в iTunes:
...
- Храните любые другие данные, которые можно легко создать повторно или повторно скачать в `Library/Caches` или `tmp` (которые не архивируются и могут быть "очищены").
- Если у вас есть файлы, которые могут быть подходящими для `Library/Caches` или `tmp` папки, но вы не хотите очищать их, сохраните их в любом расположении (например, `Library/YourData`) и примените атрибут "не создавать резервную копию", чтобы файлы не могли использовать пропускную способность резервного копирования iCloud и Стора пространство GE. Эти данные по-прежнему используют пространство на устройстве, поэтому следует внимательно управлять им и удалять его по возможности.

Атрибут "не выполнять резервное копирование" задается с помощью класса `NSFileManager`. Убедитесь, что ваш класс `using Foundation` и вызовите `SetSkipBackupAttribute` следующим образом:
...
Если `SetSkipBackupAttribute` имеет значение `true` файл не будет архивирован, независимо от каталога, в котором он хранится (даже в каталоге `Documents`). Вы можете запросить атрибут с помощью метода `GetSkipBackupAttribute`, а также сбросить его, вызвав метод `SetSkipBackupAttribute` с `false`следующим образом:
...
> [!IMPORTANT]
> Если возвращенный путь к группе `null`, проверьте конфигурацию прав и профиль подготовки и убедитесь, что они верны.
