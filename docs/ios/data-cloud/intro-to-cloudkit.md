...
[![поддержки CloudKit на устройствах macOS и iOS](intro-to-cloudkit-images/image1.png)](intro-to-cloudkit-images/image1.png#lightbox)
...
В [примере клаудкитатлас](https://docs.microsoft.com/samples/xamarin/ios-samples/ios8-cloudkitatlas) показано, как использовать CloudKit с Xamarin. Ниже показано, как настроить пример. он требует дополнительных параметров, помимо тех, которые требуются только для CloudKit:
...
7. Убедитесь, что для приложения существует **контейнер для повсеместного** существования. Пример: `iCloud.com.your-company.CloudKitAtlas`
...
Контейнеры представлены в платформе CloudKit с помощью класса `CKContainer`. По умолчанию одно приложение обращается к одному контейнеру, и этот контейнер разделяет данные для этого приложения. Это означает, что несколько приложений могут хранить информацию в одной учетной записи iCloud, но эта информация никогда не будет запутанной.
...
Компания Apple предлагает использовать обратную нотацию DNS при создании пространства имен для контейнеров приложений. Пример: `iCloud.com.company-name.application-name`
...
Базы данных представлены в CloudKit Framework с помощью класса `CKDatabase`. Каждое приложение имеет доступ к двум базам данных: общедоступной базе данных и частной.
...
Записи предоставляются в CloudKit Framework с помощью класса `CKRecord`, который заключает пары "ключ-значение" в оболочку. Экземпляр объекта в приложении эквивалентен `CKRecord` в CloudKit. Кроме того, каждый `CKRecord` имеет тип записи, эквивалентный классу объекта.
...
Класс `CKRecord` также поддерживает широкий спектр метаданных. Например, запись содержит сведения о том, когда она была создана, и пользователя, создавшего ее. Запись также содержит сведения о времени последнего изменения и пользователе, который ее изменил.
...
Как упоминалось выше, `CKRecords` переносить пары "ключ-значение" и таким образом могут хранить в записи следующие типы данных:
...
Идентификаторы записей представлены в платформе CloudKit с помощью класса `CKRecordID`. Для создания нового идентификатора записи можно использовать следующий код:
...
Ссылки представлены в платформе CloudKit с помощью класса `CKReference`. Они позволяют серверу iCloud понять отношения между записями.
...
На клиенте создается `CKRecord`, описывающий файл, который будет отправлен на сервер iCloud. `CKAsset` создается для хранения файла и связывается с записью, описывающей его.
...
Активы предоставляются в CloudKit Framework через класс `CKAsset` и используются для хранения больших, неструктурированных данных. Поскольку разработчик никогда не хочет иметь большие, неструктурированные данные в памяти, ресурсы реализуются с помощью файлов на диске.
...
Поскольку `CKAssets` предназначены для работы с большими файлами данных, компания Apple разработана CloudKit для эффективного отправки и загрузки ресурсов.
...
Прежде чем приступить к работе с удобным интерфейсом API CloudKit, необходимо выполнить стандартный код установки. Сначала измените файл `AppDelegate.cs` приложения и сделайте его следующим:
...
При этом добавляется ярлык для получения `AppDelegate` и доступа к созданным ранее ярлыкам открытой и закрытой баз данных.
...
1. Вызывая метод `SaveRecord` `PublicDatabase`, разработчику не нужно указывать, каким способом отправляются данные, какая зона записывается и т. д. Удобный API отвечает за все эти сведения.
...
Метод `FetchRecord` `PublicDatabase` возвращает `CKRecord`, если вызов был успешным. Затем приложение изменяет запись и вызывает `SaveRecord` снова, чтобы записать изменения обратно в базу данных.
...
Как упоминалось выше, запросы позволяют разработчику выбрать небольшое подмножество большего набора данных, существующего в облаке. Запросы представлены в платформе CloudKit с помощью класса `CKQuery`.

Запрос объединяет три различных вещи: тип записи (`RecordType`), предикат (`NSPredicate`) и (необязательно) дескриптор сортировки (`NSSortDescriptors`). CloudKit поддерживает большинство `NSPredicate`.
...
6. CloudKit поддерживает составные предикаты, соединяемые с помощью оператора `AND`.
...
Приведенный выше код принимает запрос, созданный выше, и выполняет его в общедоступной базе данных. Так как зона записи не указана, поиск выполняется во всех зонах. Если ошибки не возникли, будет возвращен массив `CKRecords`, соответствующий параметрам запроса.
...
Подписки предоставляются в CloudKit Framework через класс `CKSubscription`. Они объединяют тип записи (`RecordType`), предикат (`NSPredicate`) и push-уведомление Apple (`Push`).
...
В `AppDelegate.cs`переопределите класс `ReceivedRemoteNotification` следующим образом:
...
Так как это поступает из iCloud, существует большое резервное хранилище сведений о пользователе. И, поскольку в iCloud фактически размещается контейнер, он может сопоставлять пользователей. На приведенном выше рисунке пользователь, чья учетная запись iCloud `user@icloud.com` связан с текущим клиентом.
...
1. В Visual Studio для MA скомпилируйте приложение для **выпуска** > **устройство iOS**:
...
11. Выберите расположение для создания полученного файла `.ipa` приложения.
