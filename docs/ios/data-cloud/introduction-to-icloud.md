...
- **Добавьте идентификатор контейнера в поле** "права". plist — формат идентификатора контейнера — `TeamID.BundleID`. Дополнительные сведения см. в руководстве по [работе с](~/ios/deploy-test/provisioning/entitlements.md) назначениями.
...
В примере Иклаудуидок реализуется простой подкласс `UIDocument`, содержащий одно текстовое поле. Текст отображается в `UITextView` и изменения распространяются в iCloud на другие устройства с сообщением с уведомлением, отображаемым красным цветом. Пример кода не имеет решения с более сложными функциями iCloud, такими как разрешение конфликтов.
...
1. **Отображение документов iCloud** . Предоставьте свойства из `UIDocument`, чтобы можно было взаимодействовать с элементами управления пользовательского интерфейса.
 ...
 **Нотификатионцентер** — регистрация для получения уведомлений при завершении асинхронных операций, таких как `NSMetadataQuery.StartQuery`.

 **Обработчики завершения** — передача методов для выполнения при завершении асинхронных операций, таких как `UIDocument.Open`.
...
Этот код находится в методе `AppDelegate.FinishedLaunching` образца.
...
Пример приложения работает только с одним файлом File-Test. txt, поэтому код в **AppDelegate.CS** создает `NSPredicate` и `NSMetadataQuery`, чтобы искать именно это имя файла. `NSMetadataQuery` выполняется асинхронно и отправляет уведомление по завершении. `DidFinishGathering` вызывается наблюдателем уведомлений, останавливает запрос и вызывает Лоаддокумент, который использует метод `UIDocument.Open` с обработчиком завершения, чтобы попытаться загрузить файл и отобразить его в `MonkeyDocumentViewController`.
...
В примере **иклаудуидок\монкэйдокументвиевконтроллер.КС** отображает текст монкэйдокумент в `UITextView`. `ViewDidLoad` прослушивает уведомление, отправленное в методе `MonkeyDocument.LoadFromContents`. `LoadFromContents` вызывается, когда в iCloud есть новые данные для файла, поэтому уведомление указывает на то, что документ был обновлен.
...
Последующие изменения в документе не сохраняются напрямую, вместо этого мы будем сообщать `UIDocument` о том, что он был изменен `UpdateChangeCount`, и автоматически запланировать операцию сохранения на диске:
...
Приложения, хранящие большие объемы данных, такие как, должны либо хранить их в одном из пользовательских каталогов, не имеющих резервных копий (например, Для кэширования или TMP) или используйте `NSFileManager.SetSkipBackupAttribute`, чтобы применить к этим файлам флаг, чтобы в iCloud они игнорировались во время операций резервного копирования.
