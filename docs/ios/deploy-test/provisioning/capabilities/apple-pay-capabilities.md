...
ms.locfileid: "78917733"
---
# <a name="apple-pay-capabilities-in-xamarinios"></a>Возможности Apple Pay в Xamarin.iOS

_Для добавления возможностей в приложение часто требуется дополнительная настройка подготовки. В этом руководством объясняется настройка, необходимая для Apple Payных возможностей._
...
* Убедитесь, что в  **Подписывании пакета iOS** приложения параметр  **Настраиваемые назначения** установлен в **Entitlements.plist**. Это _не_ параметр по умолчанию для сборок отладки и симуляторов iOS.
