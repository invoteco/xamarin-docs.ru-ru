...
2. В разделе **Удостоверение** введите имя приложения и **идентификатор пакета**, который был создан при определении идентификатора приложения:
    ...
    ![Ввод идентификатора пакета](entitlements-images/servicexs01-sml.png)
...
4. На **Панели решения** дважды щелкните файл **Entitlements.plist**, чтобы открыть его для редактирования:
    ...
    ![Редактирование назначений](entitlements-images/servicexs02-sml.png)
...
2. В разделе **Целевое приложение iOS** введите имя приложения и **Идентификатор пакета**, который был создан при определении ИД приложения:
    ...
    ![Установка идентификатора пакета](entitlements-images/servicevs01-sml.png)

3. Сохраните изменения в файле **Info.plist**.
4. В **обозревателе решений** дважды щелкните файл **Entitlements.plist**, чтобы открыть его:

    ![Редактирование назначений](entitlements-images/servicevs02-sml.png)

    Вы также можете щелкнуть правой кнопкой файл **Entitlements.plist** и выбрать в пункте **Открыть с помощью...** редактор источника XML, который позволяет задавать свойства назначений и значения ключей, как описано ниже в разделе [Справочник по ключам назначений](#entitlement-key-reference).
...
1. Щелкните правой кнопкой на файле проекта и выберите **Добавить > Новый файл…** :
    ...
    ![Контекстное меню добавления файлов](entitlements-images/image1-sml.png)
2. В диалоговом окне создания файла выберите **iOS > Список свойств** и назовите его "Entitlements":

    ![Диалоговое окно "Новый файл"](entitlements-images/image2-sml.png)
