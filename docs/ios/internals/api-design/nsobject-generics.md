...
Поскольку объекты, подклассировать `NSObject`, зарегистрированы в среде выполнения цели-C, существуют некоторые ограничения на то, что возможно с универсальными подклассами `NSObject` типов.
...
Все аргументы универсального типа в сигнатуре члена, доступного для цели-C, должны иметь ограничение `NSObject`.
...
**Причина**: параметр универсального типа является `NSObject`, поэтому сигнатура селектора для `myMethod:` может быть безопасно предоставлена цели-C (она всегда будет `NSObject` или являться подклассом).
...
**Причина**: параметр `T` в экспортированном `MyMethod` цели-C ограничивается `NSObject`, неограниченный тип `U` не является частью сигнатуры.
...
Рассмотрим это определение класса, которое предоставляет конструктор, принимающий `IntPtr` (способ создания C# объекта с помощью Xamarin. iOS, который создает объект из собственного экземпляра цели-C):
...
**Причина**: это недопустимо, поскольку Xamarin. iOS не знает, какой тип использовать для аргумента типа `T` при вызове метода из цели-C.
...
Нельзя предоставить статические члены цели, если она размещена в универсальном подклассе `NSObject`.
...
Для членов экземпляра используется сам экземпляр (поскольку экземпляр не будет `Generic<T>`, он всегда будет `Generic<SomeSpecificClass>`), но для статических членов эти сведения отсутствуют.

Обратите внимание, что это применимо, даже если рассматриваемый элемент не использует аргумент типа, `T` каким бы то ни было.
