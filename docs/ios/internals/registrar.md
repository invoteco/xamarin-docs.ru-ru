...
Например, рассмотрим управляемый метод `Main`, распространенный в приложениях Xamarin. iOS:

```csharp
UIApplication.Main (args, null, "AppDelegate");
```

Этот код сообщает среде выполнения цели-C, что в качестве класса делегата приложения используется тип с именем `AppDelegate`. Чтобы среда выполнения цели-C могла создать экземпляр класса C#`AppDelegate`, этот класс должен быть зарегистрирован.
...
Чтобы создать категорию, используйте атрибут `[Category]` и укажите тип для расширения. Например, следующий код расширяет `NSString`:

```csharp
[Category (typeof (NSString))]
```

Каждый из методов категории имеет атрибут `[Export]`, делая его доступным для среды выполнения цели-C:
...
В этом примере в класс `NSString` будет добавлен собственный метод `toUpper` экземпляра. Этот метод может быть вызван из цели-C:
...
Начиная с Xamarin. iOS 8,10 интерфейсы с атрибутом `[Protocol]` будут экспортированы в цель-C в качестве протоколов:
...
- Краткосрочная недостатком является также то, что Clang необходимо использовать, если используется платформа Accounts (это связано с тем, что заголовок **Accounts. h** может быть скомпилирован только с помощью Clang). Добавьте `--compiler:clang` в дополнительные аргументы mtouch, чтобы использовать Clang, если вы используете Xcode 4,6 или более раннюю версию (Xamarin. iOS автоматически выберет Clang в Xcode 5,0 или более поздней версии).

- Если используется Xcode 4,6 (или более ранняя версия), необходимо выбрать GCC/G + +, если имена экспортируемых типов содержат символы, отличные от ASCII (это обусловлено тем, что в версии Clang, поставляемой с Xcode 4,6, не поддерживаются символы, не входящие в набор ASCII, в идентификаторах в коде цели-C). Добавьте `--compiler:gcc` в дополнительные аргументы mtouch, чтобы использовать GCC.
...
Теперь протоколы должны иметь атрибут `[Protocol]`. Если этого не сделать, будет вычислена собственная ошибка компоновщика, например:
...
Функции Variadic должны использовать аргумент `IsVariadic` в атрибуте `[Export]`:
