...
Обычно они помещаются в  **{0}XIB.CS** -файл рядом  **{0}** с файлом конструктора XIB.Designer.cs.
...
Если свойства розетки определены как тип `id` ( `NSObject`эквивалентный), генератор кода конструктора в настоящее время определяет наиболее надежный тип, основанный на объектах, подключенных к этой розетке, для удобства.
