...
Откройте оба этих файла в проекте привязки, созданном ранее. Скопируйте содержимое файла **InfColorPicker.CS** (с **узла сборки Mac**) и вставьте его в файл **ApiDefinition.CS** , заменив существующий блок кода `namespace ...` на содержимое файла **InfColorPicker.CS** (без неизмененные `using`ные операторы).
