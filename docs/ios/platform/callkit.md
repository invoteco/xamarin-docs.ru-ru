...
- `CXProvider` — это позволяет приложению Монкэйкалл уведомлять систему о любых возможностях нестандартных уведомлений, которые могут возникнуть.
- `CXCallController` — позволяет приложению Монкэйкалл уведомлять систему о действиях локальных пользователей.
...
Когда приложение хочет взаимодействовать с системой, оно использует класс `CXCallUpdate` и когда система должна взаимодействовать с приложением, используется класс `CXAction`:
...
`CXCallController` позволяет приложению уведомлять систему о локальных действиях пользователей, таких как пользователь, запускающий вызов VOIP. Реализация `CXCallController` приложение взаимозависимость с другими типами вызовов в системе. Например, если уже выполняется телефонный вызов телефонии, `CXCallController` можете разрешить приложению VOIP разместить этот вызов на удержании и запуске или ответить на вызов VOIP.
...
Когда приложение хочет передать в систему действия локальных пользователей, оно использует класс `CXTransaction`:
...
Класс `ActiveCall` используется приложением Монкэйкалл для хранения всей информации о вызове VOIP, который в настоящее время активен следующим образом:
...
Статический класс `StartCallRequest` предоставляет несколько вспомогательных методов, которые будут использоваться при работе с исходящими вызовами:
...
Класс `ActiveCallManager` обрабатывает все открытые вызовы в приложении Монкэйкалл.
...
Опять же, поскольку это только моделирование, `ActiveCallManager` поддерживает только коллекцию объектов `ActiveCall` и имеет подпрограммы для поиска данного вызова по его свойству `UUID`. Он также включает методы для запуска, завершения и изменения состояния в режиме ожидания исходящего вызова. Дополнительные сведения см. в разделе [Обработка исходящих вызовов](#handling-outgoing-calls) ниже.

### <a name="the-providerdelegate-class"></a>Класс Провидерделегате

Как упоминалось выше, `CXProvider` обеспечивает двустороннюю связь между приложением и системой для получения уведомлений по каналу. Разработчику необходимо предоставить настраиваемый `CXProviderDelegate` и подключить его к `CXProvider`, чтобы приложение обрабатывало события Каллкит по внешнему каналу. Монкэйкалл использует следующие `CXProviderDelegate`:
...
При создании экземпляра этого делегата ему передается `ActiveCallManager`, который будет использоваться для обработки любого действия вызова. Затем он определяет типы обработчиков (`CXHandleType`), на которые будет отвечать `CXProvider`:
...
Эти значения поставляются в `CXProviderConfiguration`, которые будут использоваться для настройки `CXProvider`.
...
Если это реальное приложение, метод `DidActivateAudioSession` будет использован для запуска вызова с предварительно настроенным `AVAudioSession`, предоставленным системой:
...
Он также будет использовать метод `DidDeactivateAudioSession` для завершения и освобождения подключения к системному сеансу, предоставленному системой:
...
Монкэйкалл использует AppDelegate для хранения экземпляров `ActiveCallManager` и `CXProviderDelegate`, которые будут использоваться во всем приложении:
...
Методы переопределения `OpenUrl` и `ContinueUserActivity` используются, когда приложение обрабатывает исходящий вызов. Дополнительные сведения см. в разделе [Обработка исходящих вызовов](#handling-outgoing-calls) ниже.
...
2. Приложение использует `CXProvider` для отправки `CXCallUpdate` в систему, информирующее его о вызове.
...
Этот код создает новый экземпляр `CXCallUpdate` и прикрепляет к нему маркер, который будет обозначать вызывающий объект. Далее используется метод `ReportNewIncomingCall` класса `CXProvider` для информирования системы о вызове. Если это происходит успешно, вызов добавляется в коллекцию активных вызовов приложения, если это не так, то ошибка должна быть передана пользователю.
...
2. Система отправляет `CXAnswerCallAction` в `CXProvider` приложения, уведомляя его о намерении ответа.
...
Этот код сначала ищет данный вызов в списке активных вызовов. Если вызов не найден, система получает уведомления и метод завершает работу. Если он найден, вызывается метод `AnswerCall` класса `ActiveCall` для запуска вызова, а система — сведения, если она завершается успешно или неудачно.
...
1. Приложение создает `CXEndCallAction`, которые объединяются в `CXTransaction`, отправляемые в систему, чтобы сообщить ему о завершении вызова.
...
2. Приложение будет использовать `CXCallController` для запроса действия вызова Start из системы.
3. Если система принимает действие, оно будет возвращено приложению через делегат `XCProvider`.
...
Он создает `CXHandle` и использует его для настройки `CXStartCallAction`, Объединенного в `CXTransaction`, который отправляется в систему с помощью метода `RequestTransaction` класса `CXCallController`. Вызывая метод `RequestTransaction`, система может разместить все существующие вызовы на удержании независимо от источника (приложения Phone, FaceTime, VOIP и т. д.), прежде чем начать новый вызов.

Запрос на запуск исходящего вызова VOIP может поступать из нескольких различных источников, например Siri, записи на карточке контакта (в приложении "Контакты") или из списка последних сообщений (в приложении Phone). В таких ситуациях приложение будет отправлять вызов Start в `NSUserActivity`, и AppDelegate потребуется его обрабатывать:
...
Здесь метод `CallHandleFromActivity` вспомогательного `StartCallRequest` класса используется для получения маркера для вызываемого человека (см. [класс старткаллрекуест](#the-startcallrequest-class) выше).

Метод `PerformStartCallAction` [класса провидерделегате](#the-providerdelegate-class) используется, чтобы запустить фактический исходящий вызов и информировать систему о его жизненном цикле:
...
Если создает `CXEndCallAction` с UUID вызова End, объединяет его в `CXTransaction`, который отправляется в систему с помощью метода `RequestTransaction` класса `CXCallController`. 
...
Существует несколько методов делегата поставщика (`CXProviderDelegate`), которые должны быть переопределены для корректной обработки этих ситуаций времени ожидания.
...
4. Перед подключением вызов Каллкит предоставляет `AVAudioSession` с высоким приоритетом, соответствующий конфигурации, запрошенной приложением. Приложение будет уведомлено с помощью метода `DidActivateAudioSession` его `CXProviderDelegate`.
...
3. Выберите модули **iOS** > **расширения** > **вызвать расширения каталогов** и нажмите кнопку **Далее** : 
...
В результате в проект будет добавлен класс `CallDirectoryHandler.cs`, который выглядит следующим образом:
...
Для обеспечения требуемой функциональности необходимо изменить метод `BeginRequest` в обработчике каталога вызова. В примере выше он пытается задать список заблокированных и доступных номеров в базе данных Contacts приложения VOIP. Если по какой либо причине произошел сбой любого запроса, создайте `NSError` для описания сбоя и передайте ему метод `CancelRequest` класса `CXCallDirectoryExtensionContext`.

Для установки номеров блокировки используйте метод `AddBlockingEntry` класса `CXCallDirectoryExtensionContext`. Числа, предоставленные методу, _должны_ быть отсортированы по возрастанию. Для оптимальной производительности и использования памяти при наличии большого числа телефонных номеров рассмотрите возможность загрузки подмножества чисел в заданный момент времени и использования пулов автоосвобождения для освобождения объектов, выделенных во время каждого пакета загружаемых пакетов.

Чтобы уведомить Контактное приложение о контактных номерах, известных для приложения VOIP, используйте метод `AddIdentificationEntry` класса `CXCallDirectoryExtensionContext` и укажите как число, так и идентификационную метку. Опять же, числа, предоставленные методу, _должны_ быть отсортированы по возрастанию. Для оптимальной производительности и использования памяти при наличии большого числа телефонных номеров рассмотрите возможность загрузки подмножества чисел в заданный момент времени и использования пулов автоосвобождения для освобождения объектов, выделенных во время каждого пакета загружаемых пакетов.
