...
Некоторые контейнеры позволяют упорядочивать контакты в одну или несколько _групп_ или подгрупп. Это поведение зависит от резервного хранилища для данного контейнера. Например, iCloud имеет только один контейнер, но может иметь много групп (но не содержит подгрупп). Microsoft Exchange с другой стороны, не поддерживает группы, но может иметь несколько контейнеров (по одному для каждой папки Exchange).
