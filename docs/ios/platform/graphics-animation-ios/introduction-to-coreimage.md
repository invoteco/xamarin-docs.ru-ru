...
Массив Features будет заполнен `CIFaceFeature`ными объектами (если обнаружены какие-либо лица). Для каждой грани имеется `CIFaceFeature`. `CIFaceFeature` имеет следующие свойства:
...
Координаты всех этих свойств имеют свое происхождение в левом нижнем углу, в отличие от UIKit, где в качестве источника используется левый верхний. При использовании координат на `CIFaceFeature` не забудьте перевернуть их. Это очень простое представление пользовательского изображения в Кореимаже\кореимажевиевконтроллер.КС демонстрирует, как рисовать на изображении треугольники "индикатор лица" (Обратите внимание на метод `FlipForBottomOrigin`):
...
Сначала загрузите изображение в объект `CIImage`.
...
В третьих, обратитесь к свойству `OutputImage` и вызовите метод `CreateCGImage`, чтобы отобразить окончательный результат.
...
На этих снимках экрана показан результат фильтров `CISepia` и `CIHueAdjust`, которые демонстрируются в примере кода Кореимаже. zip.

Пример фильтра `CIColorControls` см. в разделе [Настройка контракта и яркости рецепта изображения](https://github.com/xamarin/recipes/tree/master/Recipes/ios/media/coreimage/adjust_contrast_and_brightness_of_an_image) .
