...
Кнопка **восстановления** пользовательского интерфейса вызывает следующий метод, который активирует ресторекомплетедтрансактионс на `SKPaymentQueue`.
...
Так как `CustomPaymentObserver` регистрируется в качестве наблюдателя транзакций, она будет принимать сообщения при ответе серверов Apple. Ответ будет содержать все транзакции, которые этот пользователь когда-либо выполнял в этом приложении (на всех устройствах). Код выполняет цикл по каждой транзакции, обнаруживает восстановленное состояние и вызывает метод `UpdatedTransactions` для обработки, как показано ниже:
...
Если для пользователя нет продуктов restorable, `UpdatedTransactions` не вызывается.   
   
Самый простой код для восстановления заданной транзакции в примере выполняет те же действия, что и при покупке, за исключением того, что для доступа к ИДЕНТИФИКАТОРу продукта используется свойство `OriginalTransaction`.
...
В `CustomPaymentObserver` есть два дополнительных метода, которые будут вызываться StoreKit по завершении процесса восстановления (успешно или с ошибкой), как показано ниже:
...
Два примера в этом документе используют `NSUserDefaults` для мониторинга покупок:   
...
Использование `NSUserDefaults` поддерживает простой пример кода, но не предоставляет очень безопасное решение, так как может оказаться, что пользователи с техническими соображениями могут обновлять параметры (минуя механизм оплаты).   
...
7. `SKProductsRequestDelegate` приложения передается информация о продукте для показа пользователю.
...
4. Подкласс `SKPaymentTransactionObserver` получает уведомление и обрабатывает его. Поскольку продукт должен быть загружен с сервера, приложение инициирует сетевой запрос к удаленному серверу.   
...
10. Приложение включает продукт, а затем вызывает `FinishTransaction`StoreKit. Приложение может при необходимости отобразить приобретенное содержимое (например, показать первую страницу приобретенной книги или выпуска журнала).
...
Извлеките свойство `SKPaymentTansaction.TransactionReceipt` в приложении. Это данные, которые необходимо отправить в iTunes для проверки (шаг #5).
...
HTTP публикация JSON в [https://buy.itunes.apple.com/verifyReceipt](https://buy.itunes.apple.com/verifyReceipt) для рабочей среды или [https://sandbox.itunes.apple.com/verifyReceipt](https://sandbox.itunes.apple.com/verifyReceipt) для тестирования.   
...
Нулевое состояние указывает на допустимое получение. Сервер может перейти к выполнению содержимого приобретенного продукта. Ключ получения содержит словарь JSON с теми же свойствами, что и объект `SKPaymentTransaction`, полученный приложением, поэтому серверный код может запрашивать этот словарь для получения таких сведений, как product_id и количество покупок.
