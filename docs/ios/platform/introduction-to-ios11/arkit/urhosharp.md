...
1. В iOS текстур GPU должен иметь разрешение, которое не степень двойки, но кадры, которые мы получаем от камеры, не имеют разрешения, которые степень двойки, например: 1280x720.
