...
Приложение iOS должно создать и запустить трехмерное содержимое. для этого создайте реализацию подкласса `Urho.Application` и предоставьте код установки, переопределив метод `Start`.  В этом случае сцена заполняется данными, обработчики событий — Настройка и т. д.

Мы предоставили класс `Urho.ArkitApp`, который подклассы `Urho.Application` и в своем `Start` метод выполняет тяжелую работу.   Все, что нужно сделать для существующего приложения Урхо, — это изменить базовый класс на тип `Urho.ArkitApp` и приложение, которое будет запускать Урхо сцену в мире.
...
Программа установки выполняется в виртуальном методе `Start`.   При переопределении этого метода в подклассе необходимо создать цепочку к родительскому элементу, используя `base.Start()` для своей собственной реализации.

Метод `Start` настраивает сцену, область просмотра, камеру и направленный источник, а также отображает их как открытые свойства:

- `Scene` для хранения объектов,
- направление `Light` с тенями, расположение которых доступно через свойство `LightNode`
- `Camera`, компоненты которого обновляются, когда ARKit доставляет обновление в приложение и
- `ViewPort`, отображающий результаты.

### <a name="your-code"></a>Ваш код

Затем необходимо создать подкласс класса `ArkitApp` и переопределить метод `Start`.   Первое, что должен сделать метод, — это привязка к `ArkitApp.Start` путем вызова `base.Start()`.  После этого можно использовать любую настройку свойств с помощью Аркитапп для добавления объектов в сцену, настройки индикаторов, теней или событий, которые требуется обменять.
...
На следующей схеме показано, что происходит в классе `ArkitApp`:

[![схемы классов и экранов в Аркитапп](urhosharp-images/image2.png)](urhosharp-images/image2.png#lightbox)
...
Самый простой способ сделать это — вставить `RenderPathCommand` в основной `RenderPath`.  Это набор команд, которые выполняются для рисования одного кадра.  Эта команда заполнит окно просмотра на любую текстуру, которую мы передали.    Мы устанавливаем это на первом кадре, который обрабатывается, и фактическое определение выполняется в файле **аррендерпас. XML** , который загружается в этот момент.

Однако мы столкнулись с двумя проблемами для объединения этих двух миров:

1. В iOS текстуры GPU должны иметь разрешение, которое является степенью двух, но кадры, получаемые от камеры, не имеют разрешения, которые являются степенью двух, например: 1280 x 720.
