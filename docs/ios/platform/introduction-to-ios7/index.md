...
@No__t_0 принимает экземпляр ссылочного `UIView` или `UICollectionViewLayout`, который содержит элементы, которые будут анимированы в соответствии с присоединенным поведением.
...
@No__t_0 имеет свойство с именем `TranslatesReferenceBoundsIntoBoundry`. Присвоение этому параметру значения `true` приводит к тому, что границы ссылочного представления будут использоваться в качестве границы конфликта.
