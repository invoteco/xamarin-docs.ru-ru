...
Дополнительные сведения см. [в статье Введение в](~/ios/platform/touchid.md) документацию по СЕНСОРному идентификатору.
