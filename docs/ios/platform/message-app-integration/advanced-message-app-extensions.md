...
- **Наклейки** — это изображения, создаваемые приложением, которые могут быть включены в сообщения, отправляемые между пользователями. Пример реализации приложения пакета наклейок см. в нашем примере приложения " [Построитель](https://docs.microsoft.com/samples/xamarin/ios-samples/ios10-icecreambuilder) ".
...
Расширения приложений для работы с сообщениями похожи на существующие приложения iOS, которые знакомы разработчику, и получат доступ ко всем стандартным платформам и функциям стандартного приложения iOS. Пример:
...
1. Объект `MSMessageAppViewController` в расширении приложения сообщения является основным классом, который вызывается, когда пользователю отображается представление расширения.
2. Диалог представляется пользователю как экземпляр объекта `MSConversation`.
3. Класс `MSMessage` представляет данное всплывающее сообщение в диалоге.
4. `MSSession` определяет, как отправляется сообщение.
...
2. Метод `DidBecomeActive` вызывается и передает `MSConversation`, представляющий диалог, в котором выполняется расширение приложения сообщения.
3. Так как расширение основано на `UIViewController` и `ViewWillAppear`, и `ViewDidAppear` вызываются.
...
1. При деактивации расширения приложения для сообщений сначала вызывается метод `ViewWillDisappear`.
2. Затем будет вызван метод `ViewDidDisappear`.
3. Метод `WillResignActive` вызывается и передает `MSConversation`, представляющий диалог, в котором выполняется расширение приложения сообщения. На этом этапе будет выпущено подключение между приложением "сообщения" и расширением.
...
Этот код создает новый `MSMessage` и задает несколько свойств (например, `Url`). Хотя сообщение может быть создано только в iOS, оно может быть отправлено как в iOS, так и в macOS.
...
Свойство `AccessibilityLabel` используется программами чтения с экрана для чтения записи диалога для пользователя. Свойство `Layout` указывает, как будет отображаться сообщение, в настоящее время поддерживается только `MSMessageTemplateLayout` и выглядит следующим образом:
...
Свойство `Image` `MSMessageTemplateLayout` предоставляет содержимое для основного текста Мессажебуббле на экране. Свойство `MediaFileUrl` также предоставляет содержимое для текста всплывающего окна сообщения, но допускает содержимое, которое не поддерживается `UIImage` (например, видеофайл, который должен пройти в фоновом режиме). Если указаны свойства `Image` и `MediaFileUrl`, свойство `Image` будет иметь приоритет. `MediaFileUrl` поддерживает форматы мультимедиа PNG, JPEG, GIF и видео (в любом формате, который может воспроизводиться платформой проигрывателя мультимедиа).
...
Свойства `ImageTitle` и `ImageSubtitle` содержат описание носителя, отображаемого в пузырьковом сообщении. Эти свойства будут отправлены в виде текста на принимающее устройство, где они будут четко отображены в левом нижнем углу изображения.

Свойства `Caption`, `SubCaption`, `TrailingCaption` и `TrailingSubcaption` описывают изображение и будут подготавливаться к просмотру в разделе под изображением. Если задать для всех этих свойств значение `null`, будет создан пузырьк сообщений без области субтитров:
...
После составления `MSMessage` для его отправки можно использовать следующий код:
...
Свойство `ActiveConversation` `MSMessagesAppViewController` будет содержать текущий диалог, в котором было запущено расширение приложения сообщений.

Вызовите `InsertMessage` `MSConversation`, чтобы включить сообщение в диалог и обработайте любые ошибки, которые могут возникнуть. Если сообщение успешно включено, всплывающее сообщение будет отображаться в поле ввода.

Кроме того, расширение может передавать в диалоге различные типы данных, например:

- `ActiveConversation.InsertText ("Message", (error) => {...});` **текста** - 
...
Взгляните на следующий пример обработки переключения между двумя разными режимами представления. Для каждого состояния потребуется два разных контроллера представления. `StickerBrowserViewController` обрабатывает **свернутое** представление, а `AddStickerViewController` будет обрабатывать **развернутое** представление:
...
При необходимости приложение могло бы использовать метод `WillTransition` для изменения режима просмотра до того, как он будет представлен пользователю (как это делается в примере Ицекреам Builder выше). Дополнительные сведения см. в дополнительной документации по [настройке наклейок](~/ios/platform/message-app-integration/intro-to-message-app-extensions.md) .
...
3. Метод `DidBecomeActive` вызывается и передает `MSConversation`, представляющий диалог, в котором выполняется расширение приложения сообщения.
4. Так как расширение основано на `UIViewController` и `ViewWillAppear`, и `ViewDidAppear` вызываются.
...
1. Пользователь касания всплывающего сообщения расширения.
2. Так как расширение приложения сообщений уже активно, метод `WillTransition` `MSMessagesAppViewController` вызывается для обработки переключения из сжатия в расширенный режим просмотра.
3. Метод `DidSelectMessage` `MSMessagesAppViewController` вызывается и передает `MSMessage` и `MSConversation`, к которому относится пузырьковое сообщение.
4. Метод `DidTransition` `MSMessagesAppViewController` вызывается для управления переключением из сжатия в расширенный режим просмотра.
...
В любом случае, когда пользователь отменяет всплывающее сообщение, принадлежащее расширению приложения сообщений, ему необходимо получить доступ к `MSMessage`, которая была нажата с помощью свойства `SelectedMessage` `MSConversation`.
...
Это обрабатывается с помощью `MSSession`, чтобы свернуть все существующие шаги. Поэтому метод `DidSelectMessage` класса `MSMessagesAppViewController` можно изменить так, чтобы он выглядел следующим образом:
...
Если выбранное сообщение уже имеет выход `MSSession`, используется другой `MSSession`. Свойство `SummaryText` `MSMessage` используется для добавления заголовка к свернутым предыдущим шагам. Если свойство `SummaryText` имеет значение `null`, предыдущие шаги диалога будут полностью удалены из записи диалога.
...
Во первых, существует несколько других методов переопределения в классе `MSMessagesAppViewController`, которые обеспечивают более глубокий доступ к диалогу.
...
В любом случае это поведение нежелательно, так как пользователь 1 должен иметь доступ к выбору пользователя 2 и пользователя 3. Для решения этой ситуации компания Apple предлагает, что расширение приложения для обработки сообщений сохраняет состояние сообщения в облаке, и использует свойство URL-адреса `MSMessage` (который отправляется между пользователями) для доступа к этому состоянию.
...
4. Так как компания Apple очень важна в отношении конфиденциальности пользователей, расширение приложения для сообщений знает только о уникальном идентификаторе (в качестве `NSUUID`), которому назначен каждый участник диалога. На локальном устройстве известен только идентификатор текущего пользователя.
5. `MSMessage` имеет свойство `SenderIdentifier`, которое соответствует одному из пользователей в списке участников, известном расширению.
...
7. При отправке сообщения также известно, что его `SenderIdentifier` свойство является локальным пользователем.
...
Идентификатор отправителя можно использовать в любом текстовом поле `MSMessageTemplateLayout` путем добавления к нему знака доллара (`$`). Пример:
...
Когда приложение сообщений отображает всплывающее сообщение с таким типом форматирования, оно заменит `$uuid...` именем контактного лица в диалоговом окне диалога, отправившего сообщение.
...
Из трех платформ только iOS 10 позволит пользователю создавать интерактивное сообщение. На macOS Sierra, если пользователь щелкнет интерактивное всплывающее сообщение, URL-адрес, прикрепленный к `MSMessage`, будет открыт в Safari, и в нем должно отобразиться представление сообщения.
