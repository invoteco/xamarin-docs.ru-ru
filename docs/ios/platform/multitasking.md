...
![](multitasking-images/about02-sml.png "Пример разделения экрана") ![](multitasking-images/about03-sml.png "Пример рисунка на рисунке")
...
Сначала мы переопределяем свойство `CanBecomeFirstResponder` и возвращаем `true`, чтобы контроллер представления или представления мог получать ввод с клавиатуры. 

Далее мы переопределяем свойство `KeyCommands` и создадим новый `UIKeyCommand` для нажатия клавиши **Command-N** . При активации нажатия клавиши вызывается метод `NewEntry` (который мы предоставляем в iOS 9 с помощью команды `Export`) для выполнения запрошенного действия.
...
- Если для показа видео используется `AVPlayerViewController`, установите для свойства `AllowsPictureInPicturePlayback` значение `false`.
- Если вы используете `AVPlayerLayer` для показа видео, не создавайте экземпляр `AVPictureInPictureController`.
- Если для показа видео используется `WKWebView`, установите для свойства `AllowsPictureInPictureMediaPlayback` значение `false`.
