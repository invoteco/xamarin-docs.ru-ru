...
1. Напишите `manifest.json` как список ключей и значений JSON каждого имени файла с его хэшем.
1. Используйте сертификат, чтобы подписать файл `manifest.json` и записать результат в файл с именем `signature`.
1. Заархивируйте все содержимое и придайте результирующему файлу расширение файла `.pkpass`.
...
Основной операцией приложения многофакторной передачи является получение файла Pass и вызов PassKit `PKAddPassesViewController`, чтобы предоставить пользователю возможность добавить передачу в бумажник. Реализация этого контроллера представления рассматривается в следующем разделе **сопутствующих приложений**.
...
Чтобы добавить право на бумажник, присвойте **свойству** значение `Passbook Identifiers` в раскрывающемся списке, который автоматически установит **тип** `Array`. Затем задайте строковое **значение** `$(TeamIdentifierPrefix)*`:
...
Если у вас возникли проблемы при развертывании приложения, убедитесь, что вы используете правильный **профиль подготовки** и что в качестве файла **настраиваемых** прав в параметрах **подписи пакета iPhone** выбран параметр `Entitlements.plist`.
...
После этого массив прав `pass-type-identifiers` неверен (или не соответствует **профилю подготовки**). Проверьте правильность идентификаторов типа Pass и идентификатора команды.
