...
- **подпись** — обязательное. Создается путем подписывания файла `manifest.json` с сертификатом, созданным на портале подготовки iOS.
...
- **SerialNumber** — уникальная строка в этом `passTypeidentifier`. Значение является непрозрачным для бумажника, но важно для отслеживания конкретных проходов при взаимодействии с сервером.
...
Преобразование передачи на несколько языков аналогично локализации приложения iOS — создание каталогов на языке с расширением `.lproj` и размещение локализованных элементов внутри. Переводы текста следует вводить в файл `pass.strings`, тогда как локализованные изображения должны иметь то же имя, что и образ, заменяющий в корневом каталоге Pass.
...
1. Вычислите хэш SHA1 для каждого файла в каталоге Pass (не включайте файл `manifest.json` или `signature`, ни один из которых не должен существовать на этом этапе).
1. Запишите `manifest.json` как список ключей и значений JSON каждого имени файла с его хэшем.
1. Используйте сертификат, чтобы подписать файл `manifest.json` и записать результат в файл с именем `signature`.
1. Заархивируйте все содержимое и придайте результирующему файлу `.pkpass` расширение файла.
...
2. Введите **Описание** (имя) и **идентификатор** (уникальную строку) для прохода. Обратите внимание, что все идентификаторы типа Pass должны начинаться со строки `pass.` в этом примере используется `pass.com.xamarin.coupon.banana`:[![](passkit-images/register.png "Введите описание и идентификатор")](passkit-images/register.png#lightbox)
...
В [образце кода](https://docs.microsoft.com/samples/xamarin/ios-samples/passkit) для этой статьи есть некоторые исходные файлы, которые можно использовать для создания прохода. Используйте файлы в каталоге `CouponBanana.raw` каталога Креатеапассмануалли. Имеются следующие файлы:

 [![](passkit-images/image18.png "These files are present")](passkit-images/image18.png#lightbox)

Откройте Pass. JSON и измените JSON. Необходимо по крайней мере обновить `passTypeIdentifier` и `teamIdentifer` в соответствии с учетной записью разработчика Apple.
...
Затем необходимо вычислить хэши для каждого файла и создать файл `manifest.json`. По завершении он будет выглядеть примерно следующим образом:
...
Загрузите **материалы о поддержке начального значения бумажника** на сайте [загрузки Apple](https://developer.apple.com/downloads/index.action?name=Passbook) . Используйте средство `signpass`, чтобы превратить папку в проход (при этом также будут вычисляться хэши SHA1 и заархивировать выходные данные в пкпасс-файл).

#### <a name="testing"></a>Тестирование

Если вы изучите выходные данные этих средств (задав для файла имя. zip, а затем открыли его), вы увидите следующие файлы (Обратите внимание на добавление файлов `manifest.json` и `signature`):

 [![](passkit-images/image19.png "Examining the output of these tools")](passkit-images/image19.png#lightbox)

После подписания ZIP-файла и его переименования (например, чтобы `BananaCoupon.pkpass`), можно перетащить его в симулятор, чтобы протестировать, или отправить его самому себе для получения на реальном устройстве. Вы увидите экран, чтобы **Добавить** этот проход, как показано ниже.
...
- **Сопутствующие приложения** — они пишутся поставщиками для распространения проходов и предлагают дополнительные функции для их просмотра или редактирования. Приложения Xamarin. iOS имеют полный доступ к API PassKit для создания и обработки проходов. Затем можно добавить в бумажник, используя `PKAddPassesViewController`. Этот процесс подробно описан в разделе " **сопутствующие приложения** " этого документа.
...
Основной операцией приложения многоканалной передачи является получение файла Pass и вызов `PKAddPassesViewController` PassKit, чтобы предоставить пользователю возможность добавить передачу в бумажник. Реализация этого контроллера представления рассматривается в следующем разделе **сопутствующих приложений**.
...
Чтобы добавить право на бумажник, задайте для **Свойства** значение `Passbook Identifiers` в раскрывающемся списке, который автоматически установит `Array`**типа** . Затем задайте строковое **значение** `$(TeamIdentifierPrefix)*`:
...
Где `pass.$(CFBundleIdentifier)` — это идентификатор прохода, который был создан [выше](~/ios/platform/passkit.md)

-----

### <a name="debugging"></a>Отладка

Если у вас возникли проблемы при развертывании приложения, убедитесь, что вы используете правильный **профиль подготовки** и `Entitlements.plist` выбран в качестве файла **настраиваемых** прав в параметрах **подписи пакета iPhone** .
...
в этом случае `pass-type-identifiers` неверно (или не соответствует **профилю подготовки**). Проверьте правильность идентификаторов типа Pass и идентификатора команды.
...
Можно также использовать метод `LocalizedValueForFieldKey()` для получения данных из полей в разрабатываемых вами обработах (поскольку вы будете иметь представление о том, какие поля должны присутствовать). В примере кода это не показано.
...
После создания обновленного файла Pass используйте метод `Replace`, чтобы перезаписать старые данные на устройстве.

### <a name="display-a-pass-for-scanning"></a>Отображение прохода для сканирования

Как отмечалось ранее, только бумажник может отображать проход для сканирования. Проход можно отобразить с помощью метода `OpenUrl`, как показано ниже.
