...
Реагировать на пользователя, коснувшись результат поиска (`NSUserActivity`) для нашего приложения изменить **AppDelegate.cs** файл и переопределить `ContinueUserActivity` метод. Например:
