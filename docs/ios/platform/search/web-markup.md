...
Вы можете предоставить глубокие ссылки на содержимое вашего приложения с помощью карточки Twitter. Пример:
...
Дополнительные сведения см. в документации по [протоколу карты Twitter](https://developer.twitter.com/en/docs/tweets/optimize-with-cards/overview/abouts-cards) в Twitter.

## <a name="using-facebook-app-links"></a>Использование ссылок на приложения Facebook

Ссылки на содержимое приложения можно предоставить с помощью ссылки на приложение Facebook. Пример:
...
Дополнительные сведения см. в документации по [ссылкам для приложений](https://developers.facebook.com/docs/applinks) Facebook.

## <a name="opening-deep-links"></a>Открытие ссылок с глубокими ссылками

Необходимо добавить поддержку для открытия и отображения ссылок в приложении Xamarin. iOS. Измените файл **AppDelegate.CS** и переопределите метод `OpenURL`, чтобы он обрабатывал пользовательский формат URL-адреса. Пример:
...
В приведенном выше коде мы ищем URL-адрес, содержащий `/appname`, и передаем значение `query` (`123` в этом примере) в пользовательский контроллер представления в приложении, чтобы отобразить запрошенное содержимое для пользователя.
...
Один из вариантов предоставления структурированных данных — с помощью Open Graph. Пример:
...
Дополнительные сведения см. на веб-сайте [Open Graph](https://ogp.me) .

Другим распространенным форматом разметки структурированных данных является формат Schema. org микроданные. Пример:
...
Дополнительные сведения об этих типах схем см. в разделе [Schema.org](https://schema.org).
