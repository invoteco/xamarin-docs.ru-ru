...
1. Дважды щелкните `Entitlements.plist` файл **обозревателе решений** чтобы открыть его для редактирования.
2. Добавить `com.apple.developer.siri` **свойство**, задайте **тип** для `Boolean` и **значение** для `Yes`: 
...
4. Дважды щелкните **файл проекта** в **обозревателе решений** чтобы открыть его для редактирования.
5. Выберите **подписывание пакета iOS** и убедитесь, что `Entitlements.plist` выборе файла в **настраиваемые назначения** поля.
...
19. Убедитесь, что **идентификатор пакета** совпадает со значением в портале разработчика Apple выше: 

    [![](implementing-sirikit-images/setup06.png "Идентификатор пакета")](implementing-sirikit-images/setup06.png#lightbox)
20. В **обозревателе решений**выберите **проекта**.
21. Щелкните правой кнопкой мыши проект и выберите **параметры**.
22. Выберите **подписывание пакета iOS**выберите **удостоверение подписывания** и **профиль подготовки** созданный выше: 
...
На устройстве iOS пользователю возможность выбрать язык для Siri, который отличается, а затем системной настройки по умолчанию. При работе с локализованные данные, приложение потребуется использовать `SiriLanguageCode` метод `INPreferences` класса для получения кода языка из Siri. Например:
...
4. В **обозревателе решений**, щелкните правой кнопкой мыши **ссылки** папку созданного расширения Intents и выберите **Добавить > ссылка**. Проверьте имя Общие библиотеки проекта общего кода (создания приложения выше) и нажмите кнопку **ОК** кнопки:
...
После этого разработчику необходимо настроить основной класс, который выступает в качестве главную точку входа для расширения намерения в Siri. Он должен быть подклассом `INExtension` которого соответствует `IINIntentHandler` делегировать. Например:
...
2. В диалоговом окне выберите **iOS** > **расширения** > **расширения пользовательского интерфейса намерение** и нажмите кнопку **Далее** кнопка.
3. Далее введите **имя** намерение расширение и нажмите кнопку **ОК** кнопки.
4. В **обозревателе решений**, щелкните правой кнопкой мыши **ссылки** папку только что созданный намерение расширения. Проверьте имя Общие библиотеки проекта общего кода (создания приложения выше) и нажмите кнопку **ОК** кнопки.
...
Настройте основной класс, который выступает в качестве главную точку входа для расширения пользовательского интерфейса намерения в Siri. Он должен быть подклассом `UIViewController` которого соответствует `IINUIHostedViewController` интерфейс. Например:
