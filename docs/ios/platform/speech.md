...
Диктовка клавиатуры поддерживается любым элементом интерфейса, который поддерживает Тексткит (например, `UITextField` или `UITextArea`) и активируется пользователем нажатием кнопки диктофона (непосредственно слева от пробела) на виртуальной клавиатуре iOS.
...
Как отмечалось выше в разделе Диктовка клавиатуры, распознавание речи требует передачи и временного хранения данных на серверах Apple через Интернет, и, таким образом, приложение _должно_ запросить разрешение пользователя на выполнение распознавания, включив ключ `NSSpeechRecognitionUsageDescription` в своем `Info.plist`ном файле и вызов метода `SFSpeechRecognizer.RequestAuthorization`. 

В зависимости от источника звука, используемого для распознавания речи, могут потребоваться другие изменения в файле `Info.plist` приложения. Дополнительные сведения см. в документации по [улучшению безопасности и конфиденциальности](~/ios/app-fundamentals/security-privacy.md) .
...
- Укажите описание использования в файле `Info.plist` приложения с помощью ключа `NSSpeechRecognitionUsageDescription`. Например, приложение камеры может включать следующее описание: _«это позволяет получить фотографию, произнося слово «Мак»._
- Запросите авторизацию, вызвав метод `SFSpeechRecognizer.RequestAuthorization`, чтобы получить пояснения (предоставленное в `NSSpeechRecognitionUsageDescription`ном выше ключе), почему приложение хочет получить доступ к распознаванию речи для пользователя в диалоговом окне и позволить им принять или отклонить.
- Создайте запрос распознавания речи:
  - Для предварительно записанного звука на диске используйте класс `SFSpeechURLRecognitionRequest`.
  - Для аудио (или из памяти) в реальном времени используйте класс `SFSPeechAudioBufferRecognitionRequest`.
- Передавайте запрос распознавания речи распознавателю речи (`SFSpeechRecognizer`), чтобы начать распознавание. Приложение может дополнительно содержать возвращенные `SFSpeechRecognitionTask` для отслеживания и отслеживания результатов распознавания.

Эти действия будут подробно рассмотрены ниже.

### <a name="providing-a-usage-description"></a>Предоставление описания использования

Чтобы предоставить необходимый ключ `NSSpeechRecognitionUsageDescription` в файле `Info.plist`, выполните следующие действия.

# <a name="visual-studio-for-mactabmacos"></a>[Visual Studio для Mac](#tab/macos)

1. Дважды щелкните файл `Info.plist`, чтобы открыть его для редактирования.
...
3. Щелкните **Добавить новую запись**, введите `NSSpeechRecognitionUsageDescription` для **Свойства**, `String` для **типа** и **Описание использования** в качестве **значения**. Пример: 

    [![](speech-images/speech03.png "Adding NSSpeechRecognitionUsageDescription")](speech-images/speech03.png#lightbox)
4. Если приложение будет обрабатывать голосовую транскрипцию, для него также потребуется описание использования микрофона. Щелкните **Добавить новую запись**, введите `NSMicrophoneUsageDescription` для **Свойства**, `String` для **типа** и **Описание использования** в качестве **значения**. Пример: 
...
1. Дважды щелкните файл `Info.plist`, чтобы открыть его для редактирования.
2. Щелкните **Добавить новую запись**, введите `NSSpeechRecognitionUsageDescription` для **Свойства**, `String` для **типа** и **Описание использования** в качестве **значения**. Пример: 

    [![](speech-images/speech03w.png "Adding NSSpeechRecognitionUsageDescription")](speech-images/speech03w.png#lightbox)
3. Если приложение будет обрабатывать голосовую транскрипцию, для него также потребуется описание использования микрофона. Щелкните **Добавить новую запись**, введите `NSMicrophoneUsageDescription` для **Свойства**, `String` для **типа** и **Описание использования** в качестве **значения**. Пример: 
...
> [!IMPORTANT]
> Если не предоставить ни один из указанных выше ключей `Info.plist` (`NSSpeechRecognitionUsageDescription` или `NSMicrophoneUsageDescription`), это может привести к сбою приложения без предупреждения при попытке получить доступ к распознаванию речи или микрофону для живого аудио.
...
Метод `RequestAuthorization` класса `SFSpeechRecognizer` будет запрашивать у пользователя разрешение на доступ к распознаванию речи с помощью причины, указанной разработчиком в `NSSpeechRecognitionUsageDescription`ном ключе `Info.plist` файла.

`SFSpeechRecognizerAuthorizationStatus` результат возвращается в подпрограммы обратного вызова метода `RequestAuthorization`, которую можно использовать для выполнения действий в зависимости от разрешения пользователя. 
...
Рассмотрим этот код подробно, сначала он попытается создать распознаватель речи (`SFSpeechRecognizer`). Если язык по умолчанию не поддерживается для распознавания речи, возвращается `null` и функции завершаются.

Если распознаватель речи доступен для языка по умолчанию, приложение проверяет, доступно ли в настоящее время распознавание с помощью свойства `Available`. Например, распознавание может быть недоступно, если у устройства нет активного подключения к Интернету.

`SFSpeechUrlRecognitionRequest` создается из `NSUrl` расположения предварительно записанного файла на устройстве iOS и передается распознавателю речи для обработки с помощью подпрограммы обратного вызова.

При вызове обратного вызова, если `NSError` не `null` возникла ошибка, которая должна быть обработана. Так как распознавание речи выполняется постепенно, подпрограммы обратного вызова могут быть вызваны более одного раза, поэтому свойство `SFSpeechRecognitionResult.Final` проверяется на предмет завершения перевода и выводится лучшая версия перевода (`BestTranscription`).

### <a name="recognizing-live-speech"></a>Распознавание речи в реальном времени

Если приложение хочет распознать речь, процесс очень похож на распознавание предварительно записанных речевых функций. Пример:
...
Он использует AV Foundation для записи звука, который будет передан `SFSpeechAudioBufferRecognitionRequest` для выполнения запроса на распознавание:
...
Важно вызывать `RecognitionTask.Cancel`, если пользователь отменяет перевод, освобождая память и процессор устройства.

> [!IMPORTANT]
> Если не предоставить `NSSpeechRecognitionUsageDescription` или `NSMicrophoneUsageDescription` ключи `Info.plist` могут привести к сбою приложения без предупреждения при попытке получить доступ к распознаванию речи или микрофону для аудио в реальном времени (`var node = AudioEngine.InputNode;`). Дополнительные сведения см. в разделе **предоставление описания использования** выше.
