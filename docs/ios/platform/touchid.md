...
7. Создайте ручной `AuthenticationViewController` перехода из, щелкнув черную полосу в нижней части экрана и **удерживая нажатой клавишу CTRL, перетащите** ползунок в область `MasterViewController` и выберите команду **Отправить** (или **покажите** , если используется класс размера):
