...
До версии iOS 10 приложение iOS использовало `UIApplication` для регистрации уведомления в системе и планирования его активации (по времени или расположению).
...
В iOS 10 компания Apple представила новую платформу уведомлений пользователей, которая заменяет существующий метод `UIApplication`, указанный выше.
...
Разрешение на уведомление следует запрашивать сразу после запуска приложения, добавив следующий код в метод `FinishedLaunching` `AppDelegate` и указав требуемый тип уведомления (`UNAuthorizationOptions`):
...
> Отсутствует право на отправку push-уведомлений — приложение включает API для службы push-уведомлений Apple, но в сигнатуре приложения отсутствует право `aps-environment`.

Чтобы предоставить необходимый объем обслуживания, выполните следующие действия.

# <a name="visual-studio-for-mactabmacos"></a>[Visual Studio для Mac](#tab/macos)

1. Дважды щелкните файл `Entitlements.plist` в **панель решения** , чтобы открыть его для редактирования.
...
4. Введите `aps-environment` для **Свойства**, оставьте **тип** `String` и введите для **значения**либо `development`, либо `production`. 
...
3. Введите `aps-environment` для **Свойства**, оставьте **тип** `String` и введите для **значения**либо `development`, либо `production`. 
...
Если приложение будет отправлять и получать удаленные уведомления, ему по-прежнему потребуется _Регистрация маркеров_ с помощью существующего `UIApplication` API. Для этой регистрации устройство должно иметь действующее сетевое подключение APNs, что приведет к формированию необходимого маркера, который будет отправлен в приложение. Приложению необходимо затем перенаправить этот маркер в приложение на стороне сервера разработчика для регистрации удаленных уведомлений:
...
В iOS 10 приложение может выполнять уведомления по-разному, когда оно находится на переднем плане, и запускается уведомление. Предоставляя `UNUserNotificationCenterDelegate` и реализуя метод `WillPresentNotification`, приложение может принять за собой ответственность за отображение уведомления. Пример:
...
Этот код просто записывает содержимое `UNNotification` в выход приложения и запрашивает у системы отображение стандартного оповещения для уведомления. 

Если приложение хочет отобразить само уведомление, когда оно находилось на переднем плане, и не использовало системные значения по умолчанию, передайте `None` обработчику завершения. Пример

```csharp
completionHandler (UNNotificationPresentationOptions.None);
```

Выполнив этот код, откройте файл `AppDelegate.cs` для редактирования и измените метод `FinishedLaunching`, чтобы он выглядел следующим образом:
...
Этот код присоединяет настраиваемый `UNUserNotificationCenterDelegate` выше к текущему `UNUserNotificationCenter` поэтому приложение может управлять уведомлением, когда оно активно и находится на переднем плане.
...
Важной частью управления уведомлениями является _идентификатор запроса_ , который был назначен уведомлению при его создании и планировании в системе. Для удаленных уведомлений это значение назначается с помощью нового поля `apps-collapse-id` в заголовке HTTP-запроса.
...
Каждое созданное действие должно быть связано с категорией. При создании нового `UNNotificationCategory`ему назначается уникальный идентификатор, список действий, которые он может выполнить, список идентификаторов намерений для предоставления дополнительных сведений о намерениях действий в категории и некоторые параметры для управления поведением категории.

Наконец, все категории регистрируются в системе с помощью метода `SetNotificationCategories`.
...
Для локальных уведомлений задайте свойство `CategoryIdentifier` объекта `UNMutableNotificationContent`. Пример:
...
Когда пользователь взаимодействует с настраиваемыми действиями и категориями, созданными ранее, приложение должно выполнить запрошенную задачу. Это делается путем предоставления `UNUserNotificationCenterDelegate` и реализации метода `UserNotificationCenter`. Пример:
...
Переданный класс `UNNotificationResponse` имеет свойство `ActionIdentifier`, которое может быть либо действием по умолчанию, либо действием закрытия. Используйте `response.Notification.Request.Identifier` для проверки каких-либо настраиваемых действий.

Свойство `UserText` содержит значение любого вводимого пользователем текста. Свойство `Notification` содержит исходное уведомление, включающее запрос с триггером и содержимым уведомления. Приложение может решить, было ли оно локальным или удаленным уведомлением в зависимости от типа триггера.
...
3. Выберите **расширения** > **iOS** > **расширения службы уведомлений** и нажмите кнопку **Далее** : 
...
> [!IMPORTANT]
> Идентификатор пакета для расширения службы должен соответствовать идентификатору пакета основного приложения с `.appnameserviceextension`, добавленным в конец. Например, если основное приложение имело идентификатор пакета `com.xamarin.monkeynotify`, расширение службы должно иметь идентификатор пакета `com.xamarin.monkeynotify.monkeynotifyserviceextension`. Это значение должно автоматически устанавливаться при добавлении расширения в решение. 
...
Первый метод, `DidReceiveNotificationRequest`, будет передавать идентификатор уведомления, а также содержимое уведомления через объект `request`. Переданный `contentHandler` потребуется вызвать, чтобы предоставить пользователю уведомление.

Второй метод, `TimeWillExpire`, будет вызываться непосредственно до того момента, пока будет запущено расширение службы для обработки запроса. Если расширение службы не может вызвать `contentHandler` за отведенное время, исходное содержимое будет отображаться для пользователя.
...
Новый ключ `mutable-content` указывает, что для обновления удаленного содержимого уведомлений необходимо запустить расширение службы. Ключ `encrypted-content` содержит зашифрованные данные, которые расширение службы может расшифровать перед представлением пользователю.
...
Этот код расшифровывает зашифрованное содержимое из ключа `encrypted-content`, создает новую `UNMutableNotificationContent`, устанавливает свойство `Body` в расшифрованное содержимое и использует `contentHandler` для предоставления пользователю уведомления.
