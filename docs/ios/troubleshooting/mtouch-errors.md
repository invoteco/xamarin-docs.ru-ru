...
- Полные журналы сборки с максимальной степенью детализации (например, `-v -v -v -v` в **дополнительных аргументах mtouch**);
...
### <a name="mt0011--was-built-against-a-more-recent-runtime--than-monotouch-supports"></a>MT0011: \* был построен на более поздней версии среды выполнения (\*), чем поддерживает однокасание.
...
Удалите параметры `--unsupported--enable-generics-in-registrar` и `--registrar`. Начиная с Xamarin. iOS 7.2.1 регистратор по умолчанию поддерживает универсальные шаблоны.
...
### <a name="mt0030-the-executable-name--and-the-app-name--are-different-this-may-prevent-crash-logs-from-getting-symbolicated-properly"></a>MT0030: имя исполняемого файла (\*) и имя приложения (\*) отличаются, это может препятствовать правильному проведения результативного журналов аварийного завершения.
...
### <a name="mt0034-cannot-include-both-monotouchdll-and-xamariniosdll-in-the-same-xamarinios-project----is-referenced-explicitly-while--is-referenced-by-"></a>MT0034: не может одновременно включать "неtouch. dll" и "Xamarin. iOS. dll" в один и тот же проект Xamarin. iOS — указана явная ссылка на "\*", а "*" ссылается на "\*".
...
### <a name="mt0041-cannot-reference-both-monotouchdll-and-xamariniosdll"></a>MT0041: не может ссылаться на "неtouch. dll" и "Xamarin. iOS. dll".

<a name="MT0042" />

### <a name="mt0042-no-reference-to-either-monotouchdll-or-xamariniosdll-was-found-a-reference-to-monotouchdll-will-be-added"></a>MT0042: не найдена ссылка на ни один из этих файлов. dll или Xamarin. iOS. dll. Будет добавлена ссылка на библиотеку. dll.
...
### <a name="mt0051-xamarinios--requires-xcode-50-or-later-the-current-xcode-version-found-in--is-"></a>MT0051: Xamarin. iOS \* требует Xcode 5,0 или более поздней версии. Текущая версия Xcode (находится в \*) \*.
...
### <a name="mt0071-unknown-platform--this-usually-indicates-a-bug-in-xamarinios-please-file-a-bug-report-at-httpbugzillaxamarincom-with-a-test-case"></a>MT0071: неизвестная платформа: *. Обычно это указывает на ошибку в Xamarin. iOS; Запишите отчет об ошибке в http://bugzilla.xamarin.com с тестовым случаем.
...
### <a name="mt0079-the-recommended-xcode-version-for-xamarinios--is-xcode--or-later-the-current-xcode-version-found-in--is-"></a>MT0079: рекомендуемая версия Xcode для Xamarin. iOS \* — Xcode \* или более поздней версии. Текущая версия Xcode (находится в \*) \*.
...
В этом примере выполняется попытка создать динамическую библиотеку и платформу с одним и тем же созданием (`MyBinary`).
...
Дополнительные сведения см. в документации по `--assembly-build-target`.
...
### <a name="mt0107-the-assemblies--have-different-custom-llvm-optimizations--which-is-not-allowed-when-they-are-all-compiled-to-a-single-binary"></a>MT0107: сборки "\*" имеют различные пользовательские оптимизации LLVM (\*), что недопустимо, если все они компилируются в один двоичный файл.
...
- так как аргументы для компилятора AOT отличаются между собой в приложении-контейнере (\*) и расширением (\*).
...
- так как другие аргументы компилятора AOT отличаются от других аргументов в приложении-контейнере (\*) и расширении (\*).
...
- так как LLVM не включена и не отключена как в приложении-контейнере (\*), так и в расширении (\*).
...
- так как пропущенные сборки для управляемого компоновщика различаются между приложением-контейнером (\*) и расширением (\*).
...
- так как приложение-контейнер строится для интерфейса ABI \*, который несовместим с ABI (\*) расширения.
...
Как правило, Xamarin. iOS запрашивает у собственного компоновщика возможность сохранения таких символов (с помощью флага компоновщика `-u symbol`), но при компиляции для bitcode собственный компоновщик не принимает флаг `-u`.
...
Если `--dynamic-symbol-mode=linker` передается в mtouch, это альтернативное решение будет отключено, а Xamarin. iOS попытается передать `-u` в собственный компоновщик. Скорее всего, это приведет к ошибкам в собственном компоновщике.

Решение заключается в удалении аргумента `--dynamic-symbol-mode=linker` из дополнительных аргументов mtouch в параметрах сборки проекта.
...
### <a name="mt0135-did-not-link-system-framework-0-referenced-by-assembly-1-because-it-was-introduced-in-2-3-and-were-using-the-2-4-sdk"></a>MT0135: не удалось связать системную платформу "{0}" (на которую ссылается сборка "{1}"), так как она появилась в {2} {3}, и мы используем пакет SDK {2} {4}.
...
### <a name="mt1005-could-not-kill-the-application--on-the-device----you-may-have-to-kill-the-application-manually"></a>MT1005: не удалось завершить работу приложения "\*" на устройстве "\*": *-возможно, потребуется вручную завершить работу приложения.
...
### <a name="mt1013-dependency-tracking-error-no-files-to-compare-please-file-a-bug-report-at-httpbugzillaxamarincom-with-a-test-case"></a>MT1013: ошибка отслеживания зависимостей: нет файлов для сравнения. Запишите отчет об ошибке в http://bugzilla.xamarin.com с тестовым случаем.
...
Удалите `NOTICE` каталога из проекта.
...
### <a name="mt1026-could-not-download-the-crash-report--from-the-device-"></a>MT1026: не удалось загрузить отчет о сбое \* с устройства \*.
...
Для выполнения нескольких операций из mtouch требуется наличие файла `DeveloperDiskImage.dmg`.   Этот файл является частью Xcode и обычно находится относительно пакета SDK, который используется для сборки в `Xcode.app/Contents/Developer/iPhoneOS.platform/DeviceSupport/VERSION/DeveloperDiskImage.dmg`.
...
### <a name="mt1210-invalid-simulator-specification--unknown-key-"></a>MT1210: Недопустимая спецификация симулятора: \*, неизвестный ключ "\*"
...
- Передайте явную версию пакета SDK с помощью--SDK \<версии >.
...
### <a name="mt1301-native-library---was-ignored-since-it-does-not-match-the-current-build-architectures-"></a>MT1301: собственная библиотека `*` (\*) была пропущена, так как она не соответствует текущей архитектуре сборки (\*)
...
### <a name="mt1304-the-embedded-framework--in--is-invalid-it-does-not-contain-an-infoplist"></a>MT1304: Встроенная платформа "\*" в \* недопустима: она не содержит info. plist.
...
### <a name="mt1402-could-not-read-file-info-for--afcfileinfoopen-returned-"></a>MT1402: не удалось прочитать сведения о файле для \*: Афкфилеинфупен возвращено \*
...
### <a name="mt1403-could-not-read-crash-report-afcdirectoryopen--returned-"></a>MT1403: не удалось прочитать отчет о сбоях: Афкдиректорйопен (\*) вернул: \*
...
### <a name="mt1404-could-not-read-crash-report-afcfilerefopen--returned-"></a>MT1404: не удалось прочитать отчет о сбоях: Афкфилерефопен (\*) вернул: \*
...
### <a name="mt1405-could-not-read-crash-report-afcfilerefread--returned-"></a>MT1405: не удалось прочитать отчет о сбоях: Афкфилерефреад (\*) вернул: \*
...
### <a name="mt1406-could-not-list-crash-reports-afcdirectoryopen--returned-"></a>MT1406: не удалось вывести список отчетов о сбоях: Афкдиректорйопен (\*) вернул: \*
...
Формат библиотеки можно проверить с помощью команды `file` в терминале:
...
Формат архива FAT можно проверить с помощью команды `file` из терминала:
...
Формат файла можно проверить с помощью команды `file` из терминала:
...
Единственными допустимыми значениями являются `default` и `appletls`, и оба они представляют один и тот же параметр, обеспечивающий поддержку SSL/TLS с помощью собственного API Apple TLS.
...
### <a name="mt2016-invalid-tlsprovider-legacy-option-the-only-valid-value-appletls-will-be-used"></a>MT2016: недопустимый параметр `legacy` Тлспровидер. Будет использовано единственное допустимое значение `appletls`.

Поставщик `legacy`, который был полностью управляемым поставщиком только для SSLv3 и TLSv1, не поставляется с Xamarin. iOS больше. Проекты, которые использовали этот старый поставщик, теперь производили сборку с более новой `appletls`.
...
### <a name="mt2018-the-assembly--is-referenced-from-two-different-locations--and-"></a>MT2018: сборка "\*" упоминается из двух разных расположений: "\*" и "*".
...
### <a name="mt2030-remove-user-resources-failed-processing-"></a>MT2030: не удалось удалить ресурсы пользователя при обработке `...`.

Произошла непредвиденная ошибка при попытке удаления пользовательских ресурсов. Сборка, вызвавшая ошибку, называется в сообщении об ошибке. Чтобы устранить эту проблему, необходимо предоставить сборке новую проблему на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new) вместе с полным журналом сборки с включенной детализацией (т. е. `-v -v -v -v` в **дополнительных аргументах mtouch**).
...
- `__monotouch_content_*` и `__monotouch_pages_*` ресурсов; перетаскивани
- Встроенные библиотеки, внедренные в сборку привязки;

<a name="MT2040" />

### <a name="mt2040-default-httpmessagehandler-setter-failed-processing-"></a>MT2040: сбой при обработке метода задания HttpMessageHandler по умолчанию `...`.

Возникла непредвиденная ошибка при попытке задать `HttpMessageHandler` по умолчанию для приложения. Запишите новую ошибку на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new) вместе с полным журналом сборки с включенной детализацией (т. е. `-v -v -v -v` в **дополнительных аргументах mtouch**).

<a name="MT2050" />

### <a name="mt2050-code-remover-failed-processing-"></a>MT2050: код удаления не смог обработать `...`.
...
### <a name="mt2060-sealer-failed-processing-"></a>MT2060: не удалось обработать `...`с запечатыванием.
...
### <a name="mt2070-metadata-reducer-failed-processing-"></a>MT2070: не удалось обработать `...`у сокращения метаданных.
...
Возникла непредвиденная ошибка при попытке пометить `NSObject` подклассами из приложения. Сборка, вызвавшая ошибку, называется в сообщении об ошибке. Чтобы устранить эту проблему, необходимо предоставить сборке новую проблему на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new) вместе с полным журналом сборки с включенной детализацией (т. е. `-v -v -v -v` в **дополнительных аргументах mtouch**).

<a name="MT2090" />

### <a name="mt2090-inliner-failed-processing-"></a>MT2090: не удалось обработать `...`во встроенном коде.
...
Сборка, вызвавшая ошибку, называется в сообщении об ошибке. Чтобы устранить эту проблему, необходимо предоставить сборке новую проблему на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new) вместе с полным журналом сборки с включенной детализацией (т. е. `-v -v -v -v` в **дополнительных аргументах mtouch**).
...
### <a name="mm2104-unable-to-link-assembly-0-as-it-is-mixed-mode"></a>MM2104: не удалось связать сборку "{0}", так как она смешанного режима.

Сборки в смешанном режиме не могут быть обработаны компоновщиком.

Дополнительные сведения о сборках в смешанном режиме см. в разделе https://docs.microsoft.com/cpp/dotnet/mixed-native-and-managed-assemblies.
...
### <a name="mt4002-failed-to-compile-the-generated-code-for-pinvoke-methods-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4002: не удалось скомпилировать созданный код для методов P/Invoke. Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
### <a name="mt4102-the-registrar-found-an-invalid-type--in-signature-for-method--use--instead"></a>MT4102: регистратор обнаружил недопустимый тип `*` в сигнатуре для метода `*`. Используйте вместо этого `*`.
...
### <a name="mt4106-the-registrar-cannot-marshal-the-return-value-for-structure--in-signature-for-method-"></a>MT4106: регистратор не может маршалировать возвращаемое значение для Structure `*` в сигнатуре для метода `*`.
...
### <a name="mt4109-failed-to-compile-the-generated-registrar-code-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4109: не удалось скомпилировать созданный код регистратора. Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
### <a name="mt4114-unexpected-error-in-the-registrar-for-the-method----please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4114: непредвиденная ошибка в регистраторе для метода "\*.\*"— файл отчета об ошибке можно отправить по адресу http://bugzilla.xamarin.com
...
### <a name="mt4117-the-registrar-found-a-signature-mismatch-in-the-method----the-selector-indicates-the-method-takes--parameters-while-the-managed-method-has--parameters"></a>MT4117: регистратор обнаружил несовпадение сигнатур в методе "\*.\*"— селектор указывает, что метод принимает \* параметры, а управляемый метод имеет \* параметров.

<a name="MT4118" />

### <a name="mt4118-cannot-register-two-managed-types--and--with-the-same-native-name-"></a>MT4118: не удается зарегистрировать два управляемых типа ("\*" и "\*") с одним и тем же собственным именем ("*").
...
### <a name="mt4120-the-registrar-found-an-unknown-field-type--in-field--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4120: регистратор обнаружил неизвестный тип поля "\*" в поле "\*. *". Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
### <a name="mt4124-invalid--found-on--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4124: в "\*" обнаружен недопустимый \*. Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
### <a name="mt4134-your-application-is-using-the--framework-which-isnt-included-in-the-ios-sdk-youre-using-to-build-your-app-this-framework-was-introduced-in-ios--while-youre-building-with-the-ios--sdk-please-select-a-newer-sdk-in-your-apps-ios-build-options"></a>MT4134: ваше приложение использует платформу "\*", которая не включена в пакет SDK для iOS, который вы используете для сборки приложения (Эта платформа появилась в iOS \*, а при создании с помощью пакета SDK для iOS \*.) Выберите более новый пакет SDK в параметрах сборки iOS приложения.
...
В этом случае Xamarin. iOS переопределит `retain`, `release` и `dealloc` в классе `MyNSObject`, и нет конфликтов.
...
### <a name="mt4144-cannot-register-the-method--since-it-does-not-have-an-associated-trampoline-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4144: не удается зарегистрировать метод "*", так как у него нет связанного трамполине. Запишите отчет об ошибке по адресу http://bugzilla.xamarin.com.
...
Имя класса Обжектице-C не может содержать пробелы. Это означает, что атрибут `Register` в соответствующем управляемом классе не может иметь параметр `Name` не может содержать пробелы.

Убедитесь, что атрибут `Register` в управляемом классе, упомянутом в сообщении об ошибке, не содержит пробелов.
...
### <a name="mt4150-cannot-register-the-type--because-the-type-property--in-its-category-attribute-does-not-inherit-from-nsobject"></a>MT4150: не удается зарегистрировать тип "\*", так как свойство "тип" ("\*") в атрибуте category не наследует от Нсобжект.
...
### <a name="mt4153-cannot-register-the-type--as-a-category-because-its-generic"></a>MT4153: невозможно зарегистрировать тип "\*" как категорию, так как он является универсальным.
...
### <a name="mt4162-the-type--used-as--2-is-not-available-in---it-was-introduced-in---please-build-with-a-newer--sdk-usually-done-by-using-the-most-recent-version-of-xcode"></a>MT4162: тип "\*" (используется как * {2}) недоступен в * * (он был введен в * *)\* выполните сборку с более новым пакетом SDK * (обычно это делается с помощью последней версии Xcode.
...
### <a name="mt4163-internal-error-in-the-registrar--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4163: Внутренняя ошибка в регистраторе (*). Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com

Эта ошибка указывает на ошибку в Xamarin. iOS. Запишите новую ошибку на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new).

<a name="MT4164" />

### <a name="mt4164-cannot-export-the-property--because-its-selector--is-an-objective-c-keyword-please-use-a-different-name"></a>MT4164: не удается экспортировать свойство "\*", так как его селектор "\*" является ключевым словом C. Укажите другое имя.
...
### <a name="mt4166-cannot-register-the-method--because-the-signature-contains-a-type--that-isnt-a-reference-type"></a>MT4166: не удается зарегистрировать метод "\*", так как сигнатура содержит тип (\*), который не является ссылочным типом.
...
### <a name="mt4167-cannot-register-the-method--because-the-signature-contains-a-generic-type--with-a-generic-argument-type-that-isnt-an-nsobject-subclass-"></a>MT4167: не удается зарегистрировать метод "\*", так как сигнатура содержит универсальный тип (\*) с универсальным типом аргумента, который не является подклассом Нсобжект (*).

Обычно это указывает на ошибку в Xamarin. iOS. Запишите новую ошибку на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new).

<a name="MT4168" />

### <a name="mt4168-cannot-register-the-type-managed_name-because-its-objective-c-name-exported_name-is-an-objective-c-keyword-please-use-a-different-name"></a>MT4168: не удается зарегистрировать тип "{Managed\_Name}", так как его цель-C "{Exported\_Name}" является ключевым словом с. Укажите другое имя.
...
### <a name="mt5102-failed-to-assemble-the-file--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT5102: не удалось собрать файл "*". Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com

<a name="MT5103" />

### <a name="mt5103-failed-to-compile-the-file--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT5103: не удалось скомпилировать файл "*". Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
### <a name="mt5106-could-not-compile-the-files--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT5106: не удалось скомпилировать файлы "*". Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
- Для привязки к третьей стороне требуется платформа, но привязка не указывает ее в атрибуте `[LinkWith]`. Решения:
  ...
  - Если вы не можете изменить привязку стороннего разработчика, можно вручную связать ее с требуемой платформой, передав `-gcc_flags '-framework SystemFramework'` `mtouch` (это делается путем изменения дополнительных аргументов mtouch на странице параметров сборки iOS проекта. Помните, что это необходимо сделать для каждой конфигурации проекта.
...
- Привязка или библиотека стороннего разработчика построена с помощью C++, но привязка не указывает это в атрибуте `[LinkWith]`. Обычно это довольно легко понять, поскольку символы являются искаженными C++ символами (один из распространенных примеров — `__ZNKSt9exception4whatEv`).
  ...
  - Если не удается изменить привязку стороннего разработчика или связывание с сторонней библиотекой вручную, можно установить эквивалентный флаг, передав `-cxx` mtouch (это делается путем изменения дополнительных аргументов mtouch на странице параметров сборки iOS проекта. Помните, что это необходимо сделать для каждой конфигурации проекта.
...
- Собственная библиотека неверно построена и содержит один и тот же символ более одного раза.
  ...
  - Попросите компоновщика удалить неиспользуемый код. Xamarin. iOS сделает это автоматически при соблюдении всех следующих условий:
    ...
    - `-gcc_flags` не передается в mtouch (в поле дополнительных аргументов mtouch в параметрах сборки iOS проекта).
    - Также можно попросить компоновщика удалить неиспользуемый код, добавив `-gcc_flags -dead_strip` к дополнительным аргументам mtouch в параметрах сборки iOS проекта.
...
### <a name="mt5216-native-linking-failed-for--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT5216: не удалось выполнить собственную компоновку для *. Запишите отчет об ошибках по адресу http://bugzilla.xamarin.com
...
Как правило, Xamarin. iOS запрашивает у собственного компоновщика возможность сохранения таких символов с помощью флага компоновщика `-u symbol`, но если имеется много таких символов, вся Командная строка может превысить максимальную длину командной строки, указанную операционной системой.

Существует несколько возможных источников для таких динамических символов:

- P/вызывает методы в статически связанных библиотеках (где имя библиотеки DLL `__Internal` в атрибуте DllImport `[DllImport ("__Internal")]`).
- Поле ссылается на места в памяти в статически связанных библиотеках из привязки проектов (`[Field]` атрибуты).
...
- Включите статический регистратор, передав `--registrar:static` в качестве дополнительного аргумента mtouch в параметрах сборки iOS проекта (для сборок симулятора, так как статический регистратор уже используется по умолчанию для сборок устройств). Статический регистратор создаст код, который ссылается на классы цели-C статически, поэтому нет необходимости запрашивать собственный компоновщик для сохранения таких классов.
...
Аргумент командной строки `--ignore-dynamic-symbol=symbol` был передан, но этот символ не является символом, который был распознан как динамический символ, который необходимо сохранить вручную.

Для этого существуют две основные причины.

- Неверное имя символа.
  - Не ставьте символ подчеркивания в начале имени символа.
  - Символ для классов цели-C — `OBJC_CLASS_$_<classname>`.
...
### <a name="mt6002-could-not-strip-assembly-"></a>MT6002: не удалось удалить `*`сборки.
...
- Попробуйте запустить `csharp -e 'System.Net.Dns.GetHostEntry (System.Net.Dns.GetHostName ()).AddressList'` (который должен предоставить IP-адрес, а не ошибку).
...
В некоторых случаях это "локальная сеть", и ее можно устранить, добавив неизвестный `127.0.0.1    MyHost.local` узла в `/etc/hosts`.
...
### <a name="mt7035-the-watch-app--does-not-have-a-valid-uidevicefamily-value-expected--but-found--"></a>MT7035: приложение Watch "\*" не имеет допустимого значения Уидевицефамили. Ожидалось "\*", но найдено "\* (\*)".
...
### <a name="mt8001-version-mismatch-between-the-native-xamarinios-runtime-and-monotouchdll-please-reinstall-xamarinios"></a>MT8001: несоответствие версий собственной среды выполнения Xamarin. iOS и файла. dll. Переустановите Xamarin. iOS.
...
### <a name="mt8004-cannot-create-an-instance-of--for-the-native-object-0x-of-type--because-another-instance-already-exists-for-this-native-object-of-type-"></a>MT8004: не удается создать экземпляр \* для собственного объекта 0x * (типа "\*"), так как уже существует другой экземпляр для этого собственного объекта (типа \*).
...
### <a name="mt8008-the-loaded-version-of-xamariniosdll-was-compiled-for--bits-while-the-process-is--bits-please-file-a-bug-at-httpbugzillaxamarincom"></a>MT8008: загруженная версия Xamarin. iOS. dll была скомпилирована для \* бит, а процесс — \* битах. Запишите ошибку по адресу http://bugzilla.xamarin.com.
...
### <a name="mt8009-unable-to-locate-the-block-to-delegate-conversion-method-for-the-method-s-parameter--please-file-a-bug-at-httpbugzillaxamarincom"></a>MT8009: не удалось разместить блок для делегирования метода преобразования для метода *.* " s параметр # *. Запишите ошибку по адресу http://bugzilla.xamarin.com.
...
### <a name="mt8018-internal-consistency-error-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT8018: Внутренняя ошибка согласованности. Запишите отчет об ошибке по адресу http://bugzilla.xamarin.com.
...
### <a name="mt8022-expected-the-token-reference--to-be-a--but-its-a--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT8022: ожидалось, что ссылка на маркер \* быть \*, но это \*. Запишите отчет об ошибке по адресу http://bugzilla.xamarin.com.

Это указывает на ошибку в Xamarin. iOS. Запишите новую ошибку на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new).

<a name="MT8023" />

### <a name="mt8023-an-instance-object-is-required-to-construct-a-closed-generic-method-for-the-open-generic-method--token-reference--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT8023: объект экземпляра требуется для создания закрытого универсального метода для открытого универсального метода: \* (ссылка на маркер: \*). Запишите отчет об ошибке по адресу http://bugzilla.xamarin.com.

Это указывает на ошибку в Xamarin. iOS. Запишите новую ошибку на [GitHub](https://github.com/xamarin/xamarin-macios/issues/new).

<a name="MT8024" />

### <a name="mt8024-could-not-find-a-valid-extension-type-for-the-smart-enum-smart_type-please-file-a-bug-at-httpsbugzillaxamarincom"></a>MT8024: не удалось найти допустимый тип расширения для интеллектуального перечисления "{smart_type}". Запишите ошибку по адресу https://bugzilla.xamarin.com.
