...
- Добавьте к элементу атрибут [`[Preserve]`](http://www.go-mono.com/docs/index.aspx?link=T:MonoTouch.Foundation.PreserveAttribute) .  Это предотвратит удаление компоновщиком.
...
Указанное выше определение автоматически создается Visual Studio для Mac для любых файлов XIB, добавляемых в Visual Studio для Mac в `NAME_OF_YOUR_XIB_FILE.designer.xib.cs` файле.
...
Создаваемые конструктором разделяемые классы должны иметь соответствующие разделяемые классы в пользовательском коде, который наследует от какого-либо подкласса `NSObject`, часто `UIViewController`. Убедитесь, что у вас есть такой класс для типа, который предоставляет ошибку.
...
Проблема заключается в том, что Mono берет на себя `libsqlite3.dylib`OS X, а не `libsqlite3.dylib` файл iPhoneSimulator. Приложение *будет* работать на устройстве, но только не в симуляторе.
...
Если проблема по-прежнему существует, подайте сообщение [об ошибке](https://monodevelop.com/Developers#Reporting_Bugs), присоединив файл **~/либрари/логс/ксамаринстудио-{версион}/иде-{тиместамп}.лог**, **андроидтулс-{timestamp}. log**и **компоненты — {timestamp}. log** Files.
...
Вложите журналы XS, **~/либрари/логс/ксамаринстудио-{версион}/иде-{тиместамп}.лог**, **АНДРОИДТУЛС-{timestamp}. log**и **Components-{timestamp}. log** (в более ранних версиях XS/MonoDevelop, просто отправьте **~/либрари/логс/монодевелоп-(3.0 | 2.8 | 2.6)/MonoDevelop.log**).
...
В Xamarin. iOS 1,4 мы предложим более детализированный метод отладки. по умолчанию будет предоставляться только инструментирование отладки для кода и библиотек, и это не будет сделано для всех [сборок Mono](~/cross-platform/internals/available-assemblies.md) (это по-прежнему возможно, но вам придется принять участие в отладке этих сборок).
...
Если вы хотите использовать Interface Builder, установите серию XCode 3, доступную на веб-сайте Apple.
...
Это можно устранить, открыв параметры проекта Xamarin. iOS, перейдя в **iOS Build** > **интернационализации** и проверяя **западную** международную линию.
