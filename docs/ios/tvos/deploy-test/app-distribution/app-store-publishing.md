...
Подробные инструкции см. в нашей документации по [подготовке служб приложений](~/ios/get-started/installation/device-provisioning/manual-provisioning.md#provisioning-for-application-services). Хотя этот документ был написан для iOS, те же действия используются для предоставления приложения Xamarin. tvOS.
