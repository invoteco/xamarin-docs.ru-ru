...
Кроме того, `UIKit` была изменена для работы в новом расширенном колорспаце **sRGB** , что упрощает смешение цветов в широких цветовых палитрах без значительной потери производительности.

При работе с широкими цветами Apple предлагает следующие рекомендации:

- `UIColor` теперь использует цветовое пространство sRGB и больше не будет отменять значения для `0.0` до `1.0` диапазона. Если приложение использует предыдущее поведение среза, его необходимо изменить для tvOS 10.
