...
- **Используйте короткое, значимые существительные** -заголовок сегмента следует явно указано, тип содержимого, который пользователя следует ожидать, что при выборе конкретного сегмента. Пример: Музыки или видео.
