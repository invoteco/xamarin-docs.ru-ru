...
`UITableViewCell` используется для представления отдельных строк данных в табличном представлении. Компания Apple определила несколько типов ячеек таблицы по умолчанию:
...
1. В Visual Studio для Mac запустите новый проект приложения tvOS и выберите **tvOS** > **приложение** > приложение с **одним представлением** и нажмите кнопку **Далее** : 
...
1. В **панель решения**дважды щелкните файл `Main.storyboard`, чтобы открыть его в конструкторе iOS: 
...
1. В Visual Studio запустите новый проект приложения tvOS и выберите **tvOS** > приложение с **одним представлением** и введите имя приложения. Нажмите кнопку " **ОК** ", чтобы создать новое решение: 

    [![](table-views-images/table02-vs.png "Select Single View App")](table-views-images/table02-vs.png#lightbox)
1. В **Обозреватель решений**дважды щелкните файл `Main.storyboard`, чтобы открыть его в конструкторе iOS: 
...
Чтобы создать модель данных для **притяжения**, щелкните правой кнопкой мыши имя проекта в **панель решения** и выберите **добавить** > **новый файл...** . Введите `AttractionInformation` для **имени** и нажмите кнопку **New (создать** ): 

[![](table-views-images/data01.png "Enter AttractionInformation for the Name")](table-views-images/data01.png#lightbox)

# <a name="visual-studiotabwindows"></a>[Visual Studio](#tab/windows)

Чтобы создать модель данных для **притяжения**, щелкните правой кнопкой мыши имя проекта в **Обозреватель решений** и выберите **добавить** > **новый элемент...** . Выберите **класс** и введите `AttractionInformation` для **имени** и нажмите кнопку **добавить** : 
...
Измените файл `AttractionInformation.cs` и сделайте его следующим:
...
Затем щелкните правой кнопкой мыши имя проекта в **панель решения** и выберите команду **Добавить** > **новый файл...** . Введите `CityInformation` для **имени** и нажмите кнопку **New (создать** ): 

[![](table-views-images/data02.png "Enter CityInformation for the Name")](table-views-images/data02.png#lightbox)

# <a name="visual-studiotabwindows"></a>[Visual Studio](#tab/windows)

Затем щелкните правой кнопкой мыши имя проекта в **Обозреватель решений** и выберите **Добавить** > **новый элемент...** . Введите `CityInformation` **имени** и нажмите кнопку **Добавить** : 

[![](table-views-images/data02-vs.png "Enter CityInformation for the Name")](table-views-images/data02-vs.png#lightbox)

-----

Измените файл `CityInformation.cs` и сделайте его следующим:
...
Этот класс содержит все сведения о **городе**назначения, коллекцию **аттрактионс** для этого города и предоставляет два вспомогательных метода (`AddAttraction`), облегчающих добавление аттрактионс в город.

<a name="The-Table-Data-Source" />

## <a name="the-table-view-data-source"></a>Источник данных табличного представления

Для каждого табличного представления требуется источник данных (`UITableViewDataSource`), чтобы предоставить данные для таблицы и создать необходимые строки в соответствии с требованиями табличного представления.

В приведенном выше примере щелкните правой кнопкой мыши имя проекта в **Обозреватель решений**, выберите **Добавить** > **новый файл...** и вызовите его `AttractionTableDatasource` и нажмите кнопку **создать** . Затем измените файл `AttractionTableDatasource.cs` и сделайте его следующим:
...
Например, метод `PopulateCities` просто создает объекты модели данных в памяти, однако их можно легко прочитать из базы данных или веб-службы в реальном приложении:
...
Метод `RowsInSection` возвращает число строк в текущем разделе:
...
Для **простого** типа табличного представления оставьте поле заголовка пустым (`""`).

Наконец, по запросу табличного представления создайте и заполните ячейку прототипа с помощью метода `GetCell`: 
...
Для каждого табличного представления требуется делегат (`UITableViewDelegate`), отвечающий на взаимодействие с пользователем или другие системные события в таблице.

В приведенном выше примере щелкните правой кнопкой мыши имя проекта в **Обозреватель решений**, выберите **Добавить** > **новый файл...** и вызовите его `AttractionTableDelegate` и нажмите кнопку **создать** . Затем измените файл `AttractionTableDelegate.cs` и сделайте его следующим:
...
Метод `CanFocusRow` вызывается для каждой строки, которая собирается получить фокус в представлении таблицы. Возвращает `true`, если строка может получить фокус, иначе возвращается `false`. В этом примере мы создали пользовательское событие `AttractionHighlighted`, которое будет создаваться в каждой строке по мере получения фокуса.
...
Для примера приложения дважды щелкните файл `AttractionTableCell.cs`, чтобы открыть его для редактирования, и сделайте его следующим:
...
Этот класс предоставляет хранилище для объекта модели данных притяжения (`AttractionInformation`, как определено выше), отображаемое в данной строке:
...
Для примера приложения дважды щелкните файл `AttractionTableViewController.cs`, чтобы открыть его для редактирования, и сделайте его следующим:
...
Давайте подробнее рассмотрим этот класс. Сначала мы создали ярлыки, чтобы упростить доступ к `DataSource` и `TableDelegate`табличного представления. Эти данные будут использоваться позже для взаимодействия между табличным представлением в левой части разделенного представления и подробным представлением сведений в правой части.

Наконец, когда табличное представление загружается в память, создаются экземпляры `AttractionTableDatasource` и `AttractionTableDelegate` (созданные выше) и присоединяются их к табличному представлению.
...
В примере приложения для командировок, представленного выше, Пользовательский класс (`AttractionViewController`) определяется для стандартного контроллера представления, представленного в правой части разделенного представления в виде подробного представления:
...
Здесь мы указали **притяжения** (`AttractionInformation`), отображаемые как свойство, и создали метод `UpdateUI`, который заполняет мини-приложения пользовательского интерфейса, добавленные в представление в конструкторе интерфейсов.
...
Наконец, в три экземпляра `UIButton`, созданные в конструкторе интерфейсов, были добавлены пользовательские действия (события), позволяющие пользователю помечать притяжения в качестве _избранного_, получать _направления_ к притяжения и _Book рейс_ в заданный город.

<a name="The-Navigation-View-Controller" />

### <a name="the-navigation-view-controller"></a>Контроллер представления навигации

Поскольку контроллер представления таблицы вложен в контроллер представления навигации в левой части разделенного представления, контроллеру представления навигации был назначен пользовательский класс (`MasterNavigationController`) в конструкторе интерфейсов и он определяется следующим образом:
...
- `SplitView` — это ссылка на контроллер разделенного представления (`MainSpiltViewController`), к которому относится контроллер представления навигации.
...
Во-первых, мы создаем ярлыки для области **сведений** в разделенном представлении (`AttractionViewController`) и на стороне **главного** (`MasterNavigationController`). Опять же, это упрощает обмен данными между двумя сторонами позже.

Затем, когда представление с разделением загружается в память, мы подключим контроллер разделенного представления к обеим сторонам разделенного представления и реагируем на пользователя, выделяющий притяжения в табличном представлении (`AttractionHighlighted`), отображая новый притяжения в части **сведений** Представление с разделением.
