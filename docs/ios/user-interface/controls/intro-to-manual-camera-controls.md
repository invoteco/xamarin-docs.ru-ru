...
Независимо от того, используете ли видеоизображение или изображение на устройстве iOS, процесс, используемый для записи этих образов, практически одинаков. Это справедливо для приложений, использующих автоматизированные элементы управления камерой по умолчанию или использующие преимущества новых ручных элементов управления камеры:
 ...
 [![](intro-to-manual-camera-controls-images/image1.png "Общие сведения об объектах отслеживания Авфаундатион")](intro-to-manual-camera-controls-images/image1.png#lightbox)

Входные данные берутся `AVCaptureDeviceInput` из объекта `AVCaptureSession` в объект с помощью `AVCaptureConnection`. Результат выводится как изображение по-прежнему или как поток видео. Весь процесс управляется `AVCaptureDevice`.
...
Сеанс записи вирусов-кадров используется для управления записью в реальном времени с камеры устройства iOS и требуется для получения видео в приложение iOS. Поскольку пример приложения использует сеанс записи в нескольких разных местах, он настраивается `AppDelegate` в и становится доступным для всего приложения. `ManualCameraControl`
...
[![](intro-to-manual-camera-controls-images/image2.png "Бокеный результат")](intro-to-manual-camera-controls-images/image2.png#lightbox)

Или создайте фокус на [броскую фокусировку](http://www.mediacollege.com/video/camera/focus/pull.html), например:

[![](intro-to-manual-camera-controls-images/image3.png "Побочный результат фокуса")](intro-to-manual-camera-controls-images/image3.png#lightbox)
...
iOS 7 и более ранние версии предоставили существующие элементы `FocusMode`управления фокусом через свойство как:
...
Существующие элементы управления также предоставили настраиваемую точку интересов через`FocusPointOfInterest` свойство, чтобы пользователь мог сосредоточиться на определенной области. Приложение также может отслеживать перемещение линзы, `IsAdjustingFocus` отслеживая свойство.

Кроме того `AutoFocusRangeRestriction` , ограничение диапазона было предоставлено свойством следующим образом:
...
Используя общий код настройки записи вирусов, можно добавить в раскадровку приложения и настроить его `UIViewController` следующим образом:

[![](intro-to-manual-camera-controls-images/image5.png "UIViewController можно добавить в раскадровку приложений и настроить, как показано здесь.")](intro-to-manual-camera-controls-images/image5.png#lightbox)
...
1. Коснитесь заблокированного сегмента и перетащите ползунок положение, чтобы изменить положение линзы вручную:
    ...
    [![](intro-to-manual-camera-controls-images/image7.png "Настройка расположения линзы вручную")](intro-to-manual-camera-controls-images/image7.png#lightbox)
...
[![](intro-to-manual-camera-controls-images/image8.png "Пример изображения, показывающего раскрытие от нереалистичного, темно-светлого и Муди")](intro-to-manual-camera-controls-images/image8.png#lightbox)
...
[![](intro-to-manual-camera-controls-images/image9.png "Как работает экспозиция")](intro-to-manual-camera-controls-images/image9.png#lightbox)
...
[![](intro-to-manual-camera-controls-images/image10.png "Как работает постоянная автоматическая экспозиция на устройстве iOS")](intro-to-manual-camera-controls-images/image10.png#lightbox)
...
[![](intro-to-manual-camera-controls-images/image11.png "Как работает заблокированная экспозиция на устройствах iOS")](intro-to-manual-camera-controls-images/image11.png#lightbox)
...
iOS 7 и более поздних версий, предоставьте следующие существующие элементы управления `ExposureMode` экспозицией через свойство:
...
Можно использовать, чтобы открыть сцену, выбрав целевой объект для предоставления, и приложение может `AdjustingExposure` отслеживать свойство, чтобы узнать, когда настраивается раскрытие. `ExposurePointOfInterest`
...
Для реализации описанных выше функций был добавлен `AVCaptureExposureModeCustom` новый режим. Если камера находится в пользовательском режиме, можно использовать следующий код для настройки длительности экспозиции и ISO:
...
[![](intro-to-manual-camera-controls-images/image12.png "UIViewController можно добавить в раскадровку приложений и настроить, как показано здесь.")](intro-to-manual-camera-controls-images/image12.png#lightbox)
...
1. Коснитесь заблокированного сегмента и перетащите ползунок сдвига, чтобы настроить сдвиг автоматической экспозиции вручную:
    ...
    [![](intro-to-manual-camera-controls-images/image14.png "Настройка смещения автоматической экспозиции вручную")](intro-to-manual-camera-controls-images/image14.png#lightbox)
1. Коснитесь пользовательского сегмента и перетащите ползунки длительность и ISO, чтобы вручную контролировать раскрытие:

    [![](intro-to-manual-camera-controls-images/image15.png "Перетащите ползунки длительность и ISO, чтобы вручную управлять выдержкой")](intro-to-manual-camera-controls-images/image15.png#lightbox)
1. Завершите работу приложения.
...
[![](intro-to-manual-camera-controls-images/image16.png "Пример изображения, в котором показаны корректировки белого баланса вручную")](intro-to-manual-camera-controls-images/image16.png#lightbox)
...
[![](intro-to-manual-camera-controls-images/image17.png "Цветовое пространство RGB ЦИЕ 1931 и цветовое пространство ЦИЕ 1931 XYZ")](intro-to-manual-camera-controls-images/image17.png#lightbox)
...
в iOS 7 и выше предоставлены следующие существующие элементы управления " `WhiteBalanceMode` белый баланс" через свойство:
...
И приложение может отслеживать `AdjustingWhiteBalance` свойство, чтобы увидеть, когда раскрытие регулируется.
...
В iOS 8 добавлены подпрограммы преобразования, помогающие выполнять преобразование в ненезависимые от устройства цветовые пространства. Для реализации подпрограмм `AVCaptureWhiteBalanceChromaticityValues` преобразования структура добавлена со следующими элементами:

-   `X`— значение от 0 до 1.
-   `Y`— значение от 0 до 1.


Также `AVCaptureWhiteBalanceTemperatureAndTintValues` Добавлена структура со следующими элементами:
...
`CaptureDevice.GetTemperatureAndTintValues`Используйте методы`CaptureDevice.GetDeviceWhiteBalanceGains`и для преобразования между цветовыми пространствами температуры и оттенками, цветностью и усилением RGB.
...
Затем приложение будет блокировать прибыль баланса белого для метода, `SetWhiteBalanceModeLockedWithDeviceWhiteBalanceGains` используя значения `GrayWorldDeviceWhiteBalanceGains` из свойства для применения изменений.
...
[![](intro-to-manual-camera-controls-images/image18.png "UIViewController можно добавить в раскадровку приложений и настроить, как показано здесь.")](intro-to-manual-camera-controls-images/image18.png#lightbox)
...
1. При использовании камеры в режиме Auto ползунки автоматически перемещаются, когда камера корректирует баланс белого:
    ...
    [![](intro-to-manual-camera-controls-images/image19.png "Ползунки будут автоматически перемещаться по мере регулировки белого баланса.")](intro-to-manual-camera-controls-images/image19.png#lightbox)
1. Коснитесь заблокированного сегмента и перетащите ползунки временные и оттенки, чтобы изменить баланс белого вручную:

    [![](intro-to-manual-camera-controls-images/image20.png "Перетащите ползунки \"временные и оттенки\", чтобы изменить баланс белого вручную")](intro-to-manual-camera-controls-images/image20.png#lightbox)
1. Выбрав заблокированный сегмент, поместите физическую серую карту перед камерой и коснитесь кнопки с серой картой, чтобы настроить баланс белого цвета на серый мир:
    ...
    [![](intro-to-manual-camera-controls-images/image21.png "Нажмите кнопку с серым картой, чтобы настроить баланс белого цвета на серый мир")](intro-to-manual-camera-controls-images/image21.png#lightbox)
1. Завершите работу приложения.
...
[![](intro-to-manual-camera-controls-images/image22.png "Как работает захват в квадратных скобках")](intro-to-manual-camera-controls-images/image22.png#lightbox)
...
Все команды записи в квадратных скобках реализуются в `AVCaptureStillImageOutput` классе. `CaptureStillImageBracket`Используйте метод, чтобы получить ряд изображений с заданным массивом параметров.
...
-  Параметры в `AVCaptureDevice` квадратных скобках временно переопределяют параметры.
...
[![](intro-to-manual-camera-controls-images/image23.png "UIViewController можно добавить в раскадровку приложений и настроить, как показано здесь.")](intro-to-manual-camera-controls-images/image23.png#lightbox)
...
1. Кадрировать сцену и нажать кнопку "записать скобку":
    ...
    [![](intro-to-manual-camera-controls-images/image24.png "Кадрировать сцену и нажать кнопку \"записать скобку\"")](intro-to-manual-camera-controls-images/image24.png#lightbox)
1. Проведите вправо влево, чтобы увидеть три изображения, взятые с помощью записи в квадратных скобках:

    [![](intro-to-manual-camera-controls-images/image25.png "Проведите вправо влево, чтобы увидеть три изображения, взятые с записью в квадратных скобках")](intro-to-manual-camera-controls-images/image25.png#lightbox)
1. Завершите работу приложения.
...
- [ManualCameraControls (образец)](https://docs.microsoft.com/samples/xamarin/ios-samples/manualcameracontrols)
