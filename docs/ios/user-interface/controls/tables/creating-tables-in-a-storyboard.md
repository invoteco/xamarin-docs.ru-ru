...
При настройке свойств представления таблицы в конструкторе iOS существует два типа содержимого ячеек, которые можно выбрать: **динамическое** или **статическое** содержимое прототипа.

<a name="Prototype_Content" />

## <a name="dynamic-prototype-content"></a>Динамическое содержимое прототипа

`UITableView` с содержимым прототипа обычно предназначено для вывода списка данных, в которых ячейка прототипа (или ячейки, как можно определить более одного) используется повторно для каждого элемента в списке. Не требуется создавать экземпляры ячеек, они получаются в методе `GetView` путем вызова метода `DequeueReusableCell` `UITableViewSource`.

 <a name="Static_Content" />

## <a name="static-content"></a>Статическое содержимое

`UITableView`с статическим содержимым позволяет разрабатывать таблицы прямо в области конструктора. Ячейки можно перетаскивать в таблицу и настраивать, изменяя свойства и добавляя элементы управления.
...
Решение откроется с некоторыми C# файлами, а файл`Main.storyboard`уже создан. Дважды щелкните файл `Main.storyboard`, чтобы открыть его в конструкторе iOS.
...
4. Выберите новый перехода, который вы создали, и присвойте ему идентификатор для ссылки на этот перехода в коде. Щелкните перехода и введите `TaskSegue` для **идентификатора** в **панель свойств**следующим образом:    
  [![именования перехода на панели свойств](creating-tables-in-a-storyboard-images/image16a-sml.png)](creating-tables-in-a-storyboard-images/image16a.png#lightbox) 
...
6. Измените корневой контроллер представления на **содержимое: динамические прототипы** (представление область конструктора будет помечено как **содержимое прототипа** ):

    [![присвоения свойству содержимого динамических прототипов](creating-tables-in-a-storyboard-images/image17a.png)](creating-tables-in-a-storyboard-images/image17a.png#lightbox)

7. Измените новый **уитаблевиевконтроллер** на " **содержимое": статические ячейки**. 

8. Для нового Уитаблевиевконтроллер необходимо указать имя класса и идентификатор. Выберите контроллер представления и введите _таскдетаилвиевконтроллер_ для **класса** в **панель свойств** — в панель решения будет создан новый файл `TaskDetailViewController.cs`. Введите **сторибоардид** в качестве _подробностей_, как показано в примере ниже. Он будет использоваться позже для загрузки этого представления в C# коде:  

    [![установки идентификатора раскадровки](creating-tables-in-a-storyboard-images/image18a.png)](creating-tables-in-a-storyboard-images/image18a.png#lightbox)
...
Сначала выберите ячейку прототипа в контроллере главного представления и задайте **идентификатор** как _таскцелл_, как показано ниже. Он будет использоваться позже в коде для получения экземпляра этого Уитаблевиевцелл:
 ...
 [![установки идентификатора ячейки](creating-tables-in-a-storyboard-images/image22a-sml.png)](creating-tables-in-a-storyboard-images/image22a.png#lightbox)

Далее необходимо создать кнопку, которая будет добавлять новые задачи, как показано ниже:

[элемент кнопки панели![на панели навигации](creating-tables-in-a-storyboard-images/image23-sml.png)](creating-tables-in-a-storyboard-images/image23.png#lightbox)

Выполните следующие действия: 

- Перетащите **элемент панели** инструментов с панели элементов в правой части _панели навигации_.
- В **панель свойств**в разделе **элемент кнопки на панели** выберите **идентификатор: добавить** (чтобы сделать его *+* и кнопкой). 
...
Подробное представление требует гораздо больше работы. Ячейки табличного представления должны быть перенесены в представление и затем заполнены метками, текстовыми представлениями и кнопками. На снимке экрана ниже показан готовый пользовательский интерфейс с двумя разделами. Один раздел содержит три ячейки, три метки, два текстовых поля и один переключатель, а второй раздел содержит одну ячейку с двумя кнопками:
 ...
 [макет представления подробностей![](creating-tables-in-a-storyboard-images/image24a-sml.png)](creating-tables-in-a-storyboard-images/image24a.png#lightbox)
...
- **Разделы**: _2_ 
- **Стиль**: _сгруппировано_
- **Разделитель**: _нет_
- **Выделенный фрагмент**: _нет выделенных элементов_

Выберите верхний раздел и в разделе **свойства > раздел представление таблицы** измените **строки** на _3_, как показано ниже:

 [![установка в верхней части трех строк](creating-tables-in-a-storyboard-images/image29-sml.png)](creating-tables-in-a-storyboard-images/image29.png#lightbox)
...
- Перетащите две кнопки в ячейку и задайте соответствующие названия (например, _сохранить_ и _Удалить_), как показано ниже:
   ...
   [![установки двух кнопок в нижнем разделе](creating-tables-in-a-storyboard-images/image30-sml.png)](creating-tables-in-a-storyboard-images/image30.png#lightbox)
...
- **Title текстовое поле uitextfield** : _титлетекст_
...
Сначала мы хотим создать класс `Chores`, который предоставит способ получения и задания значения ID, Name, Notes и Boolean, чтобы мы могли использовать эти значения в приложении.

В классе `Chores` добавьте следующий код:
...
Затем создайте класс `RootTableSource`, который наследует от `UITableViewSource`. 

Различие между этим и раскадровым представлением заключается в том, что методу `GetView` не нужно создавать экземпляры ячеек — `theDequeueReusableCell` метод всегда возвращает экземпляр ячейки прототипа (с совпадающим идентификатором).

Приведенный ниже код относится к файлу `RootTableSource.cs`:
...
Чтобы использовать класс `RootTableSource`, создайте новую коллекцию в конструкторе `ItemViewController`:
...
В `ViewWillAppear` передайте коллекцию в источник и присвойте ей представление таблицы:
...
Чтобы отправить параметр в перехода, переопределите метод `PrepareForSegue` и задайте свойства `DestinationViewController` (`TaskDetailViewController` в этом примере). Экземпляр класса целевого контроллера представления будет создан, но еще не отображается для пользователя. Это означает, что вы можете задать свойства класса, но не изменять элементы управления пользовательского интерфейса:
...
В `TaskDetailViewController` метод `SetTask` назначает свои параметры свойствам, чтобы на них можно было ссылаться в Виеввиллаппеар. Свойства элемента управления не могут быть изменены в `SetTask`, так как при вызове `PrepareForSegue` могут отсутствовать:
...
Далее необходимо добавить обработчик событий `TouchUpInside` кнопки в метод `ViewDidLoad` **TaskDetailViewController.CS**. Ссылка на свойство `Delegate` для `ItemViewController` была создана специально, поэтому мы можем вызвать `SaveTask` и `DeleteTask`, которые закрывают это представление как часть своей операции:
...
Наконец, Назовите кнопку на панели навигации в методе `ViewDidLoad` **ItemViewController.CS**, чтобы вызвать его:
...
[приложение![завершено](creating-tables-in-a-storyboard-images/image28a.png)](creating-tables-in-a-storyboard-images/image28a.png#lightbox)
...
- Создание перехода и переопределение метода `PrepareForSegue` для уведомления целевого представления о необходимых параметрах. 
- Загрузка представлений раскадровки напрямую с помощью метода `Storyboard.InstantiateViewController`.
