...
- Переключатель в **режиме ограничений**. ранее вы перешли в режим ограничений, щелкнув снова в выбранном представлении в области конструктора. Теперь этот переключатель следует использовать на панели ограничений:
...
- Во всех представлениях этот проход проходит **Обновление** , обработчик автоматической разметки проходит по иерархии представления и вызывает метод `UpdateViewConstraints` на всех контроллерах представления и методе `UpdateConstraints`.
- **Передаваемый макет** , обработчик автоматической разметки проходит по иерархии представлений, но в этот раз вызывает метод `ViewWillLayoutSubviews` для всех контроллеров представления и метод `LayoutSubviews` для всех представлений. Метод `LayoutSubviews` обновляет свойство `Frame` каждого подпредставления с прямоугольником, вычисленным обработчиком автоматической разметки.
...
Ключ здесь вызывает метод `LayoutIfNeeded` родительского представления внутри блока анимации. Это означает, что представление будет рисовать каждый «кадр» анимированного расположения или изменения размера. Без этой строки представление будет просто привязано к окончательной версии без анимации.
