...
- Когда приложение включает сложности, оно будет представлено в коллекции Apple Watch лиц.
...
Новое свойство `HasContentPending` класса [вксессион](https://developer.apple.com/reference/watchconnectivity/wcsession) указывает, что сеанс получил данные в фоновом режиме, который должен быть обработан. А свойство `RemainingComplicationUserInfoTransfers` возвращает оставшееся время, в течение которого приложение iOS может обновить свою watchOS сложность.
...
- Класс [вкекстенсион](https://developer.apple.com/reference/watchkit/wkextension) теперь включает метод `ApplicationState` и константу [вкаппликатионстате](https://developer.apple.com/reference/watchkit/wkapplicationstate) , которую приложение может использовать для отслеживания состояния среды выполнения приложения. `WKExtension` также предоставляет два новых метода, которые можно использовать для планирования фоновых задач.
- [Вкекстенсионделегате](https://developer.apple.com/reference/watchkit/wkextensiondelegate) теперь включает новые методы `ApplicationWillEnterForeground`, `ApplicationDidEnterBackground` и `HandleBackgroundTasks` для отслеживания изменений состояния приложения и управления обновлениями фоновых задач.
...
- Класс `UILocalNotification` UIKit является устаревшим и должен быть заменен платформой уведомлений пользователя.
