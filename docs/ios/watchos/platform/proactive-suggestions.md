...
- **Контекстные напоминания Siri** . в iOS 10 `NSUserActivity` была расширена, чтобы позволить Siri быстро создать напоминание для просмотра содержимого, которое они сейчас просматривают в приложении позже.
...
Убедитесь, что это тот же идентификатор типа действия (`com.xamarin.platform`), что и созданное выше действие. Приложение использует сведения, хранящиеся в `NSUserActivity`, для восстановления состояния, в котором пользователь вышел из системы.
...
3. Это даже работает для сторонних приложений (которые поддерживают `NSUserActivity`), поэтому пользователь может переключиться на приложение с общим доступом, и адрес ресторана автоматически предлагается в качестве назначения.
