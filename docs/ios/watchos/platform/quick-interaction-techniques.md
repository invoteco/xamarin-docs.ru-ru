...
Все дискретные жесты запускаются в `Possible` состоянии и переходят в состояние `Failed` или `Recognized`. При использовании дискретных жестов разработчик обычно не работает напрямую с состоянием. Вместо этого они используют действие, вызываемое, когда жест распознается только.
...
Кроме того, непрерывные жесты запускаются в `Possible` состоянии, но они выполняются над несколькими обновлениями. В этом случае разработчику необходимо принять во внимание состояние распознавателя и обновить пользовательский интерфейс приложения на этапе `Changed`, пока жест не `Recognized` или не `Canceled`.
...
Начиная с watchOS 2, Apple Watch приложение может использовать объект `WKInterfacePicker` для доступа к Digital Crown, предоставляя список `WKPickerItems` и стиль средства выбора (список, стек или последовательность изображений). затем watchOS разрешает пользователю использовать Digital Crown для выбора элемента из списка.
...
Доступ к поворотам в секунду из Digital Crown полезен при работе с анимациями на основе физических объектов. Чтобы получить доступ к поворотам в секунду, используйте свойство `CrownSequencer` `WKInterfaceController` расширения Watch. Пример:
...
Для подсчета числа поворотов используйте поворотные разницы из Digital Crown. Используйте метод переопределения `CrownDidRotate` `WKCrownDelegate` для доступа к сдвигам. Пример:
...
Здесь приложение поддерживает накапливаемое (`AccumulatedRotations`), чтобы определить число поворотов. Один полный поворот Digital Crown равен накопленной Дельта `1.0` и половина поворота будет `0.5`.
...
Знак (`+/-`) сдвиговой разницы указывает направление, в котором пользователь переключается Digital Crown:
...
Так как watchOS 3 использует ту же платформу API `UINotification`, что и iOS 10, дополнительные сведения см. в документации по [пользовательской инфраструктуре уведомлений](~/ios/platform/user-notifications/index.md) iOS 10.
...
- `WKInterfaceSKScene` — для работы с двухмерной графикой SpriteKit.
...
С этого момента работа с сценой SpriteKit или SceneKit работает так же, как и в приложении iOS. Приложение Watch будет представлять `WKInterfaceSKScene`, вызвав один из методов `Present`. Для SceneKit просто задайте свойство `Scene` объекта `WKInterfaceSCNScene`.
...
Для приложений, выполняющих сетевые подключения, они могут использовать фоновые `NSURLSession` для управления связью по сети с задачей скачивания. Это позволит приложению быть пробуждении в фоновом режиме для обработки загруженных данных. Для приложения, требующего фоновой обработки, используйте утверждение фоновой задачи для обработки необходимой обработки.
