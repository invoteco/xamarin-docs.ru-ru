...
1. В **Обозреватель решений**дважды щелкните вспомогательный файл `Info.plist` приложения iPhone для расширения контрольных значений, чтобы открыть его для редактирования.
...
3. Добавьте новый раздел с именем `WKBackgroundModes` и задайте для **типа** значение `Array`: 

    [![](workout-apps-images/plist02.png "Add a new key called WKBackgroundModes")](workout-apps-images/plist02.png#lightbox)
4. Добавьте новый элемент в массив с **типом** `String` и значением `workout-processing`: 
...
Сеансы тренировки создаются с помощью объекта настройки тренировки (`HKWorkoutConfiguration`), который указывает тип тренировки (например, `HKWorkoutActivityType.Running`) и место тренировки (например, `HKWorkoutSessionLocationType.Outdoor`):
...
Чтобы обрабатывались события, которые могут возникнуть во время сеанса тренировки, приложению потребуется создать экземпляр делегата для сеанса тренировки. Добавьте в проект новый класс и отключить его от класса `HKWorkoutSessionDelegate`. Для примера внешнего запуска он может выглядеть следующим образом:
...
Он создает предикат для задания начальной даты, которую требуется получить для получения данных с помощью метода `GetPredicateForSamples`. Он создает набор устройств для извлечения HealthKit данных из метода `GetPredicateForObjectsFromDevices`, в данном случае только для локального Apple Watch (`HKDevice.LocalDevice`). Два предиката объединяются в составной предикат (`NSCompoundPredicate`) с помощью метода `CreateAndPredicate`.

Для нужной точки данных создается новый `HKAnchoredObjectQuery` (в данном случае `HKQuantityTypeIdentifier.ActiveEnergyBurned` для активной точки данных, записанной в энергопотреблении) ограничения на объем возвращаемых данных (`HKSampleQuery.NoLimit`) не накладываются, а для обработки данных, возвращаемых приложению из HealthKit, определен обработчик обновления. 
...
Код выполняет циклическое переключение всех примеров (`HKSample`), возвращаемых в массиве `addedObjects`, и приводит их к примеру Quantity (`HKQuantitySample`). Затем он получает двойное значение примера как жауле (`HKUnit.Joule`) и накапливает его в суммарный объем активной энергии, записываемой в ходе тренировки, и обновляет пользовательский интерфейс.
...
Чтобы наблюдать за событиями, создаваемыми HealthKit, приложение переопределит метод `DidGenerateEvent` `HKWorkoutSessionDelegate`:
...
- `HKWorkoutEventType.Lap` — предназначены для событий, которые нарушают тренировку на одинаковые расстояния. Например, для пометки на одну запись во время выполнения.
...
Этот код создает новый экземпляр события маркера (`HKWorkoutEvent`) и сохраняет его в закрытой коллекции событий (которая позже будет записана в сеанс тренировки) и оповещает пользователя о событии через хаптикс.
...
События приостановки и возобновления, которые будут созданы из HealthKit, могут быть обработаны путем переопределения метода `DidGenerateEvent` `HKWorkoutSessionDelegate`:
...
Кроме того, в watchOS 3 — это события приостановки движения (`HKWorkoutEventType.MotionPaused`) и перемещения (`HKWorkoutEventType.MotionResumed`). Эти события автоматически создаются HealthKit во время выполнения тренировки, когда пользователь запускает и прекращает перемещение.
...
> [!IMPORTANT]
> События "пауза перемещения" и "возобновление движения" поддерживаются только для типа действия Руннингворкаут (`HKWorkoutActivityType.Running`).

Эти события можно обработать, переопределив метод `DidGenerateEvent` `HKWorkoutSessionDelegate`:
...
Чтобы завершить сеанс тренировки, вызовите метод `EndWorkoutSession` `HKHealthStore`, передав `HKWorkoutSession`:
...
Это приведет к сбросу датчиков устройств в нормальный режим. Когда HealthKit завершает тренировку, он получит обратный вызов метода `DidChangeToState` `HKWorkoutSessionDelegate`:
...
Когда приложение завершит сеанс тренировки, необходимо будет создать тренировку (`HKWorkout`) и сохранить ее (вместе с событиями) в хранилище данных HealthKit (`HKHealthStore`):
...
Этот код создает общий объем записываемой энергии и расстояние для тренировки в качестве `HKQuantity` объектов. Будет создан словарь метаданных, определяющих создание тренировок, и указано расположение тренировки:

```csharp
metadata.Add (new NSString ("HKMetadataKeyIndoorWorkout"), new NSString ("NO"));
```

Новый объект `HKWorkout` создается с тем же `HKWorkoutActivityType`, что и `HKWorkoutSession`, даты начала и окончания, список событий (накапливается из приведенных выше разделов), число записанных в энергию, общее расстояние и словарь метаданных. Этот объект сохраняется в хранилище данных о работоспособности и обрабатываются все ошибки.  
...
Каждое приложение тренировки watchOS 3 имеет родительское приложение для работы с iOS 10 и, что новое для iOS 10, это приложение iOS можно использовать для запуска тренировки, который будет размещать Apple Watch в режиме тренировки (без вмешательства пользователя) и запускать приложение watchOS в фоновом режиме выполнения [(см. для получения дополнительных сведений используется фоновый рисунок грамме](#about-background-running) выше).
...
1. Приложение iPhone создает объект `HKWorkoutConfiguration` и задает тип тренировки и расположение.
2. Объект `HKWorkoutConfiguration` отправляет Apple Watch версию приложения и, если она еще не запущена, запускается системой.
...
Затем он создает `HKWorkoutConfiguration` как обычно и использует метод `StartWatchApp` `HKHealthStore`, чтобы отправить его в Apple Watch и запустить приложение и сеанс тренировки.
...
Он принимает `HKWorkoutConfiguration` и создает новый `HKWorkoutSession` и присоединяет экземпляр настраиваемого `HKWorkoutSessionDelegate`. Сеанс тренировки начинается с хранилища данных о работоспособности HealthKit пользователя.
...
2. **watchOS 3 `ExtensionDelegate.cs`** — обрабатывает версию watchOS 3 для приложения тренировки.
3. **watchOS 3 `OutdoorRunDelegate.cs`** — настраиваемый `HKWorkoutSessionDelegate` для управления событиями тренировки.
...
Файл `ViewController.cs` в родительской версии iOS 10 приложения тренировки будет включать следующий код:
...
Файл `ExtensionDelegate.cs` в версии watchOS 3 приложения тренировки будет включать следующий код:
...
Файл `OutdoorRunDelegate.cs` в версии watchOS 3 приложения тренировки будет включать следующий код:
