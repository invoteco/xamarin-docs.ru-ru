...
Любые изменения, внесенные в свойства **Name**, **профессии** или **Manage** , будут отправляться в базу данных, если данные были сохранены ранее (например, если `_conn`ая переменная не `null`). Теперь рассмотрим методы, которые мы добавили для **создания**, **обновления**, **загрузки** и **удаления** пользователей из базы данных.
...
Позже мы задали значения для параметров с помощью метода `Parameters.AddWithValue` на `SQLiteCommand`. С помощью параметров мы гарантируете, что значения (такие как одинарные кавычки) будут правильно кодироваться перед отправкой в SQLite. Пример

```csharp
command.Parameters.AddWithValue ("@COL1", ID);
```

Наконец, поскольку пользователь может быть менеджером и иметь в нем коллекцию сотрудников, мы рекурсивно вызываем метод `Create` для этих пользователей, чтобы сохранить их в базе данных:
...
Как и в случае с **CREATE** , мы получаем `SQLiteCommand` из переданного `SQLiteConnection` и настроили наш SQL для обновления нашей записи (предоставляя параметры):

```csharp
command.CommandText = "UPDATE [People] SET Name = @COL2, Occupation = @COL3, isManager = @COL4, ManagerID = @COL5 WHERE ID = @COL1";
```

Мы заполняем значения параметров (например: `command.Parameters.AddWithValue ("@COL1", ID);`) и опять, рекурсивно вызываем обновление для всех дочерних записей:
...
Наконец, мы используем модуль чтения данных для выполнения запроса и возвращения полей записей (которые копируются в экземпляр класса `PersonModel`):
...
Если этот человек является руководителем, необходимо также загрузить всех своих сотрудников (опять же, путем рекурсивного вызова метода `Load`):
...
После удаления записи мы очищаете текущий экземпляр класса `PersonModel`:
...
Здесь мы используем перегрузку конструктора для класса `PersonModel`, чтобы загрузить пользователя в память:

```csharp
var person = new PersonModel (_conn, childID);
```

Мы также вызываем класс привязки данных, чтобы добавить человека в коллекцию людей `AddPerson (person)`. Это гарантирует, что наш пользовательский интерфейс распознает изменение и отобразит его:
...
Единственное вещественное отличие в инструкции SQL (которая загружает только менеджеры `command.CommandText = "SELECT ID FROM [People] WHERE isManager = 1"`), но работает так же, как в предыдущем разделе.
...
В этом примере мы создаем новый `NSComboBoxDataSource`, который может представлять элементы поля со списком из любого источника данных SQLite. Сначала определим следующие свойства:
...
- `RecordCount` — возвращает количество записей в данной таблице.
...
Метод `GetRecordCount` возвращает количество записей в данной таблице:
...
Он вызывается при каждом изменении значения свойства `TableName`, `IDField` или `DisplayField`.

Метод `IDForIndex` Возвращает уникальный идентификатор (`IDField`) для записи по заданному индексу элемента раскрывающегося списка: 
...
Метод `ValueForIndex` возвращает значение (`DisplayField`) для элемента в заданном индексе раскрывающегося списка:
...
Метод `IDForValue` Возвращает уникальный идентификатор (`IDField`) для заданного значения (`DisplayField`):
...
`ItemCount` возвращает предварительно вычисленное количество элементов в списке, вычисленное при изменении свойств `TableName`, `IDField` или `DisplayField`:
...
Метод `ObjectValueForItem` предоставляет значение (`DisplayField`) для данного индекса элемента раскрывающегося списка:
...
Обратите внимание, что мы используем операторы `LIMIT` и `OFFSET` в команде SQLite, чтобы ограничить одну запись, которую нам требуется.

Метод `IndexOfItem` возвращает индекс элемента раскрывающегося списка для заданного значения (`DisplayField`):
...
Если значение не найдено, возвращается значение `NSRange.NotFound` и все элементы отменяются в раскрывающемся списке.

Метод `CompletedString` возвращает первое совпадающее значение (`DisplayField`) для частично типизированной записи:
...
Чтобы объединить все элементы, измените `SubviewSimpleBindingController` и сделайте его следующим:
...
Свойство `DataSource` предоставляет ярлык `ComboBoxDataSource` (созданного выше), присоединенного к полю со списком.

Метод `LoadSelectedPerson` загружает пользователя из базы данных для заданного уникального идентификатора:
...
В переопределении метода `AwakeFromNib` сначала мы присоединяем экземпляр пользовательского источника данных поля со списком:

```csharp
EmployeeSelector.DataSource = new ComboBoxDataSource (Conn, "People", "Name");
```

Далее мы реагируем на то, что пользователь редактирует текстовое значение поля со списком, находя соответствующий уникальный идентификатор (`IDField`) представления данных и загружает данного пользователя, если он найден:
...
Как упоминалось выше, с помощью диспетчера связей объектов [SQLite.NET](https://www.sqlite.org) (ORM) с открытым исходным кодом можно значительно сократить объем кода, необходимого для чтения и записи данных из базы данных SQLite. Это может быть не лучший маршрут, который следует предпринять при связывании данных, из-за ряда требований, которые задаются с помощью кодирования и привязки данных для объекта.

В соответствии с веб-сайтом SQLite.Net _"SQLite — это библиотека программного обеспечения, которая реализует автономное ядро СУБД SQL без сервера без конфигурации. SQLite — это наиболее широко развернутое ядро СУБД в мире. Исходный код для SQLite находится в общедоступном домене "._
...
Добавим в проект новый класс и вызовите его в `OccupationModel`. Теперь измените файл **OccupationModel.CS** и сделайте его следующим:
...
Во-первых, мы включаем SQLite.NET (`using Sqlite`), а затем предоставляем несколько свойств, каждый из которых будет записываться в базу данных при сохранении этой записи. Первое свойство, которое мы создаем в качестве первичного ключа, и устанавливайте для него значение Auto, следующим образом:
...
В качестве примера использования мы добавим табличное представление в пользовательский интерфейс в конструкторе интерфейсов Xcode. Мы предоставляем это табличное представление через розетку (`OccupationTable`), чтобы мы могли получить к C# нему доступ через код:
...
Когда мы создадим экземпляр этого класса позже, мы передаем нам подключение к открытой базе данных SQLite.NET. Метод `LoadOccupations` запрашивает базу данных и копирует найденные записи в память (с помощью нашей `OccupationModel` модели данных).
...
Здесь мы используем коллекцию `Occupations` источника данных (которую мы загрузили из базы данных SQLite.NET) для заполнения столбцов таблицы с помощью переопределения метода `GetViewForItem`.

### <a name="populating-the-table"></a>Заполнение таблицы

После того, как все части на месте, запустим таблицу, когда она будет сведена из XIB-файла, переопределив метод `AwakeFromNib` и сделав его следующим образом:
