...
Во-первых `[Register("PersonModel")]` , атрибут регистрирует класс и предоставляет его цели — C. Затем класс должен наследовать от `NSObject` (или от подкласса, который наследует от `NSObject`), в результате чего добавляется несколько базовых методов, позволяющих КВК соответствовать классу. `[Export("Name")]` Далее атрибут`Name` предоставляет свойство и определяет значение ключа, которое впоследствии будет использоваться для доступа к свойству с помощью методов КВК и кво.

Наконец, чтобы иметь возможность принимать изменения, внесенные в ключ, в значение свойства, метод доступа должен заключить изменения в его значение `WillChangeValue` в `DidChangeValue` вызовах методов и (указав `Export` тот же ключ, что и атрибут).  Например:
...
Путь к ключу к имени `self.Child.Name` дочернего элемента будет или просто `Child.Name` (в зависимости от того, как использовалось значение ключа).

### <a name="getting-values-using-key-value-coding"></a>Получение значений с помощью кодирования "ключ — значение"

Метод возвращает значение для указанного ключа (в `NSString`виде) относительно экземпляра класса КВК, получающего запрос. `ValueForKey` Например, если `Person` является экземпляром класса, `PersonModel` определенного выше:
...
Будет возвращено значение `Name` свойства для этого `PersonModel`экземпляра.

### <a name="setting-values-using-key-value-coding"></a>Установка значений с помощью кодирования «ключ-значение»

Аналогичным `NSString`образом задаетсязначениедляуказанногоключа(ввиде)относительноэкземпляраклассаКВК,`SetValueForKey` получающего запрос. Опять же, используя экземпляр `PersonModel` класса, как показано ниже:
...
Используя отслеживание "ключ-значение" (кво), можно присоединить наблюдателя к конкретному ключу класса, совместимого с КВК, и получать уведомления каждый раз, когда изменяется значение этого ключа (с помощью методов КВК или прямого доступа к заданному C# свойству в коде). Например:
...
Теперь при `Name` каждом изменении свойства `Person` экземпляра `PersonModel` класса новое значение записывается на консоль.
...
Большинство функций этого класса были рассмотрены в разделе [что такое кодирование "ключ — значение](#What_is_Key-Value_Coding) " выше. Однако рассмотрим несколько отдельных элементов и некоторые дополнения, которые позволят этому классу работать в качестве модели данных для **контроллеров массива** и **контроллеров дерева** (которые мы будем использовать позже для **представлений дерева**привязки данных, **представлений структуры** и **представления коллекций**).

Во-первых, так как сотрудник может быть руководителем, мы использовали `NSArray` ( `NSMutableArray` точнее, чтобы можно было изменить значения), чтобы разрешить к ним доступ сотрудникам, которыми они управляют.
...
1. Мы использовали `NSMutableArray` вместо стандартного C# массива или коллекции, так как это требование к связыванию данных с элементами управления AppKit, такими как **табличные представления**, **представления структуры** и **коллекции**.
2. Мы предоставляли массив сотрудников путем приведения их к `NSArray` типу для привязки данных и изменили C# его форматированное `People`имя на ту, которая ожидает привязки данных, `personModelArray` в виде **массива {class_name}** (Обратите внимание, что первая символ был введен в нижнем регистре).
...
Они позволяют контроллерам запрашивать и изменять отображаемые данные. Как показано выше, они имеют очень специфичное соглашение об именовании (отличающееся от стандартных C# соглашений об именовании): `NSArray`
...
Наконец, поскольку `Icon` свойство зависит от `isManager` значения `isManager` свойства, изменения свойства могут не отражаться в `Icon` для элементов пользовательского интерфейса, привязанных к данным (во время кво):
...
Обратите внимание, что в дополнение к собственному `isManager` ключу метод доступа также `WillChangeValue` отправляет сообщения `DidChangeValue` и для `Icon` ключа, так что он также будет видеть изменения.
...
Для привязки данных формы к `PersonModel` , которую мы предоставляем `Person` через ключ, выполните следующие действия.
...
9. Это говорит о том, что метка будет скрыта, если `isManager` свойство имеет `false`значение.
...
Как `PersonModel` и в приведенном выше классе в разделе [Определение модели данных](#Defining_your_Data_Model) , мы предоставили четыре специально именованных открытых метода, чтобы контроллер массива и читал и записывал данные `PersonModels`из нашей коллекции.
...
5. Переключитесь в **инспектор привязок** и в разделе **массив содержимого** выберите **Привязка к** и **контроллер представления таблицы**. Введите `self.personModelArray` **путь к ключу модели** :
...
Как `PersonModel` и в приведенном выше классе в разделе [Определение модели данных](#Defining_your_Data_Model) , мы предоставили четыре специально именованных открытых метода, чтобы контроллер дерева и читал и записывал данные `PersonModels`из нашей коллекции.
...
5. В разделе **контроллер дерева** `personModelArray` введите для **дочерних элементов**, `NumberOfEmployees` введите в поле **число** и введите `isEmployee` в поле **конечный**:
...
7. Переключитесь в **инспектор привязок** и в разделе **массив содержимого** выберите **Привязка к** и **владелец файла**. Введите `self.personModelArray` **путь к ключу модели** :
...
1. Модель данных не наследует от или `NSObject` от `NSObject`подкласса.
2. Вы не предоставили свое свойство цели-C с помощью `[Export("key-name")]` атрибута.
3. Вы не переносите изменения в значения метода доступа в `WillChangeValue` и `DidChangeValue` вызовы методов (указав `Export` тот же ключ, что и атрибут).
...
Сохраните изменения, вернитесь в Visual Studio для Mac для синхронизации с Xcode и запустите наше приложение. При отображении представления коллекции происходит `SIGABRT` сбой приложения с ошибкой (как показано в **выходных данных приложения** в `PersonModel` Visual Studio для Mac), поскольку не предоставляет свойство с ключом: `Title`
