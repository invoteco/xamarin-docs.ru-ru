...
2. Мы предоставили доступ к массиву сотрудников, приведя его к `NSArray` для привязки данных и C# изменяли его отформатированное имя, `People`, в ту, что ожидает привязка данных, `personModelArray` в форме **{class_name} Array** (Обратите внимание, что первый символ был создан нижний регистр).
