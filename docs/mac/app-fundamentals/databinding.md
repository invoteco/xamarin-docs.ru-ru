...
2. Мы предоставляли массив сотрудников путем приведения их к `NSArray` для привязки данных и изменения C# форматированного имени,`People`, на ту, что она ожидает привязки данных,`personModelArray`в форме **{class_name} Array** (Обратите внимание, что первым символом был сделано в нижнем регистре).
