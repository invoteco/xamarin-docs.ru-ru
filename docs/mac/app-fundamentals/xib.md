...
Предпочтительным способом создания пользовательского интерфейса приложения Xamarin. Mac является Раскадровка (Дополнительные сведения см. [в](~/mac/platform/storyboards/index.md) документации по раскадровкам). в результате все новые проекты, запущенные в Xamarin. Mac, будут использовать раскадровки, параметры.
...
3. Выберите**контроллер Windows** **Mac** > :

    ![Добавление нового контроллера окна](xib-images/setup00.png "Добавление нового контроллера окна")
4. Введите `MainWindow` в поле имя и нажмите кнопку **New (создать** ):
...
7. Оставьте имя `MainMenu` , а затем нажмите кнопку **создать** .
...
13. В **инспекторе библиотек**введите `object` в поле поиска и перетащите новый **объект** в область конструктора:
...
15. Выберите **владелец файла** в **иерархии интерфейс**, переключитесь в **инспектор подключений** и перетащите строку из делегата в `AppDelegate` **объект** , только что добавленный в проект:
...
Этот файл заглушек. h отражает **MainWindow.Designer.CS** , который автоматически добавляется в проект Xamarin. Mac при создании нового `NSWindow` . Этот файл будет использоваться для синхронизации изменений, внесенных Interface Builder, и здесь будут созданы ваши розетки и действия, чтобы элементы пользовательского интерфейса были доступны C# коду.
...
7. В **редакторе интерфейса**удерживайте нажатой клавишу **CTRL** на клавиатуре и перетащите метку, созданную выше, в редактор кода `@interface MainWindow : NSWindow { }` непосредственно под кодом:
...
8. Откроется диалоговое окно. Оставьте **Подключение** установленным в розетке и `ClickedLabel` введите для **имени**:
...
2. В **редакторе интерфейса**удерживайте нажатой клавишу **CTRL** на клавиатуре и перетащите кнопку, созданную выше, в редактор кода `@property (assign) IBOutlet NSTextField *ClickedLabel;` непосредственно под кодом:
...
И добавьте следующий код в `MainWindow` класс для работы с образцом выхода, созданным ранее:
...
Обратите внимание `NSLabel` , что доступ C# к предоставляется по непосредственному имени, которое вы назначили в Xcode при создании розетки в Xcode, в данном случае он называется `ClickedLabel`. Можно получить доступ к любому методу или свойству предоставленного объекта так же, как и C# к любому обычному классу.

> [!IMPORTANT]
> Необходимо использовать `AwakeFromNib`вместо другого метода, такого как `Initialize`, поскольку `AwakeFromNib` вызывается _после_ загрузки ОС и создания экземпляра пользовательского интерфейса из XIB-файла. Если вы попытались получить доступ к элементу управления Label до того, как файл. XIB будет полностью загружен и создан, возникнет `NullReferenceException` ошибка, так как элемент управления "метка" еще не создавался.

Затем добавьте в `MainWindow` класс следующий разделяемый класс:
...
Ключевая строка здесь означает `[Export ("openDocument:")]`, `NSMenu` что **AppDelegate** имеет метод `void OpenDialog (NSObject sender)` , отвечающий `openDocument:` на действие.
...
`var preferences = new PreferencesWindowController ();` Строка создает новый экземпляр контроллера окна, который загружает окно из XIB-файла и расширяет его. В `preferences.Window.MakeKeyAndOrderFront (this);` строке отображается новое окно для пользователя.
...
1. В **панель решения**щелкните правой кнопкой мыши проект и выберите **Добавить** > **новый файл..** .
2. В диалоговом окне новый файл выберите **Xamarin. Mac** > **Cocoa View with Controller**:
...
Измените файл XIB в окне, который будет использовать представление и отобразит его. Добавьте **пользовательское представление** , которое будет действовать в качестве контейнера для представления после его загрузки в память C# кодом и предоставления его вызываемой `ViewContainer`розетке:
...
При этом создается новый экземпляр контроллера представления для отображения нового представления, устанавливается его тип (как указано в перечислении, добавленном в проект) и используется `DisplaySubview` метод, добавленный в класс окна для фактического отображения представления. Например:
