...
Это приведет к созданию представления содержимого, которое будет заполнять окно. Теперь мы добавим в окно наш первый элемент `NSButton`пользовательского интерфейса —.
...
Наконец, `ContentView.AddSubview (ClickMeButton)` метод `NSButton` добавляет в представление содержимого, чтобы оно отображалось на экране при запуске приложения и отображенном окне.

Затем в окно добавляется метка, отображающая количество раз, когда `NSButton` была нажата кнопка: 
...
Поскольку у macOS нет определенного элемента пользовательского интерфейса _Label_ , мы добавили специально оформленный, нередактируемый `NSTextField` , чтобы действовать как метка. Точно так же, как и на кнопке, размер и расположение принимают учетные записи, которые (0, 0) находятся в левом нижнем углу окна. Свойство использует оператор **or** для объединения двух `NSViewResizingMask` компонентов. `AutoresizingMask = NSViewResizingMask.WidthSizable | NSViewResizingMask.MinYMargin` Это приведет к тому, что метка останется в том же расположении, что и верхняя часть окна, когда размер окна изменяется вертикально и сжимается и увеличивается по ширине по мере изменения размера окна по горизонтали.
...
Сначала мы определим новый экземпляр `MainWindow` класса и присваиваем его `Window` свойству базового контроллера окна:
...
Последние два свойства определяют _тип буферизации_ для окна и, если отрисовка окна будет отложена. Дополнительные сведения о см `NSWindows`. в документации Apple [Введение в Windows](https://developer.apple.com/library/mac/documentation/Cocoa/Conceptual/WinPanel/Introduction.html#//apple_ref/doc/uid/10000031-SW1) .

Наконец, поскольку окно не было сведено из файла. Storyboard или. XIB, нам нужно смоделировать его в нашей **MainWindowController.CS** , вызвав метод Windows `AwakeFromNib` :
...
Было ли окно создано в коде или загружено из файла. Storyboard или. XIB, иногда требуется добавить элемент пользовательского интерфейса в окно из кода. Например:
...
Из-за текущих ограничений в Xamarin. Mac не рекомендуется создавать строку меню приложения Xamarin. Mac —`NSMenuBar`— в коде, но продолжать использовать файл **Main. Storyboard** или **MainMenu. XIB** для определения. С другой стороны, можно добавлять и удалять меню и пункты меню в C# коде.
