...
- Для всех классов, которые не заносятся в оболочку существующего класса цели-c, он создает новый класс цели-c с элементами цели-c, которые отражают все управляемые члены `[Export]` , имеющие атрибут.
...
Генератор, вызываемый `bmac` в Xamarin. Mac, принимает эти файлы определения и использует средства .NET для их компиляции во временную сборку. Однако эта временная сборка непригодна для вызова кода цели-C. Затем генератор считывает временную сборку и создает C# код, который можно использовать во время выполнения. Вот почему, например, при добавлении случайного атрибута в файл Definition. CS он не будет отображаться в выводимом коде. Генератор не знает о нем и, следовательно `bmac` , не знает, как найти его во временной сборке для вывода.

После создания Xamarin. Mac. dll упаковщик, `mmp`будет объединять все компоненты.
