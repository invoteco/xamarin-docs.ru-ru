...
Компания Apple включила наблюдаемое `ContentLayoutRect` свойство " [ключ-значение](~/mac/app-fundamentals/databinding.md#Observing_Value_Changes) " `NSWindow` класса, чтобы получить текущую область содержимого в коде. Разработчик может использовать это значение, чтобы вручную разместить необходимые элементы при изменении области содержимого.
...
Ключевые моменты этого кода — это то место, где для представления задано пользовательское представление, которое было определено в Interface Builder ипредоставляется в качестве розетки:
...
- **Применить начальные и конечные атрибуты** , так как направление `Leading` текста может измениться в зависимости от языка пользователя, используйте новые атрибуты и `Trailing` ограничения, а не существующие и `Left` `Right` атрибута. `Leading`и `Trailing` будут автоматически корректироваться в зависимости от направления языков.
