...
- Проверьте выходные данные приложения Visual Studio для Mac (**просмотр** -> **Pad** -> **выходные данные приложения**) для трассировки стека или вывода красным цветом из Cocoa, которые могут описывать выходные данные.
...
- Можно подключить собственный отладчик (`lldb`) к процессу, чтобы узнать, предоставляет ли он дополнительные сведения (для этого требуется платная лицензия). Например, выполните следующие действия.
...
- В качестве последнего средства перед вызовом `NSApplication.Init` в методе `Main` (или в других местах по мере необходимости) можно было бы записать текст в файл в известном расположении, чтобы отвести внимание на том, на каком этапе запуска возникают проблемы.
...
Это можно устранить, открыв параметры для проекта Xamarin. Mac, перейдя в **Mac Build** > **интернационализации** и проверяя **западную** международную линию:

[![Изменение параметров сборки](troubleshooting-images/issue01.png "Изменение параметров сборки")](troubleshooting-images/issue01-large.png#lightbox)
