...
A `NSCollectionView` отображает сетку вложенных представлений, `NSCollectionViewLayout`упорядоченных с помощью. Каждое вложенное представление в сетке представляется `NSCollectionViewItem` объектом, который управляет загрузкой содержимого представления `.xib` из файла.

[![Пример выполнения приложения](collection-view-images/intro01.png)](collection-view-images/intro01.png#lightbox)

В этой статье рассматриваются основы работы с представлениями коллекций в приложении Xamarin. Mac. Мы настоятельно рекомендуем сначала ознакомиться со статьей [Hello, Mac](~/mac/get-started/hello-mac.md) , в частности [введением в разделы Xcode и Interface Builder](~/mac/get-started/hello-mac.md#introduction-to-xcode-and-interface-builder) , а также [Outlets and Actions](~/mac/get-started/hello-mac.md#outlets-and-actions) о том, как они посвящены основным концепциям и методикам, используемым в этой статье.
...
2. Выберите > **контроллер представления**Mac, присвойте ему имя (например, `EmployeeItem` в этом примере) и нажмите кнопку **создать** , чтобы создать: 
    ...
    Это приведет к `EmployeeItem.cs`добавлению `EmployeeItemController.cs` файла `EmployeeItemController.xib` и в решение проекта.
3. Дважды щелкните `EmployeeItemController.xib` файл, чтобы открыть его для редактирования в Interface Builder Xcode.
4. Добавьте и два`NSLabel` элемента управления в представление и разметьте их следующим образом: `NSImageView` `NSBox`
...
7. В **инспекторе привязки**выберите привязать **к** > **владельцу файла** и введите `self.Person.Icon` **путь к ключу модели** :

    ![Привязка значка](collection-view-images/proto04.png)
8. Выберите первую метку и в инспекторе **привязки**выберите **Привязка к** > **владельцу файла** и введите `self.Person.Name` **путь к ключу модели** :

    ![Привязка имени](collection-view-images/proto05.png)
9. Выберите вторую метку и в инспекторе **привязки**выберите **Привязка к** > **владельцу файла** и введите `self.Person.Occupation` **путь к ключу модели** :
...
`EmployeeItemController.cs` Измените файл и сделайте его следующим:
...
Свойство является ярлыком `NSBox` элемента управления `FillColor` , который будет использоваться для отображения состояния выбора ячейки. `BackgroundColor` При переопределении `Selected` свойства `NSCollectionViewItem`объекта следующий код задает или очищает это состояние выбора:
...
Источник данных представления коллекции (`NSCollectionViewDataSource`) предоставляет все данные для представления коллекции и создает и заполняет ячейку представления коллекции ( `.xib` с использованием прототипа) в соответствии с требованиями для каждого элемента в коллекции.
...
Изучив этот код, класс наследует от `NSCollectionViewDataSource` класса и предоставляет `PersonModel` список экземпляров через его `Data` свойство.

Так как эта коллекция содержит только один раздел, код переопределяет `GetNumberOfSections` метод и всегда возвращает `1`. Кроме того, `GetNumberofItems` метод переопределяется в нем и возвращает количество элементов `Data` в списке свойств.
...
Метод представления коллекции вызывается для создания или возврата экземпляра с возможностью повторного использования, `EmployeeItemController` а его `Person` свойство имеет значение Item, отображаемое в запрошенной ячейке. `MakeItem` 
...
**Идентификатор** (`EmployeeCell` ),используемыйввызове,долженсоответствоватьимениконтроллерапредставления,зарегистрированноговпредставлении`MakeItem` коллекции. Этот шаг подробно рассматривается ниже.

<a name="Handling-Item-Selection"/>

### <a name="handling-item-selection"></a>Обработка выбора элементов

Чтобы обрабатывать выделение и `NSCollectionViewDelegate` отменять выборку элементов в коллекции, требуется. Поскольку в этом примере используется встроенный `NSCollectionViewFlowLayout` тип макета `NSCollectionViewDelegateFlowLayout` , потребуется определенная версия этого делегата.
...
Методы `ItemsSelected` `PersonSelected` и `ItemsDeselected` переопределяются и используются для установки или очистки свойства контроллера представления, обрабатывающего представление коллекции, когда пользователь выбирает или отменяет выбор элемента. Это будет показано подробно.
...
Все вспомогательные части теперь`PersonModel`помещены в класс `NSCollectionViewDataSource` , который будет использоваться в качестве модели данных (), добавлен `NSCollectionViewDelegateFlowLayout` для передачи данных, `NSCollectionView` был создан для обработки выбора элементов, а добавлен в основную раскадровку. и предоставляются как розетка (`EmployeeCollection`).
...
`ViewController.cs` Измените файл и сделайте его следующим:
...
Подробно взглянув на этот код, `Datasource` свойство определяется для хранения экземпляра `CollectionViewDataSource` , который предоставит данные для представления коллекции. Свойство определяется для хранения объекта, `PersonModel` представляющего текущий выбранный элемент в представлении коллекции. `PersonSelected` Это свойство также вызывает `SelectionChanged` событие при изменении выбора.
...
Обратите внимание , что`EmployeeCell`идентификатор (), используемый для регистрации прототипа, совпадает с `GetItem` именем, вызываемым в методе, `CollectionViewDataSource` указанном выше:
...
Компания Apple поставляется с представлением коллекции двух встроенных типов макетов, которые будут использовать наиболее типичные `NSCollectionViewGridLayout`варианты использования: `NSCollectionViewFlowLayout` и. Если разработчику требуется пользовательский формат, например размещение элементов в окружности, они могут создать пользовательский экземпляр `NSCollectionViewLayout` и переопределить необходимые методы для достижения желаемого результата.
...
Метод создает новый экземпляр `CollectionViewDataSource`класса, заполняет его данными, присоединяет его к представлению `ReloadData` коллекции и вызывает метод для отображения элементов: `PopulateWithData`
...
Метод переопределен и `ConfigureCollectionView` вызывает методы и `PopulateWithData` для отображения конечного представления коллекции пользователю: `ViewDidLoad`
