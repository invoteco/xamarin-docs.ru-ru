...
Если выбрать **первый ответчик** в **иерархии интерфейсов** и прокрутить вниз в **инспекторе подключений**, появится определение `openDocument:` действия, к которому присоединен пункт меню **Открыть** (вместе с несколькими другими действия по умолчанию для приложения, которые не привязаны к элементам управления автоматически.
...
Что здесь происходит? Помните о встроенных предопределенных действиях, которые связаны с элементами меню по умолчанию (как было показано выше), большинство элементов пользовательского интерфейса Cocoa, которые являются частью macOS, имеют встроенные перехватчики для определенных действий (например, `copy:`). Таким образом, когда они добавляются в окно, активно и выбрано, соответствующий элемент меню или элементы, присоединенные к этому действию, автоматически включаются. Если пользователь выбирает этот пункт меню, функциональные возможности, встроенные в элемент пользовательского интерфейса, вызываются и выполняются, все без участия разработчика.

### <a name="enabling-and-disabling-menus-and-items"></a>Включение и отключение меню и элементов

По умолчанию каждый раз, когда происходит событие пользователя, `NSMenu` автоматически включает и отключает каждое видимое меню и пункт меню в зависимости от контекста приложения. Включить или отключить элемент можно тремя способами:

- **Автоматическое включение меню** — элемент меню включается, если `NSMenu` может найти соответствующий объект, который реагирует на действие, к которому привязан элемент. Например, представленное выше текстовое представление содержит встроенное подключение к `copy:` действию.
- **Настраиваемые действия и валидатеменуитем:** — для любого элемента меню, привязанного к [настраиваемому действию контроллера окна или представления](#Working-with-Custom-Window-Actions), можно добавить действие `validateMenuItem:` и включить или отключить элементы меню вручную.
- **Включение меню вручную** . вручную задайте для свойства `Enabled` каждой `NSMenuItem` значение, чтобы включить или отключить каждый элемент в меню по отдельности.

Чтобы выбрать систему, задайте свойство `AutoEnablesItems` `NSMenu`. `true` является автоматическим (поведение по умолчанию), а `false` — вручную. 

> [!IMPORTANT]
> Если вы решили использовать ручное меню, ни один из пунктов меню, даже не управляемые AppKit классами, такими как `NSTextView`, обновляется автоматически. Вы несете ответственность за включение и отключение всех элементов вручную в коде.

#### <a name="using-validatemenuitem"></a>Использование Валидатеменуитем

Как упоминалось выше, для любого элемента меню, привязанного к [настраиваемому действию контроллера окна или представления](#Working-with-Custom-Window-Actions), можно добавить действие `validateMenuItem:` и включить или отключить элементы меню вручную.

В следующем примере свойство `Tag` будет использоваться для выбора типа элемента меню, который будет включен или отключен действием `validateMenuItem:` в зависимости от состояния выбранного текста в `NSTextView`. Свойство `Tag` задано в Interface Builder для каждого пункта меню:
...
Если при выполнении этого кода в `NSTextView` не выбран текст, то два элемента меню обтекания отключаются (несмотря на то, что они связаны с действиями на контроллере представления):
...
В **панель решения**дважды щелкните файл `AppDelegate.CS`, чтобы открыть его для редактирования:
...
Ключевая строка была `[Export ("openDocument:")]`, она сообщает `NSMenu`, что наш **AppDelegate** имеет метод `void OpenDialog (NSObject sender)`, отвечающий на действие `openDocument:`. Если вы запомним выше, пункт меню **Открыть** автоматически поддается этому действию по умолчанию в Interface Builder:
...
По умолчанию меню **файл** содержит **открытый** элемент, который отслеживает последние несколько файлов, открытых пользователем в приложении. Если вы создаете приложение Xamarin. Mac на основе `NSDocument`, это меню будет автоматически обработано. Для любого другого типа приложения Xamarin. Mac вы будете отвечать за управление этим элементом меню и реагирование на него вручную.
...
Несмотря на то, что приложение не использует `NSDocuments`, вы по-прежнему используете `NSDocumentController` для поддержания меню **Открыть последние** , отправляя `NSUrl` с расположением файла в метод `NoteNewRecentDocumentURL` `SharedDocumentController`.

Далее необходимо переопределить метод `OpenFile` делегата приложения, чтобы открыть любой файл, выбираемый пользователем из меню **Открыть последние** . Пример:
...
Возвращает `true` если файл можно открыть, иначе возвращается `false`, а для пользователя отображается встроенное предупреждение о том, что не удалось открыть файл.

Так как имя файла и путь, возвращаемые из меню **Открыть последние** , могут содержать пробелы, необходимо надлежащим образом отэкранированить этот символ перед созданием `NSUrl` или возникнет ошибка. Мы делаем это с помощью следующего кода:

```csharp
filename = filename.Replace (" ", "%20");
```

Наконец, мы создаем `NSUrl`, указывающий на файл, и используем вспомогательный метод в делегате приложения, чтобы открыть новое окно и загрузить в него файл:
...
Мы разработали класс `ViewController` для хранения пути к файлу в его свойстве `Path`. Далее мы выполним циклический перебор всех открытых окон в приложении. Если файл уже открыт в одном из окон, он переносится на передний план всех остальных окон с помощью:
...
Здесь мы сделали `AppDelegate` разделяемого класса, чтобы мы могли использовать действия и возможности, которые мы определили в Interface Builder. Мы также предоставляем `textEditor` для трассировки окна, в котором в данный момент находится фокус.
...
Если этот класс определен, следующая подпрограммы будет анализировать коллекцию `LanguageFormatCommand`objects и рекурсивно создавать новые меню и пункты меню, добавляя их в конец существующего меню (созданного в Interface Builder), которое было передано:
...
Для любого объекта `LanguageFormatCommand`, имеющего пустое свойство `Title`, эта подпрограммы создает **пункт меню разделителя** (тонкая серая линия) между разделами меню:
...
Если объект `LanguageFormatCommand` содержит дочерние объекты `LanguageFormatCommand`, создается подменю, и метод `AssembleMenu` рекурсивно вызывается для создания этого меню:
...
При наличии всего приведенного выше кода, если была создана следующая коллекция объектов `LanguageFormatCommand`:
...
И эта коллекция передается функции `AssembleMenu` (в меню **Формат** , заданном как основа), будут созданы следующие динамические меню и пункты меню:
...
Если необходимо удалить любое меню или пункт меню из пользовательского интерфейса приложения, можно использовать метод `RemoveItemAt` класса `NSMenu`, указав для него Отсчитываемый от нуля индекс удаляемого элемента.
...
Измените наш файл **AppDelegate.CS** и сделайте метод `DidFinishLaunching` выглядеть следующим образом:
...
1. В Visual Studio для Mac щелкните правой кнопкой мыши проект приложения и выберите **добавить**  > **новый файл...** В диалоговом окне новый файл выберите **Xamarin. Mac**  > **пустое определение интерфейса**, используйте для **имени** "Доккмену" и нажмите кнопку " **создать** ", чтобы создать новый файл **доккмену. XIB** :
...
5. Затем выберите **приложение** в **иерархии интерфейсов**. В **инспекторе подключений**перетащите линию из круга в `dockMenu`ную розетку в меню, которое мы только что создали:
