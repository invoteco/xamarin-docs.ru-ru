...
Также ознакомьтесь с [представлением C# классы и методы для цели-C](~/mac/internals/how-it-works.md) в документе [внутренних элементов Xamarin. Mac](~/mac/internals/how-it-works.md) . В нем объясняются `Export` атрибуты и, `Register` используемые C# для подключения классов к классам цели-C.
