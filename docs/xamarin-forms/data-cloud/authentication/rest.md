...
> [!NOTE]
> Обычная проверка подлинности должна использоваться только через HTTPS-соединение. При использовании через подключение HTTP `Authorization` легко можно расшифровать заголовок, если HTTP-трафика захвачен злоумышленником.
