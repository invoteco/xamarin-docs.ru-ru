...
[![Скачать пример](~/media/shared/download.png) Скачать пример](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/webservices-azuresignalr/)
...
Функция реагирует на веб-запросы `SignalRConnectionInfo` с помощью объекта, `Url` содержащего `AccessToken` свойство и свойство. `Negotiate` Мобильное приложение использует эти значения для регистрации в концентраторе SignalR. В следующем коде показана `Negotiate` функция:
...
Интеграция между службой Azure SignalR и приложением Xamarin. Forms — это класс службы SignalR, который создается в `MainPage` классе с обработчиками событий, назначенными трем событиям. Дополнительные сведения об этих обработчиках событий см. [в разделе Использование класса службы SignalR в Xamarin. Forms](#use-the-signalr-service-class-in-xamarinforms).

Пример приложения включает класс **Constants.CS** , который необходимо настроить с помощью конечной точки URL-адреса приложения функций Azure. В качестве значения `HostName` свойства укажите адрес приложения функций Azure. В следующем коде показаны свойства **Constants.CS** с примером `HostName` значения:
...
> [!NOTE]
> Свойство в файле примера приложения **Constants.CS** `RuntimePlatform` использует значение устройства в качестве имени пользователя. `Username` Это упрощает тестирование устройств на разных платформах и определяет, какое устройство отправляет сообщение. В реальных приложениях это значение, скорее всего, будет уникальным именем пользователя, собранным во время регистрации или входа в систему.

### <a name="the-signalr-service-class"></a>Класс службы SignalR

Класс в проекте чатклиент в примере приложения показывает реализацию, которая вызывает функции в приложении "функции Azure" для подключения к службе Azure SignalR. `SignalRService`

`SendMessageAsync` Метод`SignalRService` в классе используется для отправки сообщений клиентам, подключенным к службе SignalR Azure. Этот метод выполняет запрос HTTP POST к функции **разговора** , размещенной в приложении "функции Azure", включая сериализованный `Message` JSON-объект в качестве полезных данных POST. Функция **разговора** передает сообщение в службу Azure SignalR для вещания всем подключенным клиентам. В следующем примере кода показан метод `SendMessageAsync`:
...
Метод в классе выполняет HTTP-запрос GET к функции Negotiate, размещенной в приложении "функции Azure". `ConnectAsync` `SignalRService` Функция **Negotiate** возвращает JSON, который десериализуется в экземпляр `NegotiateInfo` класса. После получения `HubConnection` объекта он используется для непосредственной регистрации в службе Azure SignalR с помощью экземпляра класса. `NegotiateInfo`

ASP.NET Core SignalR преобразует входящие данные из открытого соединения в сообщения и позволяет разработчикам определять типы сообщений и привязывать обработчики событий к входящим сообщениям по типу. Метод регистрирует обработчик событий для имени сообщения, определенного в примере файла Constants.CS приложения, по умолчанию — "невмессаже". `ConnectAsync`
...
Метод привязан как обработчик событий `ConnectAsync` в сообщении, как показано в предыдущем коде. `AddNewMessage` При получении сообщения `AddNewMessage` метод вызывается с данными сообщения, предоставленными в `JObject`виде. Метод преобразует в `JObject` `NewMessageReceived` экземпляр `Message` класса, а затем вызывает обработчик для, если он был привязан. `AddNewMessage` В следующем примере кода показан метод `AddNewMessage`:
...
Использование класса службы SignalR в Xamarin. Forms осуществляется путем привязки `SignalRService` событий класса `MainPage` в классе кода программной части.

`Connected` Событие`SignalRService` в классе срабатывает при успешном завершении подключения SignalR. `ConnectionFailed` Событие`SignalRService` в классе срабатывает при сбое подключения SignalR. Метод обработчика `MainPage`событийпривязан к обоим событиям в конструкторе. `SignalR_ConnectionChanged` Этот обработчик событий обновляет состояния кнопок подключения и отправки на основе аргумента соединения `success` и добавляет сообщение, предоставленное событием, в очередь разговора `AddMessage` с помощью метода. В следующем коде показан `SignalR_ConnectionChanged` метод обработчика событий:
...
`NewMessageReceived` Событие`SignalRService` в классе срабатывает при получении нового сообщения от службы Azure SignalR. Метод обработчика `NewMessageReceived`событийпривязан к событию в `MainPage` конструкторе. `SignalR_NewMessageReceived` Этот обработчик событий преобразует входящий `Message` объект в строку и добавляет его в очередь разговора `AddMessage` с помощью метода. В следующем коде показан `SignalR_NewMessageReceived` метод обработчика событий:
