...
[Снимок экрана создания приложения функций Azure ![](azure-signalr-images/azure-functions-app-cropped.png)](azure-signalr-images/azure-functions-app-full.png#lightbox)
...
> [!NOTE]
> Служба SignalR использует `System.Text.Json` для сериализации и десериализации JSON по умолчанию. Данные, сериализованные с помощью других библиотек, таких как Newtonsoft, могут не быть десериализованы службой SignalR. Экземпляр `HubConnection` в примере проекта включает вызов `AddNewtonsoftJsonProtocol` для указания сериализатора JSON. Этот метод определен в специальном пакете NuGet с именем **Microsoft. AspNetCore. SignalR. Protocols. невтонсофтжсон** , который должен быть включен в проект. Если для сериализации и десериализации данных JSON используется `System.Text.Json`, этот метод и пакет NuGet использовать не следует.
