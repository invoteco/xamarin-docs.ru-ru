...
`GetContainer` Метод позволяет получить ссылку на именованный контейнер, который затем используется для получения больших двоичных объектов из контейнера или добавить в контейнер больших двоичных объектов. Метод `GetContainer` показан в следующем примере кода:
