...
Существуют различия между ASP.NET Web Services (ASMX) и WCF, но также WCF поддерживает те же возможности, которые предоставляет ASMX-сообщений SOAP по протоколу HTTP. Дополнительные сведения об использовании службы ASMX, см. в разделе [использовать ASP.NET Web Services (ASMX)](~/xamarin-forms/data-cloud/web-services/asmx.md).
