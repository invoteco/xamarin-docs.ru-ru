...
Данные, включаемые клиентским приложением в HTTP-запросе, и соответствующие ответные сообщения от веб сервера могут быть представлены в различных форматах, известных как типы мультимедиа. Когда клиентское приложение отправляет запрос, который возвращает данные в теле сообщения, он может указать типы мультимедиа, которые он может обменяться в заголовке `Accept` запроса. Если веб-сервер поддерживает этот тип мультимедиа, он может ответить ответом, содержащим заголовок `Content-Type`, указывающий формат данных в тексте сообщения. Затем клиентское приложение отвечает за синтаксический анализ ответного сообщения и интерпретирует результаты в тексте сообщения соответствующим образом.
...
Мобильное приложение eShopOnContainers использует класс `HttpClient` для выполнения запросов по протоколу HTTP, в котором в качестве типа мультимедиа используется JSON. Этот класс предоставляет функциональные возможности асинхронной отправки HTTP-запросов и получения HTTP-ответов от идентифицированного ресурса URI. Класс `HttpResponseMessage` представляет сообщение HTTP-ответа, полученное от REST API после того, как был выполнен HTTP-запрос. Он содержит сведения об ответе, включая код состояния, заголовков и любой текст. `HttpContent` Класс представляет тело HTTP и заголовки содержимого, таких как `Content-Type` и `Content-Encoding`. Содержимое можно считать с помощью любого из `ReadAs` методов, таких как `ReadAsStringAsync` и `ReadAsByteArrayAsync`, в зависимости от формата данных.

<a name="making_a_get_request" />

#### <a name="making-a-get-request"></a>Выполнение запроса GET

Класс `CatalogService` используется для управления процессом получения данных из микрослужбы каталога. В методе `RegisterDependencies` класса `ViewModelLocator` класс `CatalogService` регистрируется как сопоставление типа с типом `ICatalogService` с помощью контейнера внедрения зависимостей Autofac. Затем, когда создается экземпляр класса `CatalogViewModel`, его конструктор принимает тип `ICatalogService`, который Autofac разрешается, возвращая экземпляр класса `CatalogService`. Дополнительные сведения о внедрении зависимостей см. [в разделе Введение в внедрение зависимостей](~/xamarin-forms/enterprise-application-patterns/dependency-injection.md#introduction_to_dependency_injection).

На рис. 10-1 показано взаимодействие классов, считывающих данные каталога из микрослужбы каталога для отображения с помощью `CatalogView`.
...
При переходе к `CatalogView` вызывается метод `OnInitialize` в классе `CatalogViewModel`. Этот метод извлекает данные каталога из микрослужбы каталога, как показано в следующем примере кода:
...
Этот метод вызывает метод `GetCatalogAsync` экземпляра `CatalogService`, который был вставлен в `CatalogViewModel` с помощью Autofac. Метод `GetCatalogAsync` показан в следующем примере кода:
...
Этот метод создает URI, определяющий ресурс, в который будет отправлен запрос, и использует класс `RequestProvider`, чтобы вызвать метод GET HTTP для ресурса, прежде чем возвратить результаты в `CatalogViewModel`. Класс `RequestProvider` содержит функциональные возможности, которые отправляют запрос в форме URI, который идентифицирует ресурс, метод HTTP, указывающий операцию, выполняемую с этим ресурсом, и текст, содержащий все данные, необходимые для выполнения операции. Сведения о внедрении класса `RequestProvider` в `CatalogService class` см. [в разделе Введение в внедрение зависимостей](~/xamarin-forms/enterprise-application-patterns/dependency-injection.md#introduction_to_dependency_injection).

В следующем примере кода показан метод `GetAsync` в классе `RequestProvider`:
...
Этот метод вызывает метод `CreateHttpClient`, который возвращает экземпляр класса `HttpClient` с соответствующим набором заголовков. Затем он отправляет асинхронный запрос GET в ресурс, идентифицируемый с помощью URI, с ответом, хранящимся в экземпляре `HttpResponseMessage`. Затем вызывается метод `HandleResponse`, который вызывает исключение, если ответ не содержит код состояния успешного HTTP. Затем ответ считывается в виде строки, преобразуется из JSON в объект `CatalogRoot` и возвращается в `CatalogService`.
...
Этот метод создает новый экземпляр класса `HttpClient` и устанавливает заголовок `Accept` всех запросов, сделанных экземпляром `HttpClient`, в `application/json`, что указывает на то, что оно требует, чтобы содержимое любого ответа отформатировано с помощью JSON. Затем, если маркер доступа был передан в качестве аргумента в метод `CreateHttpClient`, он добавляется в заголовок `Authorization` всех запросов, выполняемых экземпляром `HttpClient`, с префиксом строки `Bearer`. Дополнительные сведения об авторизации см. в разделе [авторизация](~/xamarin-forms/enterprise-application-patterns/authentication-and-authorization.md#authorization).

Когда метод `GetAsync` в классе `RequestProvider` вызывает `HttpClient.GetAsync`, вызывается метод `Items` в классе `CatalogController` в проекте Catalog. API, который показан в следующем примере кода:
...
Этот метод получает данные каталога из базы данных SQL с помощью EntityFramework и возвращает их как ответное сообщение, содержащее код состояния HTTP Success, и коллекцию отформатированных экземпляров JSON `CatalogItem`.

#### <a name="making-a-post-request"></a>Выполнение запроса POST

Класс `BasketService` используется для управления получением и обновлением данных с помощью микрослужбы корзины. В методе `RegisterDependencies` класса `ViewModelLocator` класс `BasketService` регистрируется как сопоставление типа с типом `IBasketService` с помощью контейнера внедрения зависимостей Autofac. Затем, когда создается экземпляр класса `BasketViewModel`, его конструктор принимает тип `IBasketService`, который Autofac разрешается, возвращая экземпляр класса `BasketService`. Дополнительные сведения о внедрении зависимостей см. [в разделе Введение в внедрение зависимостей](~/xamarin-forms/enterprise-application-patterns/dependency-injection.md#introduction_to_dependency_injection).

На рис. 10-2 показано взаимодействие классов, которые отправляют данные корзины, отображаемые `BasketView`, в микрослужбу корзины.
...
При добавлении элемента в корзину покупок вызывается метод `ReCalculateTotalAsync` в классе `BasketViewModel`. Этот метод обновляет общее значение элементов в корзине и отправляет данные о корзине в микрослужбу корзины, как показано в следующем примере кода:
...
Этот метод вызывает метод `UpdateBasketAsync` экземпляра `BasketService`, который был вставлен в `BasketViewModel` с помощью Autofac. В следующем методе показан метод `UpdateBasketAsync`:
...
Этот метод создает URI, определяющий ресурс, в который будет отправлен запрос, и использует класс `RequestProvider` для вызова HTTP-метода POST для ресурса перед возвратом результатов в `BasketViewModel`. Обратите внимание, что маркер доступа, полученный от IdentityServer во время процесса проверки подлинности, необходим для авторизации запросов к микрослужбе корзины. Дополнительные сведения об авторизации см. в разделе [авторизация](~/xamarin-forms/enterprise-application-patterns/authentication-and-authorization.md#authorization).

В следующем примере кода показан один из `PostAsync` методов класса `RequestProvider`:
...
Этот метод вызывает метод `CreateHttpClient`, который возвращает экземпляр класса `HttpClient` с соответствующим набором заголовков. Затем он отправляет асинхронный запрос POST к ресурсу, определяемому универсальным кодом ресурса (URI), при этом данные о сериализованной корзине отправляются в формате JSON и отклики сохраняются в экземпляре `HttpResponseMessage`. Затем вызывается метод `HandleResponse`, который вызывает исключение, если ответ не содержит код состояния успешного HTTP. Затем ответ считывается в виде строки, преобразуется из JSON в объект `CustomerBasket` и возвращается в `BasketService`. Дополнительные сведения о методе `CreateHttpClient` см. [в разделе Создание запроса GET](#making_a_get_request).

Когда метод `PostAsync` в классе `RequestProvider` вызывает `HttpClient.PostAsync`, вызывается метод `Post` в классе `BasketController` в проекте корзины. API, который показан в следующем примере кода:
...
Этот метод использует экземпляр класса `RedisBasketRepository` для сохранения данных корзины в кэш Redis и возвращает их в виде ответного сообщения, содержащего код состояния HTTP Success, и экземпляр `CustomerBasket` в формате JSON.

#### <a name="making-a-delete-request"></a>Выполнение запроса на удаление

На рис. 10-3 показано взаимодействие классов, которые удаляют данные корзины из микрослужбы корзины для `CheckoutView`.
...
При вызове процесса извлечения вызывается метод `CheckoutAsync` в классе `CheckoutViewModel`. Этот метод создает новый заказ перед очисткой корзины покупок, как показано в следующем примере кода:
...
Этот метод вызывает метод `ClearBasketAsync` экземпляра `BasketService`, который был вставлен в `CheckoutViewModel` с помощью Autofac. В следующем методе показан метод `ClearBasketAsync`:
...
Этот метод создает URI, определяющий ресурс, в который будет отправлен запрос, и использует класс `RequestProvider` для вызова метода DELETE HTTP для ресурса. Обратите внимание, что маркер доступа, полученный от IdentityServer во время процесса проверки подлинности, необходим для авторизации запросов к микрослужбе корзины. Дополнительные сведения об авторизации см. в разделе [авторизация](~/xamarin-forms/enterprise-application-patterns/authentication-and-authorization.md#authorization).

В следующем примере кода показан метод `DeleteAsync` в классе `RequestProvider`:
...
Этот метод вызывает метод `CreateHttpClient`, который возвращает экземпляр класса `HttpClient` с соответствующим набором заголовков. Затем он отправляет асинхронный запрос DELETE к ресурсу, определяемому универсальным кодом ресурса (URI). Дополнительные сведения о методе `CreateHttpClient` см. [в разделе Создание запроса GET](#making_a_get_request).

Когда метод `DeleteAsync` в классе `RequestProvider` вызывает `HttpClient.DeleteAsync`, вызывается метод `Delete` в классе `BasketController` в проекте корзины. API, который показан в следующем примере кода:
...
Этот метод использует экземпляр класса `RedisBasketRepository` для удаления данных корзины из кэша Redis.
...
Мобильное приложение eShopOnContainers использует удаленные образы продуктов, которые пользуются преимуществами кэширования. Эти изображения отображаются элементом управления [`Image`](xref:Xamarin.Forms.Image) и элементом управления `CachedImage`, предоставляемым библиотекой [ффимажелоадинг](https://www.nuget.org/packages/Xamarin.FFImageLoading.Forms/) .

Элемент управления Xamarin. Forms [`Image`](xref:Xamarin.Forms.Image) поддерживает кэширование скачанных изображений. По умолчанию кэширование включено, и образ будет храниться локально в течение 24 часов. Кроме того, срок действия можно настроить с помощью свойства [`CacheValidity`](xref:Xamarin.Forms.UriImageSource.CacheValidity) . Дополнительные сведения см. в статье [кэширование скачанных образов](~/xamarin-forms/user-interface/images.md#downloaded-image-caching).

Элемент управления `CachedImage` Ффимажелоадинг является заменой элемента управления [`Image`](xref:Xamarin.Forms.Image) Xamarin. Forms, предоставляя дополнительные свойства, обеспечивающие дополнительные функции. В этих функциях элемент управления обеспечивает настраиваемое кэширование, обеспечивая поддержку ошибок и загрузку заполнителей изображений. В следующем примере кода показано, как мобильное приложение eShopOnContainers использует элемент управления `CachedImage` в `ProductTemplate`, который является шаблоном данных, используемым элементом управления [`ListView`](xref:Xamarin.Forms.ListView) в `CatalogView`:
...
Элемент управления `CachedImage` задает свойства `LoadingPlaceholder` и `ErrorPlaceholder` для образов, зависящих от платформы. Свойство `LoadingPlaceholder` указывает изображение, отображаемое во время извлечения изображения, указанного свойством `Source`, а свойство `ErrorPlaceholder` указывает изображение, отображаемое при возникновении ошибки при попытке получить изображение, указанное `Source` свойства.

Как следует из названия, элемент управления `CachedImage` кэширует удаленные образы на устройстве на время, заданное значением свойства `CacheDuration`. Если значение этого свойства не задано явно, применяется значение по умолчанию, равное 30 дням.
...
В настоящее время мобильное приложение eShopOnContainers не реализует шаблон повторных попыток при выполнении веб-запросов RESTFUL. Однако элемент управления `CachedImage`, предоставляемый библиотекой [FFImageLoading](https://www.nuget.org/packages/Xamarin.FFImageLoading.Forms/), поддерживает обработку временных сбоев за счет повторной попытки загрузки образа. Если загрузка образа завершается сбоем, будут выполнены дальнейшие попытки. Число попыток указывается свойством `RetryCount`, а повторные попытки выполняются после задержки, указанной свойством `RetryDelay`. Если эти значения свойств не заданы явно, применяются значения по умолчанию – 3 для свойства `RetryCount` и 250 мс для свойства `RetryDelay`. Дополнительные сведения об элементе управления `CachedImage` см. в разделе [кэширование образов](#caching_images).

В эталонном приложении eShopOnContainers реализуется шаблон повторных попыток. Дополнительные сведения, включая обсуждение того, как объединить шаблон повтора с классом `HttpClient`, см. в разделе [микрослужбы .NET: архитектура для контейнерных приложений .NET](https://aka.ms/microservicesebook).
