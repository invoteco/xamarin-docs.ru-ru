...
![](introduction-images/architecture.png "eShopOnContainers архитектуру высокого уровня")
...
Дополнительные сведения о реализации серверных служб см. в разделе [микрослужбы .NET. архитектура контейнерных приложений .NET](https://aka.ms/microservicesebook).
...
|Произведенный эффект|Содержит класс, который используется для изменения цвета границы конкретных `Entry` элементов управления. `EntryLineColorEffect`|
...
|Триггеры|`BeginAnimation` Содержит триггер, который используется для вызова анимации в XAML.|
