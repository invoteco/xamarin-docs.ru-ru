...
ms.locfileid: "70770646"
---
# <a name="native-views-in-xaml"></a>Собственные представления в XAML

[![Скачать пример](~/media/shared/download.png) Скачать пример](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-nativeviews-nativeswitch)
...
[ `Typeface.Create` ](xref:Android.Graphics.Typeface.Create*) Фабричный метод используется для задания [ `TextView.Typeface` ](xref:Android.Widget.TextView.Typeface) свойство в новый [ `Typeface` ](xref:Android.Graphics.Typeface) в Android. `Typeface` Имя семейства и стиля задаются с помощью аргументов метода, которые являются дочерними элементами `x:Arguments` атрибута.
...
![](xaml-images/sub-classed.png "Собственные представления с подклассами")
...
Подклассы Android реализации [ `Spinner` ](xref:Android.Widget.Spinner) представление и предоставляет свойства и события, можно было легко из XAML:
...
`MySpinner` Предоставляет `ItemsSource` и `SelectedObject` свойства и `ItemSelected` событий. Элементов, отображаемых на `MySpinner` предоставленные класс [ `Adapter` ](xref:Android.Widget.Adapter) связанный с представлением, а элементы заполняются в `Adapter` при `ItemsSource` изначально устанавливается свойство. Каждый раз, когда выбранный элемент в `MySpinner` класса изменения, `OnBindableSpinnerItemSelected` обновлениями обработчик событий `SelectedObject` свойство.
