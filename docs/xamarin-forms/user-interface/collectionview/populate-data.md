...
Сведения о том, как изменить `CollectionView` макета, см. в разделе [указывается макет](layout.md). Сведения о том, как определить внешний вид каждого элемента в `CollectionView`, см. в разделе [определения внешнего вида элемента списка](#define-list-item-appearance).
