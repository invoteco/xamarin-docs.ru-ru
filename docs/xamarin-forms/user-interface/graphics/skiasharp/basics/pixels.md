...
Указать текстовую строку, где текст начинается, координаты X и Y и `SKPaint` объекта. Координаты X указывает, где находится слева от текста, но Контрольные значения out: Координата Y определяет положение *базовых показателей* текста. Если вы когда-нибудь от руки на бумаге линией, базовые показатели — важные строки какие sit символов, и более ранних версий опускаются какие подстрочных элементов (например, на буквы g, p, q и y).
