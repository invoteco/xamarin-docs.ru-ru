...
Если вы внимательно посмотреть на результаты, вы увидите, что красный и синий штриховка не ограничен точно Скругленный прямоугольник. (Это очевидно характеристика основного кода Skia.) Если это неудовлетворительно, для диагональных линий в зеленом изображении отображается альтернативный подход: Скругленный прямоугольник используется в качестве обтравочного контура, а линии штриховки рисуются на всей странице.
...
Один из основных способов использования этого метода не затрагивает эффекты пути. `Style` `PathEffect` `GetFillPath` Если свойство `SKPaint` объектаимеетзначениеинеимеетнабора,тосоздаетпуть,представляющийконтурисходногопути,какеслибыонбылштрихом`SKPaintStyle.Stroke` свойства Paint.
