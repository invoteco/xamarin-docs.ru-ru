...
Объект, используемый для обводки половинной черты и уницикле, определяется как поле `UnicycleHalfPipePage` в классе. `SKPaint` Также определен `SKPath` объект для видел:
