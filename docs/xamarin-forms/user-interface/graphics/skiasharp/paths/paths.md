...
- [`MoveTo`](xref:SkiaSharp.SKPath.MoveTo*) Чтобы начать новый профиль
...
- [`LineTo`](xref:SkiaSharp.SKPath.LineTo*) Чтобы добавить прямой путь
- [`ArcTo`](xref:SkiaSharp.SKPath.ArcTo*) для добавления дуги, который является строкой в окружности эллипса или круга
- [`CubicTo`](xref:SkiaSharp.SKPath.CubicTo*) Чтобы добавить сплайна Безье третьего порядка
- [`QuadTo`](xref:SkiaSharp.SKPath.QuadTo*) для добавления квадратичной кривой Безье
- [`ConicTo`](xref:SkiaSharp.SKPath.ConicTo*) Чтобы добавить rational квадратичная кривая Безье, которой можно точно визуализировать conic разделы (кнопку с многоточием, параболы и гиперболы)
...
**Соединения Stroke** странице показан этих трех обводки соединения с кодом, аналогичную **концы штрихов** страницы. Это `PaintSurface` обработчик событий в [ `StrokeJoinsPage` ](https://github.com/xamarin/xamarin-forms-samples/blob/master/SkiaSharpForms/Demos/Demos/SkiaSharpFormsDemos/Paths/StrokeJoinsPage.cs) класса:
