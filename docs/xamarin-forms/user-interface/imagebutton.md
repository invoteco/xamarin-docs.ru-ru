...
> [!NOTE]
> `ImageButton` Класс также имеет [ `Margin` ](xref:Xamarin.Forms.View.Margin) и `Padding` свойства, которые управляют поведением макета `ImageButton`. Дополнительные сведения см. в статье [Поля и заполнение](~/xamarin-forms/user-interface/layouts/margin-and-padding.md).
