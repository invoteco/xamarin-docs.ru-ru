...
Образы можно загружать с [локальный файл](#Local_Images), [внедренный ресурс](#embedded-images), или [загрузки](#Downloading_Images). Кроме того, могут отображаться значки шрифта путем [ `Image` ](xref:Xamarin.Forms.Image) представление, указав данные значка шрифта в `FontImageSource` объекта. Дополнительные сведения см. в разделе [отображать значки шрифта](~/xamarin-forms/user-interface/text/fonts.md#display-font-icons) в [шрифты](~/xamarin-forms/user-interface/text/fonts.md) руководства.
