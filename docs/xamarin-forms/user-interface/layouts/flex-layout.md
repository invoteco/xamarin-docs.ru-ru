...
[`FlexLayout`](xref:Xamarin.Forms.FlexLayout) Xamarin. Forms впервые помещается в Xamarin. Forms версии 3,0. Он основан на [модуле макета гибкой рамки](https://www.w3.org/TR/css-flexbox-1/)CSS, обычно известном как _Flex Layout_ или _Flex-Box_, поэтому вызывается, так как он включает множество гибких параметров для упорядочения дочерних элементов в макете.
...
На странице **Простая стопка** показано, как `FlexLayout` можно заменить на `StackLayout`, но с помощью более простой разметки. Все данные в этом примере определены на странице XAML. `FlexLayout` содержит четыре дочерних элемента:
...
При запуске программы в пяти `Picker` представлениях отображаются значения по умолчанию для этих пяти `FlexLayout` свойств. `FlexLayout` в нижней части экрана содержит три дочерних элемента:
...
`AlignContent` не действует, если имеется только одна строка или столбец.
...
[`AlignSelf`](xref:Xamarin.Forms.FlexLayout.AlignSelfProperty) присоединенное свойство привязки имеет тип [`FlexAlignSelf`](xref:Xamarin.Forms.FlexAlignContent), перечисление с пятью членами:
