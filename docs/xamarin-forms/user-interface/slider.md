...
`Slider` также определяет `DragStarted` и `DragCompleted` событий, которые запускаются в начале и в конце операции перетаскивания. В отличие от [ `ValueChanged` ](xref:Xamarin.Forms.Slider.ValueChanged) событий, `DragStarted` и `DragCompleted` события запускаются только через управление пользователя `Slider`. Когда `DragStarted` вызывает событие `DragStartedCommand`, типа `ICommand`, выполняется. Аналогичным образом, когда `DragCompleted` вызывает событие `DragCompletedCommand`, типа `ICommand`, выполняется.
