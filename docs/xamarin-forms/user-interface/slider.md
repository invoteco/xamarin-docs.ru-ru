...
`Slider`также определяет `DragStarted` и `DragCompleted` события, которые срабатывают в начале и в конце действия перетаскивания. В [`ValueChanged`](xref:Xamarin.Forms.Slider.ValueChanged) `DragCompleted` `Slider`отличие от события, события и срабатывают только при манипуляции пользователя с. `DragStarted` При срабатывании `DragStartedCommand`событиявыполняетсяобъект типа `ICommand`. `DragStarted` Аналогично, при `DragCompleted` срабатывании `DragCompletedCommand`события выполняется тип, типа `ICommand`.
...
`Rotation` Свойства первого `Label` привязан к `Value` свойство `Slider`, так как `Text` свойство второго `Label` с `StringFormat` спецификации. **Основная** страница привязок ползунков работает немного иначе, чем на двух предыдущих страницах: При первом отображении страницы вторая `Label` отображает текстовую строку со значением. Это является преимуществом с использованием привязки данных. Для отображения текста без привязки данных, необходимо специально инициализировать `Text` свойство `Label` или имитировать срабатывание `ValueChanged` событие путем вызова обработчика событий из конструктора класса.
