...
Разметку, аналогичную следующей является основой **VSM для представления** странице в **[VsmDemos](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-vsmdemos)** пример программы. Страницы включает в себя три `Entry` представления, но только второй имеет разметки VSM, подключенные к ней:
...
[![VSM в представлении: Стандартный](vsm-images/VsmOnViewNormal.png "VSM для представления — обычная")](vsm-images/VsmOnViewNormal-Large.png#lightbox)
...
[![VSM в представлении: Посвященный(vsm-images/VsmOnViewFocused.png "VSM в представлении") ]](vsm-images/VsmOnViewFocused-Large.png#lightbox)
...
**Проверки VSM** странице в **[VsmDemos](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-vsmdemos)** примере показано, как использовать Диспетчер визуальных состояний, в связи с проверки входных данных. Файл XAML состоит из двух `Label` элементов, `Entry`, и `Button`:
...
[![Проверка VSM: Недопустимая](vsm-images/VsmValidationInvalid.png "Проверка состояния VSM — недопустимо")](vsm-images/VsmValidationInvalid-Large.png#lightbox)

Если введен правильный номер телефона, текущее состояние становится «Действителен». Второй `Entry` исчезает и `Button` включена:

[![Проверка VSM: Действительная](vsm-images/VsmValidationValid.png "Проверка состояния VSM — допустимая")](vsm-images/VsmValidationValid-Large.png#lightbox)
...
Может возникнуть вопрос: Если файл кода программной части должен ссылаться на каждый объект на странице, на который влияют эти визуальные состояния, то почему файл кода программной части просто напрямую обращается к объектам? Конечно же удалось. Однако преимущество использования VSM — возможности контролировать то, как визуальные элементы реагировать на другое состояние полностью в XAML, который хранит все Дизайн пользовательского интерфейса в одном месте. Это позволяет избежать параметр внешний вид, обратившись к визуальные элементы непосредственно из кода.
...
[![Адаптивный макет VSM: Книжная](vsm-images/VsmAdaptiveLayoutPortrait.png "Адаптивная разметка VSM — книжная")](vsm-images/VsmAdaptiveLayoutPortrait-Large.png#lightbox)

В альбомной ориентации массив кнопки может быть перемещен к одной стороне и отображаемому в столбце:

[![Адаптивный макет VSM: Альбомная](vsm-images/VsmAdaptiveLayoutLandscape.png "Адаптивная структура VSM — альбомная ориентация")](vsm-images/VsmAdaptiveLayoutLandscape-Large.png#lightbox)

Сверху вниз программа выполняется в универсальной платформы Windows, Android и iOS.

**Адаптивный макет VSM** странице в [VsmDemos](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-vsmdemos) образец определяет группу с именем «OrientationStates» с двумя визуальными состояниями с именем «» и «Книжной ориентации». (Более сложный подход может основываться на несколько различных значений ширины страницы или окно.) 
