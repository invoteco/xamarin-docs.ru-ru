...
2. Добавьте пакет NuGet **Xamarin. LiveReload** :
    ...
    2\. **Общие проекты** — установите **Xamarin. LiveReload** NuGet во все проекты платформы (например, Android, iOS, UWP и т. д.). Убедитесь, что для **источника пакета** задано значение **все**.
...
No. На самом деле, можно даже запустить все поддерживаемые целевые приложения (Android, iOS и UWP) на любом числе устройств, симуляторов и эмуляторах, а также просмотреть все обновления одновременно. 
