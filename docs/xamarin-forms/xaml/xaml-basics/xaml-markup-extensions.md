...
Теперь объекты и значения различных типов могут добавляться в словарь ресурсов. Эти типы должны быть допускающий создание экземпляров. Они не могут быть абстрактные классы, например. Эти типы также должны иметь открытый конструктор без параметров. Каждый элемент требует ключ словаря, указанного с помощью `x:Key` атрибута. Например:
...
- Вы можете определить массивы в XAML, используя расширение разметки `x:Array`. Это расширение разметки имеет обязательный атрибут с именем `Type` , указывающее тип элементов в массиве.
